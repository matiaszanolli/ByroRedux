...
**Last verified**: 2026-09-13 (session close — tests **7864**, +110 vs
Session 83's 7754; Rust `src/` LOC **~570 874**, +7 667; total `.rs` LOC
**~614 100**, +7 920; source files **1082** (999 outside `tests/`), +6/+6;
workspace members unchanged at **33**; open issue dirs **4228**, +9.
Session 84 built SKYAL, the sky abstraction layer
([`docs/engine/skyal.md`](docs/engine/skyal.md)). Bloom gained the
bright-pass whose absence had multiplied every exterior sky by a flat 1.71×.
A per-frame sky cubemap bake now feeds RT reflection misses and GI escapes,
which previously returned one direction-blind colour. Volumetric clouds are
marched into both the cubemap and the visible sky, with lighting constants
sourced from Hillaire 2016 and Schneider & Vos 2015. EXAL ground cover got its
visual pass (GI double-count, TLAS cover trace off roads, four-blade tufts,
climate-weighted species, 4× candidate budget). It also stopped translating
`GRAS` into blade species. MQ101 gained chargen-gate effects and a minimal NPC
combat AI (fragment coverage 45.9% → 50.0%), alongside a 43-issue audit-fix
tail. The bench-of-record at `4c9a5b36` is now 136 commits stale.
**R6a-stale-22** is folded below: the harness is unchanged, and the sky-cube
bake, per-pixel cloud march and ground-cover cover trace are all unmeasured
new per-frame GPU work.
...
real content. The live bench-of-record is the stepped-camera refresh above
(HEAD `4c9a5b36`, 2026-09-09); this table is kept only as the historical TAA
baseline the regression is measured against. (This sentence named `3a02b02d`
until 2026-09-09 — two records out of date, since `34074b93` and `2da754e7`
both superseded it. Point it at the LIVE section, not at a specific hash, next
time.)
...
### Bench-of-record (LIVE) — stepped-camera refresh (2026-09-09, HEAD `4c9a5b36`)
...
Ground-truth as of 2026-09-13 (session close, HEAD `673b2145`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~570 874                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~614 100                 |
| Source files (`.rs`, excl. `target/`)   | 1082 total · 999 outside `tests/` dirs (+6 / +6 this session) |
...
| Tests                                   | **7864 passing, 0 failing** (`cargo test --workspace --no-fail-fast`, 2026-09-13). Clean full-workspace run, including doc-tests. Always pass `--no-fail-fast` for the ground-truth count — without it, `cargo test --workspace` stops after the first binary with a failure and silently omits every crate queued behind it (Session 77 saw this first-hand: 1836 vs the true 6905). |
| Open issue directories                  | 4228 (`.claude/issues/`)     |
