...
**Last verified**: 2026-08-09 (Session 64 closeout — tests 4532, +160;
Rust total LOC +~15 258; workspace members unchanged at 27; ~95 issues
closed via a 13-dimension `/audit-suite` sweep + two dozen `/fix-issue`
batches. Bench-of-record (`28155b79`, 2026-08-04) is now 79 commits
stale — Session 63 left it at 37 (R6a-stale-19, not re-run) and this
session added 42 more, *and* landed real renderer-hot-path changes
(shaders, BLAS/TLAS, per-frame draw recording) unlike Session 63's
tail; R6a-stale-19 amended in place per its own fold-instead-of-duplicate
note, escalated to recommend a re-run at the next opportunity rather than
deferred again).
...
Ground-truth as of 2026-08-09 (Session 64 closeout, HEAD `2e5912f5`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~347 050                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~368 673                 |
| Source files (`.rs`, excl. `target/`)   | 859 total · 803 outside `tests/` dirs |
| Workspace members                       | 27 (24 crates + `byroredux` binary + 2 tools) |
| Tests                                   | **4532 passing, 132 ignored** (`cargo test --workspace`, 2026-08-09). |
| Open issue directories                  | 2533 (`.claude/issues/`)     |
