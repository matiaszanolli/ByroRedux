...
|-------------------|---------------|----------------------------------------------|----------------------------------------------------------|
| Oblivion          | BSA v103      | **99.93%** (8 026 / 8 032) · recover 100%    | Interior (Anvil Heinrich Oaken Halls). Exterior parse + load + render ✓ — Tamriel `(0,0)` radius 1 recorded 4,886 entities / 150.6 FPS; the repeatable readiness matrix is tracked by #2377/#2368. `#687` closed (NiGeomMorpherController + NiControllerSequence Phase fixes; 83 truncations recovered). `#688` / `#698` closed; remaining 6 truncated NetImmerse-era Oblivion files (pre-Gamebryo v3.3–v4.2 markers, #1611) tracked under git log (live `nif_stats` over `Oblivion - Meshes.bsa`, 2026-07-11 post-#1900: 0 hard failures, recoverable is a clean 100%). |
