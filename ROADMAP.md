...
- [ ] `PACK` (AI packages): only the Sandbox procedure (type 12 of ~17) has a runtime; selection is schedule + priority + CTDA conditions (M42.2, 2026-07-15 — evaluated through the M47.1 evaluator, fail-open on out-of-catalog functions), but still evaluated once at NPC spawn with no re-evaluation as game time advances. #446 (parse-level stubs) closed — see M42 row above and [docs/engine/npc-spawn-ai-packages.md](docs/engine/npc-spawn-ai-packages.md) for current detail.
