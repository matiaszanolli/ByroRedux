...
| **M44** | Audio (3D spatial)            | **Phases 1+2 shipped (2026-05-05).** Phase 1: new `byroredux-audio` crate built on [`kira`](https://crates.io/crates/kira) `0.10` (game-oriented spatial scene + reverb + streaming/static sound formats). `AudioWorld` resource wraps `AudioManager<DefaultBackend>` with graceful no-device fallback (CI/headless never refuses to boot). ECS components shipped: `AudioListener` (camera marker), `AudioEmitter` (point source w/ `Arc<StaticSoundData>` + `Attenuation` + volume + looping flag), `OneShotSound` (transient marker for play-once-and-remove). `audio_system` registered in `Stage::Late`. **Phase 2**: `load_sound_from_bytes` helper decodes BSA-extracted blobs via `StaticSoundData::from_cursor` (kira's symphonia path covers WAV + OGG by default). `SoundCache` resource: process-lifetime path-keyed `Arc<StaticSoundData>` cache, case-insensitive lookup, `get_or_load` lazy-decode pattern. 7 tests including a real-data integration: vanilla FNV `Fallout - Sound.bsa` (6465 files: 4233 WAV + 2232 OGG) — securitron arm-swing WAV decodes to 27836 frames @ 48 kHz, Goodsprings creak OGG decodes to 52544 frames @ 32 kHz, end-to-end through `BsaArchive::extract → load_sound_from_bytes → StaticSoundData`. **Phases 3–6 pending:** FOOT/IMPD footstep dispatch, REGN ambient soundscapes, MUSC + hardcoded music with crossfade, reverb zones (`kira::ReverbBuilder`) keyed off cell acoustics, raycast-occlusion attenuation. | —                  |
