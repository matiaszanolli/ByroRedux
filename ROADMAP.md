...
**Last verified**: 2026-08-18 (Session 69 closeout — tests 5303, +40, and for
the first time in five sessions **zero failing**; Rust `src/` LOC +3 652;
workspace members unchanged at 27; open issue dirs unchanged at 2963. Read the
green suite precisely: the `fire_lights` derived-reach canary did not get the
remedy its own failure message prescribed — the module it guarded was deleted
(`2325c1de`) when surface illumination from fire moved out of the analytic
primitive and into a reduction of the transported combustion field, and the
test went with it. That also flipped the feature on by default, which is
exactly what the old entry said needed a visual check first. The session drained nine of the 08-16 sweep's
non-renderer findings (PHYSAL convex-hull precondition, BSXFlags marker-child
semantics, gendered/child body meshes, player inventory seed, terrain-LOD
quads, per-game NIF texture slots, embedded-FormID load-order remap, CHARAL
policy selection) and then spent twenty-five commits building a **transported
combustion solver** on top of Session 68's fog profiles: advection around solid
geometry, reaction chemistry, soot formation/oxidation, self-shadowing,
transported overpressure and vorticity, all driven from canonical constants in
`crates/core/src/combustion.rs`. **No milestone opened or closed** — the solver
extends M55's shipped fog slice. Bench-of-record is unchanged at **`34074b93`
(2026-08-14)** and is now **84 commits stale**, nearly triple its 30-commit
gate — tracked as R6a-stale-20 in Known Issues, and materially worse than at
Session 68 close: `volumetrics_inject.comp` grew 1009 → 2625 lines (SPIR-V
64 776 → 156 004 B, 2.4×) this session against a baseline nobody has measured.
CI has also been red on `main` for an unrelated environmental reason — the
runner cannot find `vulkan/vulkan.h` when building `byroredux-fsr3-sys` — which
is tracked in Known Issues below.)
...
5 scenes × 5 configs × 3 runs of 300 frames, median with range, 1280×720
output, per-scene CWD set to that game's `Data/`. 75 runs, zero failures, every
scene-state fingerprint gate passing. Raw rows:
...
**Do not diff this against the superseded tables below.** Three variables moved
at once and the differences are methodology, not performance:

1. **Workload** — `f19f7f15` replaced the parked capture with
   `--bench-mode renderer-stepped --bench-camera orbit`. The camera now moves,
   so every frame pays disocclusion the parked capture never exercised.
2. **Framing** — `76373774` fixed the orbit/dolly radius, which had been the
   camera's distance from the **world origin** rather than from any subject.
   Under the old radius Prospector and Dugout were benchmarking an *empty
   view*: `gpu_main_render` 0.010 ms and 0.048 ms against 1214 and 1805 draws.
   Their apparent "regression" here is those two scenes rendering for the
   first time on this harness.
3. **Engine** — `4de5e78e` corrected FSR's `viewSpaceToMetersFactor` (#2834),
   which by design shifts the SDK's own distance-tuned thresholds. That shift
   is the measurement of the fix, not a regression.

What survives the change — and what the default rests on — is the shape:
FSR Quality is a net win on every scene, `native-aa` is at best break-even,
and the preset ordering is unchanged.

**`native-aa` is a net loss on every scene except Prospector**, where its +1%
sits inside that scene's own ±0.22 ms run spread and is therefore not a win.
Consistent with every prior refresh: it reconstructs at full output resolution,
paying the upscale dispatch while saving no render-resolution work.

**Dugout Inn is the weakest case for FSR Quality (+21%)**, roughly half the
recovery of the other game scenes, while Performance still reaches +59% there.
Its render-resolution work stays a larger share of the frame, so less of the
frame is upscaler-addressable. Not yet investigated.

**Caveat, unchanged from prior refreshes:** shared desktop hardware, not an
isolated bench box. Run spreads are tight (±0.01–0.43 ms) but a resident GPU
consumer would shift the whole table.
...
**Actor motion** — NPCs spawn in bind pose; general AI-package locomotion
animation is not yet decoded (M41.x animation slice). `crates/hkx`
(Session 62) now decodes Skyrim's 64-bit Havok 2010 packfiles —
...
| Starfield         | BA2 v2/v3 LZ4 | **99.99%** aggregate (89 270 / 89 276) · recover 100% (all 5 archives, re-measured 2026-07-25 post-#2201) | Per-archive: Meshes01 **100.00%** (31 058), Meshes02 **100.00%** (7 552), MeshesPatch 99.98% (29 843 / 29 849; 6 truncated), LODMeshes **100.00%** (19 535), FaceMeshes **100.00%** (1 282). Intervening parser work (≥ #1510 BSShaderType155 tail, #1606 starfield_tail, #754 BSWeakReferenceNode, #722 cloth) cleared the Meshes01/LODMeshes truncation tails; the MeshesPatch tail (was 325 truncated, 1.09%, mis-attributed to closed #746/#747) is fixed by #2105. A residual 6/29 849 files with a distinct, unexplained cause remain truncated. **#2201 (2026-07-25):** #2105's 2-byte `BSWeakReferenceNode` gap was gated on `SF_FORM_ID` (173) rather than `SF_WEAK_REF_GAP` (175), silently dropping Meshes02 to 6.10% between 2026-07-21 and 2026-07-25; the figures above are the post-fix re-measurement and confirm both that Meshes02 is restored and that MeshesPatch's 6-file residual is unchanged. |
...
| ~~R4~~  | ~~SWF/GFx strategic decision~~ | **Closed 2026-07-25 — use pinned Ruffle as the Flash VM with ByroRedux-owned Scaleform host profiles, preserving SWF mod compatibility.** Archive-grounded split: Skyrim `HUDMenu.swf` is AVM1/AS2; Fallout 4 is AVM2/AS3; FO3/FNV's XML menus remain a separate legacy-UI track. `byroredux-ui` exposes `ScaleformProfile::{SkyrimAvm1,Fallout4Avm2}` and a bidirectional `ScaleformHostBridge`: Ruffle `ExternalInterfaceProvider` queues typed ActionScript → engine calls, records unknown methods/callback registrations, and `Player::call_internal_interface` drives engine → ActionScript callbacks. M48 corrected Skyrim's real `GameDelegate` contract (method name is the ExternalInterface method; numeric request ID is argument 0; replies re-enter `respond`) and pinned 74 SkyUI call-site methods / 12 callback requests. The Fallout 4 slice recovered the distinct `BGSCodeObj` lifecycle, combines 129 reconstructed-source calls with nine installed Atomic Command holotape calls, and injects an eager 138-method ABC function table that routes namespaced calls through the same typed bridge without exposing Ruffle's private AVM2 internals. Installed BA2 tests assert HUD/Pip-Boy readiness and acknowledged destruction, the child holotape lifecycle boundary, and representative bytecode inventory coverage. | M20                                             |
| M48     | UI integration               | **Host/resource slices advanced 2026-07-26:** Skyrim/SkyUI 74-method catalog + 12 request contracts and re-entrant response routing; Fallout 4 `BGSCodeObj` create/destroy lifecycle + 138 installed-corpus methods + generated AVM2 forwarding adapter and object-aware dispatch. The adapter patches the lifecycle class constructor immediately after `BGSCodeObj` initialization, avoiding Ruffle's stubbed `LoaderInfo.getLoaderInfoByDefinition`; destruction is acknowledged only after the menu hook returns. The archive-backed navigator resolves relative resources through BSA/BA2 providers and pumps Ruffle's local executor. Installed `HUDMenu.swf`, `PipboyMenu.swf`, and `programs\AtomicCommand.swf` checks now cover readiness/destruction ownership and direct/helper-mediated host-call inventory; Atomic Command adds nine methods and correctly owns no standalone lifecycle. Remaining work: method behavior and `_global.gfx` stubs, font fidelity/menu lifecycle, and Papyrus/ECS ↔ UI callbacks. | R4, M20, M47.2                                  |
...
- [ ] `PACK` (AI packages): Sandbox (type 12), Wander (type 5, M42.3), Travel (type 6, M42.4), Follow (type 1, M42.5, 2026-07-16), Escort (type 2, M42.6, 2026-07-16), Guard (type 14, M42.7, 2026-07-16), and Patrol (type 13, M42.8, 2026-07-16, aliases Wander's algorithm) have runtimes now — 10 of ~17 procedures remain parse-only: Find/Eat/Sleep/Accompany/UseItemAt/Ambush/FleeNotCombat/CastMagic/Dialogue/UseWeapon, each blocked on a subsystem (item/furniture-use beyond Sandbox's seat-snap, combat, magic, dialogue) that doesn't exist in the engine yet, not just a missing procedure dispatch. Selection is schedule + priority + CTDA conditions (M42.2, 2026-07-15 — evaluated through the M47.1 evaluator, fail-open on out-of-catalog functions), but still evaluated once at NPC spawn with no re-evaluation as game time advances. #446 (parse-level stubs) closed — see M42 row above and [docs/engine/npc-spawn-ai-packages.md](docs/engine/npc-spawn-ai-packages.md) for current detail.
...
- [x] ~~**R4** SWF/GFx strategic decision~~ — **closed 2026-07-25:** pinned Ruffle VM + Skyrim AVM1 / Fallout 4 AVM2 Scaleform host profiles; bidirectional bridge, Skyrim's 74-method/12-request `GameDelegate` catalog, Fallout 4's 138-method installed-corpus `BGSCodeObj` catalog + injected ABC adapter, BSA/BA2-backed relative imports, and installed HUD/Pip-Boy/holotape lifecycle and inventory checks shipped in `byroredux-ui`
...
Ground-truth as of 2026-08-20 (session close, HEAD `1a428278`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~413 277                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~441 249                 |
| Source files (`.rs`, excl. `target/`)   | 937 total · 873 outside `tests/` dirs |
| Workspace members                       | 27 (24 crates + `byroredux` binary + 2 tools) |
| Tests                                   | **5623 passing, 0 failing, 161 ignored** (`cargo test --workspace`, 2026-08-21). Clean full-workspace run, including doc-tests. |
| Open issue directories                  | 2972 (`.claude/issues/`)     |
...
| **Bench-of-record (LIVE)** — full FSR matrix, all five scenes × TAA-native vs four FSR presets (stepped-camera refresh, HEAD `34074b93`, 2026-08-14, 75 runs = 5×5×3 of 300 frames, median with range, 1280×720 output). Source of every FPS/ms figure in the bench-of-record table above, incl. the FO4 Dugout Inn and Cornell rows that appear nowhere else. Per-scene CWD still applies. Scene subset via `FSR_BENCH_SCENES`, output dir via `FSR_BENCH_OUT`. **Refreshed 2026-08-14 at HEAD `34074b93` (#2835) — first matrix reproducible on the stepped-camera harness; R6a-stale-19 resolved.** | `scripts/fsr-bench-matrix.sh 3 300` |
