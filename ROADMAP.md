...
**Last verified**: 2026-06-03 (Session 46 in-progress — 23 commits since Session 45 closeout (`e6df0f5b..ea044b68`). Notable landings: REND-#1447 (stale SPIR-V) + REND-#1448 (screenshot resize) closed; ReSTIR-DI reservoir Phase 1 landed (`9abbe510`) — reservoir data structs + sampling path wired into `triangle.frag` + `VulkanContext`; `BSEffectShaderProperty.lighting_influence` packed into `GpuMaterial.material_flags` bits 16-23, EFFECT_LIT branch gates scene-lit contribution by authored intensity (`ea044b68`); condition evaluator refactored to integrate `WorldContext` (`0785661d`); 3 perf wins (#1371/#1372/#1374-#1376/#1377-#1379: dirty-Vec / billboard gate / bone-pool contraction); FO4 compat audit filed; 4 BGSM/BGEM effect-material fixes. **Current workspace has compile failures** (`byroredux-sfmaterial`, `byroredux-scripting`, `byroredux-renderer` test — see Known Issues). Bench-of-record `4e2ebe8c` is now **~140 commits stale** (R6a-stale-14 carries forward). See [HISTORY.md](HISTORY.md) Session 46 narrative.
...
| M37.3   | ReSTIR-DI              | Full spatiotemporal reservoir reuse. Drops shadow rays to 1/pixel while sampling hundreds of lights. Streaming-RIS already shipped as M31.5. **Phase 1 landed 2026-06-02** (`9abbe510`): reservoir data structs + initial sampling path wired into `triangle.frag` + `VulkanContext`; `RESERVOIR_FORMAT` + `reservoir_view` added to `GBuffer` (renderer test-suite compile errors pending resolution — see Known Issues). Phase 2 (temporal accumulation + spatial resampling passes) remains. | M31.5, M37 |
...
- [ ] **R6a-stale-14** (filed 2026-06-01, Session 44 close; updated Session 46) Bench-of-record `4e2ebe8c` (2026-05-28) is **~140 commits stale**. Original trigger: Session 44 shipped live perf changes to the ECS/renderer hot path (FxHash keys `ac8d257a`, incremental propagation `ad012f9d`/`99166c84`, terrain LOD ring `601e2969`). Session 45 added 10 more commits touching no hot paths. Session 46 added 23 commits including ReSTIR-DI Phase 1 (`9abbe510`) and EFFECT_LIT (`ea044b68`) which touch `triangle.frag` — the render hot path. Bench should be re-run before R6a-stale-14 can close. The FNV/Skyrim/FO4 steady-state numbers above are provisional pending a `--bench-frames 300` refresh on all three repro commands (run from each game's `Data/` dir per the CWD note). **Carries the still-open R6a-stale-13-collider-cost and current build failures forward** — workspace must compile before re-bench is meaningful.
...
Ground-truth as of 2026-06-03. Session 46 in-progress — workspace has compile failures in 3 crates (see Known Issues BUILD-SFMATERIAL / BUILD-SCRIPTING / BUILD-RENDERER-TEST). Last `/session-close` verification was 2026-06-02 (Session 45).

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~220 400 (Session 45 measure; Session 46 additions modest) |
| Rust total lines (all `.rs`, excl. `target/`) | ~233 600 (updated 2026-06-03)  |
| Source files (`.rs`, excl. `target/`)   | 567 total · 535 outside `tests/` dirs |
| Workspace members                       | 21 (19 crates + `byroredux` binary + `tools/byro-dbg`) |
| Tests (last reported by ROADMAP)        | **2758 passing** (Session 46, 2026-06-03). +6 vs Session 45's 2752. Session 46 build failures (BUILD-SFMATERIAL / BUILD-SCRIPTING / BUILD-RENDERER-TEST / nif type mismatch) all resolved. |
| Open issue directories                  | 1343 (`.claude/issues/`)     |
