...
**Last verified**: 2026-05-23 (Session 41 — Renderer perf-instrumentation tail + R6a-stale-11 closeout + M40 P2 interior↔exterior cell-swap + M28.5 KCC player + Tier-3 push (M27 + M47.0 + M47.1 + M30.2) + 13-issue NIF/BSA bug-bash; bench-of-record stays at `d0b52bd5` (Prospector 120.7 FPS / 8.28 ms) but **R6a-stale-12 triggered** — 63 commits past, M27 scheduler restage + #1195/#1196/#1197 skin chain + #1147 PBR/SSS/MSN flag gating + #1160 sync migration all hit the hot path; see [HISTORY.md](HISTORY.md)).
**Bench-of-record**: Prospector Saloon 120.7 FPS / 8.28 ms @ 2563
entities — at HEAD (`d0b52bd5`), 2026-05-21, wall-clock bench, 300
...
tough FNV interior. Frame is GPU-bound (fence=6.37 ms, 77% of wall;
brd=0.99 ms; draw=6.88 ms with tlas=0.13 / ssbo=0.04 / cmd=0.04 /
submit=0.09). **Per-pass GPU timer** (#1194, new this session):
gpu_skin_disp=0.029 ms, gpu_blas_refit=0.675 ms, gpu_taa=0.053 ms —
the 0.7 ms blas_refit cost is the measurable baseline for #1196
PERF-DIM7-02 conditional-refit work. **Diff from `b5726a18` (122.7
FPS / 8.15 ms / fence=6.12, R6a-stale-10 closeout)**: -1.6% FPS /
+0.13 ms wall_ms / +0.25 ms fence on flat entity count — within the
5% gate. The ~+0.25 ms fence cost is the per-frame TIMESTAMP-query
overhead introduced by #1194's instrumentation; an acceptable price
for the measurement infrastructure unblocking #1195/#1196/#1197.
Companion benches at the same HEAD: Skyrim Whiterun 211.0 FPS /
4.74 ms @ 3210 entities / 1648 draws (vs 211.8 baseline — -0.4%
FPS, within noise); FO4 MedTek 67.9 FPS / 14.72 ms @ 10 912 entities
/ 7364 draws / brd=7.10 ms (vs 68.5 baseline — -0.9% FPS, +0.14 ms
brd, frame still CPU-bound on `build_render_data`). **Texture-archive
...
| Fallout New Vegas | BSA v104      | 100% (14 881)                                | Interior (Prospector 2563 entities @ 120.7 FPS / 8.28 ms on RTX 4070 Ti, post-#1194-followup `d0b52bd5` 2026-05-21; -1.6% vs `b5726a18` 2026-05-17 baseline within 5% gate, ~+0.25 ms fence from new TIMESTAMP instrumentation). Exterior 7×7 (radius 3). |
| Skyrim SE         | BSA v105 LZ4  | 100% (18 862)                                | Interior (WhiterunBanneredMare 3210 entities @ 211.0 FPS / 4.74 ms / 1648 draws, post-#1194-followup `d0b52bd5` 2026-05-21; -0.4% vs 211.8 baseline, within noise. The cell loads 246 unique textures across `Skyrim - Textures0..8.bsa` — repro command must list all 9 archives explicitly since the asset-provider's numeric-sibling auto-load gates on a non-digit suffix). |
| Fallout 4         | BA2 v1/v7/v8  | **96.46%** (33 757 / 34 995) · recover 100%  | Interior (MedTekResearch01 10 912 entities @ 67.9 FPS / 14.72 ms / brd=7.10 ms, post-#1194-followup `d0b52bd5` 2026-05-21; -0.9% vs 68.5 baseline, +0.14 ms brd vs 6.96 baseline — frame still CPU-bound on `build_render_data` per Tier 11 narrative). FaceGen NIFs dominate the truncation tail (1 235 of 1 238 truncated files). |
...
| M40    | World streaming                | **Phase 1a/1b shipped** in Session 23 — `streaming` module with diff logic (`cdfef07`), `WorldStreamingState` wired into App (`80e2966`), async cell-pre-parse worker thread (`592e7bf`), shutdown drain (`7dc354a`). **Phase 2 shipped 2026-05-21** across 3 stages (`f6b9911a`, `1e92a471`, `a7cc9184`): Stage 1 plumbs `DoorTeleport` from REFR XTEL into `PlacedRef` (destination form-id + position + rotation) and adds the `cell_for_refr(form_id) → Option<CellId>` reverse-lookup helper. Stage 3a wires the interior↔interior orchestrator: `script.activate <door_id>` hits a door, tears the source cell down and loads the destination through `load_cell_with_masters`. Stage 3b extends to interior↔exterior with `load_exterior_grid` at the resolved `(wrld, grid)`. Single-cell-at-a-time today. Remaining: multi-cell exterior grid + BLAS streaming (evict/reload) ties into M31's LRU eviction. | M32, M35           |
...
- [ ] **R6a-stale-12** **Threshold tripped 2026-05-23 at Session 41 close (HEAD `4cbf2e6a`, 63 commits past `d0b52bd5` — both threshold limbs exceeded: >30 commits *and* real shader / pipeline / sync change landed).** The #1195/#1196/#1197 skin-chain optimisation work shipped this session (BLAS refit + skin compute dispatch + descriptor-set updates all gated on bone-pose-changed); per the audit's "ship as feature flag, promote after `--bench-hold` test" gate, these MUST re-bench against `d0b52bd5` as the immediate before/after. Other hot-path commits this session: M27 declared-access scheduler re-stage (4 systems moved to exclusive, 12 systems annotated), #1147 / FO4-D6-003 Phase 2b PBR / SSS / model-space-normals shader-flag gating (gates three triangle.frag arms), #1160 DST-side BOTTOM_OF_PIPE → NONE sync migration, #1159 SVGF nearest-tap bit-31 mask, #1198 MAX_PENDING_BIND_INVERSE_UPLOADS_PER_FRAME 16 → 227, #890 Stage 2c greyscale-to-palette LUT shader consumer. Re-run deferred to Session 42 — bench-of-record refresh is the first action item next session.
...
| Claim                                                                     | Command                                                                                                                                                                                        |
|---------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Prospector Saloon 2563 entities @ 120.7 FPS / 8.28 ms, fence=6.37 ms (post-#1194-followup, `d0b52bd5`, 2026-05-21, wall-clock bench) | `cargo run --release -- --esm "Fallout New Vegas/Data/FalloutNV.esm" --cell GSProspectorSaloonInterior --bsa "Fallout - Meshes.bsa" --textures-bsa "Fallout - Textures.bsa" --textures-bsa "Fallout - Textures2.bsa" --bench-frames 300` |
| Skyrim SE WhiterunBanneredMare 3210 entities @ 211.0 FPS / 4.74 ms, 1648 draws (post-#1194-followup, `d0b52bd5`, 2026-05-21). Must list all 9 texture archives explicitly — numeric-sibling auto-load gates on a non-digit suffix and `Textures0.bsa` ends in a digit. | `cargo run --release -- --esm "Skyrim Special Edition/Data/Skyrim.esm" --cell WhiterunBanneredMare --bsa "Skyrim - Meshes0.bsa" --bsa "Skyrim - Meshes1.bsa" --textures-bsa "Skyrim - Textures0.bsa" --textures-bsa "Skyrim - Textures1.bsa" --textures-bsa "Skyrim - Textures2.bsa" --textures-bsa "Skyrim - Textures3.bsa" --textures-bsa "Skyrim - Textures4.bsa" --textures-bsa "Skyrim - Textures5.bsa" --textures-bsa "Skyrim - Textures6.bsa" --textures-bsa "Skyrim - Textures7.bsa" --textures-bsa "Skyrim - Textures8.bsa" --bench-frames 300` |
| FO4 MedTekResearch01 10 912 entities @ 67.9 FPS / 14.72 ms, 7364 draws, brd_ms=7.10 (CPU-bound, post-#1194-followup, `d0b52bd5`, 2026-05-21) | `cargo run --release -- --esm "Fallout 4/Data/Fallout4.esm" --cell MedTekResearch01 --bsa "Fallout4 - Meshes.ba2" --bsa "Fallout4 - MeshesExtra.ba2" --textures-bsa "Fallout4 - Textures1.ba2" --textures-bsa "Fallout4 - Textures2.ba2" --textures-bsa "Fallout4 - Textures3.ba2" --textures-bsa "Fallout4 - Textures4.ba2" --textures-bsa "Fallout4 - Textures5.ba2" --textures-bsa "Fallout4 - Textures6.ba2" --textures-bsa "Fallout4 - Textures7.ba2" --textures-bsa "Fallout4 - Textures8.ba2" --textures-bsa "Fallout4 - Textures9.ba2" --textures-bsa "Fallout4 - TexturesPatch.ba2" --materials-ba2 "Fallout4 - Materials.ba2" --bench-frames 300` |
