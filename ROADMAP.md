...
| ~~R4~~  | ~~SWF/GFx strategic decision~~ | **Closed 2026-07-25 — use pinned Ruffle as the Flash VM with ByroRedux-owned Scaleform host profiles, preserving SWF mod compatibility.** Archive-grounded split: Skyrim `HUDMenu.swf` is AVM1/AS2; Fallout 4 is AVM2/AS3; FO3/FNV's XML menus remain a separate legacy-UI track. `byroredux-ui` now exposes `ScaleformProfile::{SkyrimAvm1,Fallout4Avm2}` and a bidirectional `ScaleformHostBridge`: Ruffle `ExternalInterfaceProvider` queues typed ActionScript → engine calls, records unknown methods/callback registrations, supplies synchronous defaults, and `Player::call_internal_interface` drives engine → ActionScript callbacks. Five synthetic headless tests pin both AVMs, nested values, profile detection, and Skyrim `GameDelegate` normalization without shipping Bethesda assets. This replaces the stale “in-house interpreter vs native rewrite” fork; M48 now extends the method catalog, archive navigator/fonts, input/menu lifecycle, and Papyrus/ECS routing. | M20                                             |
| M48     | UI integration               | Implement the Bethesda Scaleform host-method catalog on R4's bridge, archive-backed navigator/fonts, input routing and menu lifecycle, then Papyrus/ECS ↔ UI callbacks.                                                                                                                                                                                                                                                                                                                              | R4, M20, M47.2                                  |
...
- [x] ~~**R4** SWF/GFx strategic decision~~ — **closed 2026-07-25:** pinned Ruffle VM + Skyrim AVM1 / Fallout 4 AVM2 Scaleform host profiles; bidirectional ExternalInterface bridge and five headless tests shipped in `byroredux-ui`
