...
**Last verified**: 2026-05-21 (Session 40 — FO4 PreCombined Mesh pipeline + 13/13 legacy-compat audit closeout + M29.5/6 GPU bone palette + #1194 per-pass GPU timer + R6a-stale-11 refresh; bench-of-record moves to `d0b52bd5` (Prospector 120.7 FPS / 8.28 ms); see [HISTORY.md](HISTORY.md)).
...
- [ ] **R6a-stale-12** Fresh staleness tracker opened 2026-05-21 at HEAD `d0b52bd5`. Re-run when commit distance from `d0b52bd5` exceeds 30 *or* a real shader / pipeline / sync change lands (same dual-trigger rule as `R6a-stale-9` / `R6a-stale-10` / `R6a-stale-11`). The #1195/#1196/#1197 skin-chain optimisation work, when shipped, MUST re-bench against `d0b52bd5` as the immediate before/after to satisfy the audit's "ship as feature flag, promote after `--bench-hold` test" gate.
...
Ground-truth as of 2026-05-21, verified by `/session-close`.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (non-test)            | ~182 848                     |
| Rust total lines                        | ~193 703                     |
| Source files (non-test)                 | 471                          |
| Workspace members                       | 20                           |
| Tests (last reported by ROADMAP)        | 2310 (Session 39 2264 + Session 40 +46: spawn-site plumbing smoke from FormIdPool / LocalBound / BSXFlags inserts (#1212/#1213/#1214), zero-mesh observability surfaces (#1215/#1216/#1217), FO4 synthetic ESM fixture (#1181), BSLightingShaderProperty BSVER=130 regression pin (#1223), NifVariant v20.0.0.4/u11 ambiguous-routing pin (#1219), exterior XCRI/XPRI parse regression (#1220), plus M29.5/6 GPU bone-palette hotfix coverage (#1191/#1192/#1193)). |
| Open issue directories                  | 1169 (`.claude/issues/`)     |
