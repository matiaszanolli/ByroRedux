...
**Last verified**: 2026-06-03 (Session 46 closeout — build failures resolved; R6a-stale-14 bench run completed at HEAD `1c26bc25`). Notable since Session 45: REND-#1447/1448 closed; ReSTIR-DI Phase 1 (`9abbe510`); EFFECT_LIT `lighting_influence` packing (`ea044b68`); 6 perf wins (#1371–#1379); 4 BGSM/BGEM fixes; `IsCollisionOnly` collider-BLAS gate + ghost-entity rerouting (`1c26bc25`); M24.2 Phase 2 SPEL/ENCH/MGEF/AVIF/INFO decode (`45509f4f`); autostep + step-up camera (`99af1f79`); FO3/FNV PBR/lighting overhaul (`83d6a155`). See [HISTORY.md](HISTORY.md) Session 46.
...
Ground-truth as of 2026-06-03 (Session 46 closeout). All compile failures resolved. Last `/session-close` verification was 2026-06-03 (Session 46).

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~221 700 (Session 46 measure) |
| Rust total lines (all `.rs`, excl. `target/`) | ~234 300 (updated 2026-06-03)  |
...
| Tests (last reported by ROADMAP)        | **2773 passing** (Session 46 closeout, 2026-06-03). +15 vs mid-session 2758. |
| Open issue directories                  | 1350 (`.claude/issues/`)     |
