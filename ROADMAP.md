...
**Last verified**: 2026-08-28 (session closeout — tests **6142**, +146 vs
Session 74's 5996; Rust `src/` LOC **~444 861**, +8 196; workspace members
**28**, unchanged; open issue dirs **3412**, +229. 68 commits landed across
Sessions 74→75, dominated by a full `/audit-suite --preset comprehensive`
run: 25 audits, 34 reports under `docs/audits/`, 143 findings (0 CRITICAL,
21 HIGH), of which 139 were published as issues #3399–#3537 and 15 fixed in
four closeout batches. 103 issues closed by commit keyword this window.
Notable correctness work outside the suite: SSE `SkinPartition` global
indices (#3355/#3360 — 17.6% of skinned triangles were being dropped),
`INAM` as an array (#3356 — 61% of Skyrim outfit items silently lost),
`TPLT` "Use Stats" resolution for every stat model (#3381/#3382), and the
NIFAL soft-lighting mask boundary fix (#3458 — 50.3% of vanilla Skyrim
soft-lighting properties were taking an unauthored `vec3(1.0)`). The
audit-skill path validator is clean (0 stale paths; 240 advisory symbols in
`docs/engine` reference docs, non-fatal). Local `cargo test --workspace` is
green (6142 passing, 0 failing, 168 ignored). The close-time citation audit
added by #3218 ran for the first time on a full window and found a defect in
itself — 62 of the 162 issues it reported as uncited are in fact cited by a
commit in range, a 38% false-positive rate — filed as #3538 and recorded
under Known Issues; its genuine figure for this window is 100 of 203. The
bench-of-record remains `34074b93` (2026-08-14), now **626 commits stale**;
this environment still has no GPU for the required 75-run refresh, and
unlike the previous window this one *did* touch the hot path — see
R6a-stale-20.)
...
| Skyrim SE         | BSA v105 LZ4  | 100% (32 709 across 2 archives — `Skyrim - Meshes0/1.bsa`; measured 2026-08-28) | Interior (WhiterunBanneredMare **3237 entities @ 335.0 FPS / 3.00 ms / ~1298 draws / fence=1.21**, R6a-stale-15 `8a668eff` 2026-07-18, 3-run avg; −7.7% FPS vs R6a-stale-14 (362.8 FPS) at flat entity count — likely a shared-desktop-hardware confound during this bench run, not a code regression; no renderer-path code changed for Whiterun's hot path between the two bench dates. Session 46 perf wins (#1371–#1379) still hold structurally. The cell loads 246 unique textures across `Skyrim - Textures0..8.bsa` — as of the M35 sibling-auto-load fix (2026-06-19) passing just `Skyrim - Textures0.bsa` auto-opens `Textures1..8` (and `Meshes0.bsa` auto-opens `Meshes1.bsa`): the asset provider now treats a `…0`-suffixed archive as Skyrim's zero-based series start, so the older "list all 9 explicitly" workaround is no longer required). |
...
Ground-truth as of 2026-08-28 (session close, HEAD `d5a8c36c`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~444 861                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~477 254                 |
| Source files (`.rs`, excl. `target/`)   | 965 total · 895 outside `tests/` dirs |
| Workspace members                       | 28 (25 crates + `byroredux` binary + 2 tools) |
| Tests                                   | **6142 passing, 0 failing, 168 ignored** (`cargo test --workspace`, 2026-08-28). Clean full-workspace run, including doc-tests. |
| Open issue directories                  | 3412 (`.claude/issues/`)     |
| NIFs in per-game integration sweeps     | 184 886                       |
| Per-game NIF clean-parse rate           | See the [compatibility matrix](#compatibility-matrix) — it is the single home for per-game parse rates, sweep dates and residual truncation tails. Summary only: 100% clean on FO3 / FNV / Skyrim SE / FO4 / FO76 / Oblivion, Starfield 99.99% aggregate; recoverable 100% on all seven. |
