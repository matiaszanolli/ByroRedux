...
| **M44** | Audio (3D spatial)            | **Phase 1 shipped (2026-05-05).** New `byroredux-audio` crate built on [`kira`](https://crates.io/crates/kira) `0.10` (game-oriented spatial scene + reverb + streaming/static sound formats). `AudioWorld` resource wraps `AudioManager<DefaultBackend>` with graceful no-device fallback (CI/headless never refuses to boot). ECS components shipped: `AudioListener` (camera marker), `AudioEmitter` (point source w/ `Arc<StaticSoundData>` + `Attenuation` + volume + looping flag), `OneShotSound` (transient marker for play-once-and-remove). `audio_system` registered in `Stage::Late` (after transform propagation, sees final world poses). 4 tests: headless construction, default attenuation envelope sanity, `StaticSoundData` decoded-frame round-trip via a synthesised 440Hz sine, and empty-World system smoke. **Phases 2–6 pending:** BSA sound extraction (Fallout/Skyrim - Sound.bsa, .wav/.ogg/.xwm), FOOT/IMPD footstep dispatch, REGN ambient soundscapes, MUSC + hardcoded music with crossfade, reverb zones (`kira::ReverbBuilder`) keyed off cell acoustics, raycast-occlusion attenuation. | —                  |
