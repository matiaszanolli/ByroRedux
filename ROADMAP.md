...
**Last verified**: 2026-08-29 (session closeout — tests **6326**, +184 vs
Session 75's 6142; Rust `src/` LOC **~457 475**, +12 614; workspace members
**28**, unchanged; open issue dirs **3412**, unchanged. 34 commits landed
across Sessions 75→76, spent almost entirely closing the 139 issues Session
75's `/audit-suite --preset comprehensive` run filed: 127 issues closed,
mostly in four-issue batches. Two pieces of groundwork came out of the fixing
rather than the filing. **The NIF corpus gates were measuring a third of what
they claimed** — 34.8% of FO76, 70.8% of FO4, 74.1% of Starfield, and Skyrim
SE omitting 715 Creation Club / Anniversary NIFs (#3369, #3466); all four now
walk their full official corpora, taking the swept total from 184 886 to
**603 207** NIFs across seven games. Widening immediately surfaced a
truncation tail no gate could see (FO76 `GeneratedMeshes`, see Known Issues).
**Starfield CDB Phase 2 was unblocked** (#3398): the material lookup key is
`CompiledDB.HashMap`, a 48 749-entry `BSResource::ID → u64` index, and the
hash is reflected CRC-32 (poly `0xEDB88320`) with init 0 and **no** final XOR
over the lowercased backslash path, directory and stem hashed separately —
98.3% match against real NIF-named paths. Other correctness work: the
`dt == 0` priming tick was firing every text key of every looping clip in the
scene (#3470), the animation blend pass never re-applied the weight pass's
empty-key filter (#3471 — a bone at scale 1 blended to 2.0), and the
pre-10.1.0.104 `NiSequence` band lost every channel binding, not just its
text events (#3468). The audit-skill path validator is clean (0 stale paths;
238 advisory symbols in `docs/engine` reference docs, non-fatal). Local
`cargo test --workspace` is green (6326 passing, 0 failing). The close-time
citation audit's own defect (#3538, filed last window at a 38%
false-positive rate) is **fixed**: `rg --quiet` exits on its first match, the
`printf` still feeding its pipe takes SIGPIPE, and `set -o pipefail` promotes
that to "no citation" — size-dependent, which is why a two-line self-test
never caught it. Both call sites now use a here-string. The genuine
zero-citation figure for this window is 32 of 127; the 32%/38% figures quoted
previously came from the broken path. The bench-of-record remains `34074b93`
(2026-08-14), now **660 commits stale**; this environment still has no GPU
for the required 75-run refresh — see R6a-stale-20.)
...
| Starfield         | BA2 v2/v3 LZ4 | **99.98%** aggregate (**120 524 / 120 543**) · recover 100% (all 13 mesh-bearing archives, re-measured 2026-08-29 under #3466 — was 89 276 over 5 archives / 74.1% of the shipped corpus) | Per-archive: Meshes01 **100.00%** (31 058), Meshes02 **100.00%** (7 552), MeshesPatch 99.98% (29 843 / 29 849; 6 truncated), LODMeshes **100.00%** (19 535), FaceMeshes **100.00%** (1 282), **LODMeshesPatch 100.00% (19 540)**, **ShatteredSpace - Main01 99.86% (9 185 / 9 198; 13 truncated)**, and six `SFBGS* - Main` archives (003/004/008/00D/047/050, 2 529 NIFs, all 100.00%). `ShatteredSpace - Main02` and SFBGS006/007 are excluded on measurement, not assumption: they hold 0 NIF entries (Main02 is 14 799 `.ffxanim` + terrain). Intervening parser work (≥ #1510 BSShaderType155 tail, #1606 starfield_tail, #754 BSWeakReferenceNode, #722 cloth) cleared the Meshes01/LODMeshes truncation tails; the MeshesPatch tail (was 325 truncated, 1.09%, mis-attributed to closed #746/#747) is fixed by #2105. A residual 6/29 849 files with a distinct, unexplained cause remain truncated. **#2201 (2026-07-25):** #2105's 2-byte `BSWeakReferenceNode` gap was gated on `SF_FORM_ID` (173) rather than `SF_WEAK_REF_GAP` (175), silently dropping Meshes02 to 6.10% between 2026-07-21 and 2026-07-25; the figures above are the post-fix re-measurement and confirm both that Meshes02 is restored and that MeshesPatch's 6-file residual is unchanged. |
...
Ground-truth as of 2026-08-29 (session close, HEAD `f3f6b60c`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~457 475                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~490 877                 |
| Source files (`.rs`, excl. `target/`)   | 979 total · 909 outside `tests/` dirs |
| Workspace members                       | 28 (25 crates + `byroredux` binary + 2 tools) |
| Tests                                   | **6326 passing, 0 failing** (`cargo test --workspace`, 2026-08-29). Clean full-workspace run, including doc-tests. |
