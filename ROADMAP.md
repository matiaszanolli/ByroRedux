...
- [ ] `PACK` (AI packages): Sandbox (type 12), Wander (type 5, M42.3), Travel (type 6, M42.4), and Follow (type 1, M42.5, 2026-07-16) have runtimes now — 13 of ~17 procedures (Escort/Guard/Patrol/…) remain parse-only. Selection is schedule + priority + CTDA conditions (M42.2, 2026-07-15 — evaluated through the M47.1 evaluator, fail-open on out-of-catalog functions), but still evaluated once at NPC spawn with no re-evaluation as game time advances. #446 (parse-level stubs) closed — see M42 row above and [docs/engine/npc-spawn-ai-packages.md](docs/engine/npc-spawn-ai-packages.md) for current detail.
