...
**Last verified**: 2026-06-22 (Session 51 closeout — tests 3118, +133; src/ LOC +~8 534; bench-of-record now 332 commits stale, see staleness caveat below). Session 51 shipped the **M47.2 compiled-Papyrus vertical slice**: a full `.pex` decompiler (`crates/pex`, a Champollion port — 99.996% of the shipping corpus decompiles to AST), `translate_pex` → the M47.0 recognizer chain (compiled bytecode → ECS behavior, no VM), VMAD-driven cell-load attach via `--scripts-bsa`, the quest-advance recognizer broadened across the `default*SetStage*` family on both `OnActivate` and `OnTriggerEnter`, and a `TriggerVolume` detection system spawned from `XPRM` primitives. See [HISTORY.md](HISTORY.md) Session 51.
...
Ground-truth as of 2026-06-22 (Session 51 closeout). Last `/session-close` verification was 2026-06-22 (Session 51).

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~244 447 (Session 51 measure) |
| Rust total lines (all `.rs`, excl. `target/`) | ~261 644 (updated 2026-06-22)  |
| Source files (`.rs`, excl. `target/`)   | 613 total · 575 outside `tests/` dirs |
| Workspace members                       | 23 (21 crates + `byroredux` binary + `tools/byro-dbg`) |
| Tests (last reported by ROADMAP)        | **3118 passing** (Session 51 closeout, 2026-06-22). +133 vs Session 49. |
| Open issue directories                  | 1527 (`.claude/issues/`)     |
