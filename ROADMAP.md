...
**Last verified**: 2026-08-25 (Session 73 closeout — tests **5972**, +7; Rust
`src/` LOC **434 208**, +735; workspace members unchanged at 27; open issue
dirs unchanged at **3176** (no new issues filed). A 6-commit, single-arc
session on top of Session 72: the canonical `GpuMaterial` gained BGEM v21+
glass optics (`glass_fresnel_color`/`glass_refraction_scale`/
`glass_blur_scale`/`glass_blur_scale_factor`) and the Bethesda soft/rim/back
lighting response (`MAT_FLAG_SOFT_LIGHTING`/`RIM_LIGHTING`/`BACK_LIGHTING`
plus `lighting_effect_1/2`, `subsurface_rolloff`, `rimlight_power`,
`backlight_power`, `fresnel_power`, `grayscale_to_palette_scale`,
`lighting_mask_map_index`, `back_lighting_map_index`), growing the struct
364→396→432 B (92 hashed scalar fields, up from 83) with matching offset and
shader-contract pins. The `--cornell-oracle` ladder grew two rungs: L3/L4
(open/partitioned volumetric non-leakage) and L5 (dielectric/metal/glass/
normal-role material-population probes) — L0-L4 pass on the RTX 4070 Ti; L5
is written and ready but not yet executed in this no-GPU environment. Two
design docs (`m47-2-design.md`, `nif-engine-translation-layer.md`) had their
status sections refreshed. The audit-renderer skill's `GpuMaterial` size
documentation (`gpu_material_size_is_364_bytes`, stale since this session's
own growth) was fixed to `_432_bytes` as part of this close, along with a
matching stale byte-budget comment in `scene_buffer/constants.rs`. Local
`cargo test --workspace` is green (5972 passing, 0 failing, 163 ignored) and
the audit-skill path-reference gate is clean. Several Session 72 fixes remain
open in GitHub only because the multi-issue `98eea9b3` commit did not use
per-issue closing keywords; tracker state must not be read as code state for
#3232/#3233/#3234/#3237/#3238/#3244/#3270/#3280. The bench-of-record remains
`34074b93` (2026-08-14), now **510 commits stale**; this environment still
has no GPU for the required 75-run refresh.)
...
Ground-truth as of 2026-08-25 (session close, HEAD `cdd9aa41`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~434 208                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~465 464                 |
| Source files (`.rs`, excl. `target/`)   | 956 total · 886 outside `tests/` dirs |
| Workspace members                       | 27 (24 crates + `byroredux` binary + 2 tools) |
| Tests                                   | **5972 passing, 0 failing, 163 ignored** (`cargo test --workspace`, 2026-08-25). Clean full-workspace run, including doc-tests. |
