...
**Last verified**: 2026-09-07 (session closeout — tests **7440**, +255 vs
Session 80's 7185; Rust `src/` LOC **~544 825**, +17 782; total `.rs` LOC
**~585 913**, +22 526; source files **1034** (952 outside `tests/`), +55/+50;
workspace members unchanged at **33**; open issue dirs **3941**, +244.
Session 81 ran the audit suite far wider than usual — **24 reports** in four
days (renderer ×4, safety ×2, physics ×2, per-game ×8, plus concurrency, ESM,
NIF, NIFAL, performance, scripting, tech-debt, legacy-compat), including the
first full 23-dimension renderer sweep since 2026-08-30, which alone filed 76
issues (#3976–#4051). **The session's own most notable finding**: the severe
results were not old rot but *recent fixes that had solved only the half a test
could see*. Three of the four most severe renderer findings follow up commits
landed the day before — #3976 (CRITICAL, `709de0e6`'s `bind_inverses` requeue
fixed the bookkeeping while the same frame still built a skinned BLAS from
never-written device memory; fixed in `043dbbb9`), and `fa5c4191`'s VRAM
reservation covering 3 of 11 screen-scaled passes while excluding the default
upscaler (#3992/#3988/#4011) with its resident-vs-paper BLAS accounting inert
because the callee re-gates on the paper figure (#3994/#3979). EXAL ground
cover went from Phase 0 to Phases 1/2/5/6/7 complete (#4052 answered the §11.1
blocking bench; #4054/#4055/#3807/#4057/#4058 landed), Phase 3 (#4056) proposed
and Phase 4 gated on demonstrated need. Seven findings were stale premises
inside the audit skills themselves — one had produced the *same false finding*
in two consecutive sweeps (#3833, closed as "nothing to fix"), another
structurally hid the sixth `GpuInstance` mirror for 13 days. The close-time
citation audit found 129 zero-citation closes of 248 in this range, left
unresolved rather than back-filled with reconstructed archaeology; the orphan
audit found 7 candidates, all category (a) forward-looking references
(#3299/#3308/#3659/#3807/#3816/#4049/#4056), correctly left open. The
bench-of-record `2da754e7` (2026-09-03) is now **119 commits stale** — see
Known Issues.)
...
### Bench-of-record (LIVE) — stepped-camera refresh (2026-09-03, HEAD `2da754e7`)
...
Ground-truth as of 2026-09-07 (session close, HEAD `043dbbb9`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~544 825                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~585 913                 |
| Source files (`.rs`, excl. `target/`)   | 1034 total · 952 outside `tests/` dirs (+55 / +50 this session — EXAL ground cover Phases 1–7 and the audit-driven fix sweep) |
...
| Tests                                   | **7440 passing, 0 failing** (`cargo test --workspace --no-fail-fast`, 2026-09-07). Clean full-workspace run, including doc-tests. Always pass `--no-fail-fast` for the ground-truth count — without it, `cargo test --workspace` stops after the first binary with a failure and silently omits every crate queued behind it (Session 77 saw this first-hand: 1836 vs the true 6905). |
| Open issue directories                  | 3941 (`.claude/issues/`)     |
...
| **Bench-of-record (LIVE)** — full FSR matrix, all five scenes × TAA-native vs four FSR presets (stepped-camera refresh, HEAD `2da754e7`, 2026-09-03, 75 runs = 5×5×3 of 300 frames, median with range, 1280×720 output). Source of every FPS/ms figure in the live bench table above, including the FO4 Dugout Inn and Cornell rows. Per-scene CWD still applies. Scene subset via `FSR_BENCH_SCENES`, output dir via `FSR_BENCH_OUT`. **Refreshed 2026-09-03 at HEAD `2da754e7`; all 75 state-gated runs accepted and R6a-stale-20 resolved.** | `scripts/fsr-bench-matrix.sh 3 300` |
