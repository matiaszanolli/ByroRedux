...
**Last verified**: 2026-09-03 (session closeout — tests **7185**, +184 vs
Session 78's 7001; Rust `src/` LOC **~527 043**, −643 (a 98-file `_tmp_*`
scratch-example cleanup, #3746 + companions, outweighed this session's
additions); workspace members unchanged at **33**; open issue dirs
**3697**, +7. Session 79 ran the standing `/goal` directive ("fix all
github issues directly committing to main, one at a time") to its natural
conclusion across several context compactions — 101 commits, 158 issues
closed, spanning NIF/ESM parser correctness (the largest bucket), the
NIFAL `specular_authored` canonical-boundary fix (#2573, a save-format
major-version bump to v21), and a renderer/physics/audio/scripting tail
(see HISTORY.md Session 79 for the full bucket list). **The session's own
most notable finding**: verifying a lock-order fix (#3655) under
`BYRO_LOCK_ORDER_CHECK=1` surfaced that CI's dedicated lock-order-check
job was fully red — 26 tests failing across four distinct cross-thread
ABBA cycle families, none caused by this session. Traced and closed all
four, plus a self-inflicted perf-test assertion that failed the gate on
its own account; `BYRO_LOCK_ORDER_CHECK=1 cargo test --workspace` went
from 26 failures to **97/97 binaries green** (#3819). The close-time
citation audit found 50 zero-citation closes of 158 in this range, all
already carrying a proper "Fixed in `<hash>`" closing comment (the citing
commits simply predate this session's own commit range); the orphan audit
found 1 candidate (`#3816`, a pre-existing forward-looking scope note,
explicitly marked "not attempted here" — correctly left alone). The
bench-of-record remains `34074b93` (2026-08-14), now **1059 commits
stale**; this environment still has no GPU for the required refresh, and
this session's range is *not* hot-path-quiet — see R6a-stale-20.)
...
Ground-truth as of 2026-09-03 (session close, HEAD `4d78dce6`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~527 043                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~563 387                 |
| Source files (`.rs`, excl. `target/`)   | 979 total · 902 outside `tests/` dirs (−94 / −97 this session — a 98-file `_tmp_*` scratch-example cleanup, #3746 + companions, against 4 new files) |
...
| Tests                                   | **7185 passing, 0 failing** (`cargo test --workspace --no-fail-fast`, 2026-09-03). Clean full-workspace run, including doc-tests. Always pass `--no-fail-fast` for the ground-truth count — without it, `cargo test --workspace` stops after the first binary with a failure and silently omits every crate queued behind it (Session 77 saw this first-hand: 1836 vs the true 6905). |
| Open issue directories                  | 3697 (`.claude/issues/`)     |
