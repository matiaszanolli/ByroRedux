...
| M24.2 Phase 1a | ESM Phase 2 — QUST stages          | **Started 2026-05-24** with QUST stage + objective decoding ([`crates/plugin/src/esm/records/misc/ai.rs`](crates/plugin/src/esm/records/misc/ai.rs) `parse_qust`). Block-state walker decodes `INDX`/`QSDT`/`CNAM`/`SCHR` stage blocks (index + flags + log text + `has_script` marker) and `QOBJ`/`CNAM`/`NNAM`/`QSTA` objective blocks (index + text + target form-id list); `start_up_stage` derived from the first stage carrying QSDT bit 0. 4 new unit tests cover both-game CNAM/NNAM dispatch + start-up detection + empty-block + zero-QSTA-sentinel paths. Plugin suite at **399 passed**. **Remaining for full M24.2**: PERK entry points (3 types — Quest/Ability/Entry Point — gated on the ~120-function catalog), MGEF full effect struct + flags, SPEL/ENCH EFID/EFIT chain, DIAL/INFO full conversation tree, AVIF PERK list lookup, QUST CTDA conditions (already covered by M47.1 at the consumer side — defer per-stage CTDA until needed). | R2             |
