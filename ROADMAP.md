...
|-------------------|---------------|----------------------------------------------|----------------------------------------------------------|
| Oblivion          | BSA v103      | **99.90%** (8 024 / 8 032) · recover 99.99%  | Interior (Anvil Heinrich Oaken Halls). Exterior parse + load ✓ — TES4 worldspace + LAND wiring is implemented and game-agnostic; only an on-device exterior render bench is pending (same shape FO3 was). `#687` closed (NiGeomMorpherController + NiControllerSequence Phase fixes; 83 truncations recovered). `#688` / `#698` closed; remaining 8 truncated NetImmerse-era Oblivion files tracked under git log (live `nif_stats` over `Oblivion - Meshes.bsa`, 2026-06-15). |
...
| Per-game NIF clean-parse rate           | 100% on FO3 / FNV / Skyrim SE; Oblivion 99.90% (2026-06-15 sweep), FO4 96.46%, FO76 97.34%, Starfield 98.6% aggregate (see compat matrix for per-archive breakdown). Recoverable 100% on all except Oblivion 99.99%. Sweep date 2026-04-27 (Oblivion refreshed 2026-06-15). |
