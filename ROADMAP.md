...
| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (non-test)            | ~147 575                     |
| Rust total lines                        | ~153 335                     |
| Source files (non-test)                 | 290                          |
| Workspace members                       | 18                           |
| Tests (last reported by ROADMAP)        | 1827 (Session 31 1811 + Session 32 +16 across the M41-EQUIP Phase 2 close-out — LVLI dispatch via `expand_leveled_form_id` (8 tests), `BSEyeCenterExtraData` FO4/FO76 dispatch (#720, 2 tests), `NiTextureEffect` Phase 1 import (#891, 3 tests), `NiStencilProperty` test/write state capture (#337, 3 tests). Two audits filed (`AUDIT_FNV_2026-05-08.md`, `AUDIT_RENDERER_2026-05-08_DIM11.md`); ~9 GitHub issues closed end-to-end in the same session.) |
| Open issue directories                  | 858 (`.claude/issues/`)       |
