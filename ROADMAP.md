...
**Last verified**: 2026-08-26 (session closeout — tests **5996**, +24 vs
Session 73's 5972; Rust `src/` LOC **~436 665**, +2 457; workspace members
**28**, +1 (`crates/sdk` landed 2026-08-25 after the Session 73 snapshot);
open issue dirs **3183**, +7. Between the Session 73 snapshot and this one,
46 commits landed: the `sdk` crate, a scheduler-access-declaration + physics
lock-order audit-fix batch (#3250-#3267 range), and this session's own
27-issue doc-rot batch spanning audit-skill drift, stale ROADMAP/game-
compatibility parse-rate figures, GpuCamera/GpuMaterial doc-sync sites,
NIF/NIFAL comment drift, and one real code fix (#3040 — FNV's
`recipes`/`recipe_records` counters had COBJ and RCPE backwards; added a
standalone RCPE regression floor). 7 of the 27 doc-rot issues needed no
edit — already fixed by intervening sessions, confirmed and closed with an
explanatory comment rather than re-touched. The audit-skill path/symbol
validator dropped from 16 advisory symbol findings to 7 (the remaining 7
are label-name/doc-format-field false positives, not real drift) after
fixing three genuine ones (`IsCollisionOnly`, `AddonIndex`,
`PlacementLodProvider`) found while gathering this close's ground truth.
Local `cargo test --workspace` is green (5996 passing, 0 failing, 163
ignored). Several Session 72 fixes were open in GitHub only because the
multi-issue `98eea9b3` commit didn't use per-issue closing keywords; six of
the eight (#3232/#3233/#3234/#3237/#3238/#3280) have since closed properly
via individual commits — #3244 and #3270 remain genuinely open, both HIGH
severity, tracked separately from any doc-rot batch. The bench-of-record
remains `34074b93` (2026-08-14), now **557 commits stale**; this
environment still has no GPU for the required 75-run refresh.)
...
Ground-truth as of 2026-08-26 (session close, HEAD `2bcaf1cc`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~436 665                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~467 937                 |
| Source files (`.rs`, excl. `target/`)   | 959 total · 889 outside `tests/` dirs |
| Workspace members                       | 28 (25 crates + `byroredux` binary + 2 tools) |
| Tests                                   | **5996 passing, 0 failing, 163 ignored** (`cargo test --workspace`, 2026-08-26). Clean full-workspace run, including doc-tests. |
| Open issue directories                  | 3183 (`.claude/issues/`)     |
