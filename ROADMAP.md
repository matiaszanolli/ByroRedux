...
- [ ] TES-family player rig never grounds at cell-load spawn (RT-1 / #2013) — **partially fixed** (`e2f75456`, 2026-07-18): added `PhysicsWorld::cast_capsule_down` (a capsule-shaped ground probe alongside the existing ray-cast — a bare ray can slip past sloped/narrow geometry the KCC's own capsule would still clip) and widened the door-spawn XZ nudge to probe downward for the real local floor before trusting door height. Skyrim SE (`WhiterunDragonsreach`) now grounds immediately at frame 0 and stays stable. Oblivion (`ICMarketDistrictTheGildedCarafe`) spawn no longer falls through the floor but `is_grounded` still reads false — a diagnostic probe found the resting contact's surface normal inverted (dot-up ≈ −0.99) independent of spawn accuracy; root cause (likely somewhere in the NiTriStrips-based Oblivion collision import path) not yet isolated. **Now tracked as [#2193](https://github.com/matiaszanolli/ByroRedux/issues/2193)** (HIGH) — filed 2026-07-25 by the Oblivion compat audit, which is the follow-up #2013's own closing comment asked for.
  ...
  **The "Skyrim SE grounds at frame 0" claim above is cell-specific, not game-wide** (2026-07-26). `a88eab6e` fixed two further defects in the same door-spawn probe ladder, both found by running Skyrim's `BleakFallsBarrow01` on real data: the "widen to the whole cell's vertical extent" rung discarded the AABB's `min` and substituted `door_pos.y` for the cell bottom, so it swept 100 of the 5026 BU beneath the spawn; and nothing probed the *un-nudged* door XZ, so a 64 BU inward nudge landing over a hole fell straight through to the deep sweep instead of the solid threshold. `WhiterunDragonsreach` is non-regressive and now answers via the new rung 2 (`"nudge landed over a hole"`) — meaning its nudge was already unreliable and the deep sweep had been silently covering for it. `BleakFallsBarrow01` still free-falls, and **the ladder is not the cause**: two XZ columns swept across the cell's full 7432 BU extent find no fixed collider at all while the cell reports 2560 elsewhere. Filed as **[#2202](https://github.com/matiaszanolli/ByroRedux/issues/2202)** (HIGH) — a collision-import gap with four live candidate mechanisms (per-NIF `collisions_empty` suppressing the trimesh fallback when only *some* shapes resolve; non-zero-mass Dynamic-family architecture invisible to `QueryFilter::exclude_dynamic()`; mis-composed transform; unspawned REFRs), discriminated by one not-yet-written near-spawn collider census. #2202 + #2193 + closed #1832 look like one family — "collision geometry survives parse but arrives wrong."
