...
**Last verified**: 2026-06-02 (Session 45 closeout — M49 FO4 precombined CSG geometry closed (`b93ad7a9..2900de70`); NIFAL surface audit complete (`NiRangeLODData` last unhandled block, `633729f0`); EXAL design introduced (`docs/engine/exal.md`, `00e38caa`); renderer audit `AUDIT_RENDERER_2026-06-02` filed 4 issues (#1447–#1450); ABBA lock-order CI enabled (#1410). See [HISTORY.md](HISTORY.md) Session 45 narrative. **Bench re-run still warranted** (R6a-stale-14 carries forward) — bench-of-record `4e2ebe8c` is now **117 commits stale**. This session touched no renderer hot paths (CSG reader / cell-loader integration / NIFAL surface / lock-order CI), so the FNV/Skyrim/FO4 numbers remain provisional pending the R6a-stale-14 refresh.
...
- [ ] **R6a-stale-14** (filed 2026-06-01, Session 44 close; updated Session 45) Bench-of-record `4e2ebe8c` (2026-05-28) is **117 commits stale**. Original trigger: Session 44 shipped live perf changes to the ECS/renderer hot path (FxHash keys `ac8d257a`, incremental propagation `ad012f9d`/`99166c84`, terrain LOD ring `601e2969`). Session 45 added 10 more commits — none touching renderer/ECS hot paths (CSG reader, cell-loader precombined integration, NIFAL surface, lock-order CI) — so the staleness character is unchanged. The FNV/Skyrim/FO4 steady-state numbers above are provisional pending a `--bench-frames 300` refresh on all three repro commands (run from each game's `Data/` dir per the CWD note). Carries the still-open R6a-stale-13-collider-cost finding forward — the refresh should confirm whether the synthesized-collider RT growth persists.
- [ ] **REND-#1447** HIGH (filed 2026-06-02, `AUDIT_RENDERER_2026-06-02`): Committed SPIR-V stale vs GLSL — pre-compiled `.spv` files need a rebuild after the DoF shaders landed. Tracked as #1447.
- [ ] **REND-#1448** LOW (filed 2026-06-02, `AUDIT_RENDERER_2026-06-02`): Screenshot readback uses stale swapchain extent when a resize and screenshot occur in the same frame. Tracked as #1448.
...
Ground-truth as of 2026-06-02, verified by `/session-close`.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~220 400                     |
| Rust total lines (all `.rs`, excl. `target/`) | ~233 000               |
...
| Tests (last reported by ROADMAP)        | **2752 passing** (Session 45 added +17 over Session 44's 2735). Session 45: M49 FO4 precombined CSG pipeline (5 commits), NIFAL `NiRangeLODData` surface-audit completion, EXAL design introduction, ABBA lock-order CI fix (#1410), animation inline-controller fix (#1440), `NiPSysEmitter` guard (#1411). |
