...
**Last verified**: 2026-08-11 (Session 65 closeout — tests 4660, +128, **one
failing**; Rust total LOC +10 132; workspace members unchanged at 27; the
playable vertical slice opened as the governing execution plan with P0 closed
and the P2 combat fixture frozen, and the physical lighting backbone landed.
Bench-of-record (`28155b79`, 2026-08-04) is now 116 commits stale — Session 64
left it at 79 (R6a-stale-19, not re-run) and this session added 37 more while
again changing renderer hot paths (Schlick Fresnel unification, unshadowed
ambient-fill removal, RGB caustic accumulation, adaptive ray budget, volumetric
emission); R6a-stale-19 amended in place per its own fold-instead-of-duplicate
note, escalated to recommend a re-run **before any further perf claim** rather
than deferred again).
...
Ground-truth as of 2026-08-11 (Session 65 closeout, HEAD `65217327`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~356 976                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~378 805                 |
| Source files (`.rs`, excl. `target/`)   | 868 total · 812 outside `tests/` dirs |
| Workspace members                       | 27 (24 crates + `byroredux` binary + 2 tools) |
| Tests                                   | **4660 passing, 1 failing, 134 ignored** (`cargo test --workspace --no-fail-fast`, 2026-08-11). The failure is the `fire_lights` derived-reach canary — see [Known Issues](#known-issues). Without `--no-fail-fast` the run bails at that failure and reports only 993. |
