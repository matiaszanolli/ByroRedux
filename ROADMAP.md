...
**Last verified**: 2026-06-01 (Session 44 closeout — large mixed session: ~45-issue audit bug-bash (`AUDIT_FNV_2026-05-30` + `AUDIT_SAFETY_2026-06-01` + FO4/NIFAL), a perf arc (ECS incremental transform/bound propagation #ad012f9d / #99166c84, FxHash hot-path keys #ac8d257a, distant-terrain LOD streaming #601e2969), and stochastic depth of field (#400fa68f). See [HISTORY.md](HISTORY.md) Session 44 narrative. **Bench re-run now warranted** — unlike Session 43, this session touched the renderer + ECS hot path (FxHash material/instance keys, incremental propagation, terrain LOD ring); bench-of-record `4e2ebe8c` is now **106 commits stale**. DOF is opt-in/zero-cost at `aperture==0`, but the perf changes are live, so the FNV/Skyrim/FO4 steady-state numbers below are provisional until refreshed (tracked as R6a-stale-14).
...
| Bench | This refresh (`4e2ebe8c`) | Prior record (`a9bbe8d1`) | Δ |
|---|---|---|---|
| **Prospector Saloon** (FNV) | **71.4 FPS / 14.00 ms / fence=11.65 / 3507 ent / 1225 draws** | 161.4 / 6.19 / 2.62 / 2564 ent | FPS −56% · **entities +37%** |
| **Whiterun BanneredMare** (Skyrim SE) | **329.8 FPS / 3.03 ms / fence=1.01 / 3211 ent / 1296 draws** | 287.8 / 3.47 / 0.71 / 3211 ent | **FPS +14.6%** · entities flat |
| **FO4 MedTekResearch01** | **90.7 FPS / 11.02 ms / brd=2.63 / fence=4.73 / 15546 ent / 8304 draws** | 102.1 / 9.79 / 7.81 / 10 913 ent | FPS −11% · **entities +42%** · brd −66% |

**Interpretation — content growth on FNV/FO4, NOT a per-frame regression.**
Whiterun is the control: its entity count is *unchanged* (3211 = 3211) and
FPS *rose* +14.6%, so the steady-state hot path did not regress — it
benefited from the session's optimizations. The two benches that slowed
(Prospector, MedTek) both grew entity counts ~40%. FNV/FO4 cell
architecture lacks authored `bhk` collision, so it spawns synthesized
static-trimesh colliders (the #1294 fallback-gate change is the prime
suspect); each synthesized collider adds RT BLAS geometry → Prospector's
`fence` ballooned 2.62 → 11.65 ms (super-linear vs the +37% entity count).
Skyrim, which ships real `bhk` collision, did not grow and did not slow.
MedTek's `brd_ms` actually *improved* 7.81 → 2.63 (CPU win held); its GPU
pays for the +42% entities. **Flagged for follow-up: the FNV/FO4
entity-count growth (esp. Prospector 2564 → 3507) and its FPS cost is a
likely-unintended side-effect of the synthesized-collider gate — needs a
fix-issue to confirm the collider count is intended and bound its RT cost
(see Known Issues R6a-stale-13 finding).**

**Repro-command CWD note (this refresh):** the documented commands pair a
sub-path `--esm` with bare `--bsa` names; bare BSA names resolve against
the **current working directory**, not the ESM's folder. Run each bench
with CWD set to that game's `Data/` directory (e.g.
`cd ".../Fallout New Vegas/Data"`) or the mesh/texture archives silently
fail to open and the scene loads near-empty (36 entities / 3 meshes on
Prospector — a partial load that reads as a spurious 1792 FPS). The
Repro-command table below is written for CWD = `Data/`.
Scene is glass-heavy (bottles, pitcher, marquee sign); RT refraction/
reflection cost is representative of a tough FNV interior. Frame is
GPU-bound (fence=6.17 ms, 77% of wall; brd=1.13 ms; draw=6.51 ms
with tlas=0.13 / ssbo=0.02 / cmd=0.04 / submit=0.09). **Diff from
`220e8e1` (133.5 FPS / 7.49 ms @ 2562 entities, 2026-05-11)**: -6.7%
FPS / +0.54 ms wall_ms / +0.36 ms fence on flat entity count. The
larger -18.5% regression that surfaced at `c8519082` is *closed in
this session* — bisect identified `6059e2ab` "Pick off 4 TLAS /
acceleration LOWs from bundle #926" (REN-D8-NEW-08 flipped skinned
BLAS BUILD+UPDATE flags from `PREFER_FAST_BUILD` to
`PREFER_FAST_TRACE`). Splitting the shared `UPDATABLE_AS_FLAGS`
constant into per-acceleration-type pairs (TLAS keeps `FAST_TRACE`,
skinned BLAS reverts to `FAST_BUILD`) recovered ~+15 FPS / -0.87 ms
fence on Prospector. Residual 6.7% gap vs baseline is from the
remaining 190 commits in the window (cumulative shader work: TAA
gamma widen, SVGF per-FIF + NEAREST, BSGeometry UDEC3 tangents,
volumetric changes) — not skinned-BLAS related; left for a future
session if it grows. Companion benches at the same HEAD: Skyrim
Whiterun 218.4 FPS / 4.58 ms @ 3210 entities / 1641 draws (was
217.3 baseline — within noise), FO4 MedTek 67.1 FPS / 14.91 ms @
10 810 entities / 7359 draws / brd_ms=8.07 (was 68.5 baseline —
-2.0%, CPU-bound on `build_render_data`). See Repro commands.
...
The live gaps: **Oblivion exterior** needs TES4 worldspace + LAND
wiring (same shape FO3 was in the pre-cell-loader era — the
...
| Fallout New Vegas | BSA v104      | 100% (14 881)                                | Interior (Prospector **3507 entities @ 71.4 FPS / 14.00 ms / fence=11.65** on RTX 4070 Ti, R6a-stale-13 refresh `4e2ebe8c` 2026-05-28; entity count grew +37% vs the 2564 `a9bbe8d1` record — FNV architecture has no `bhk` collision so it spawns synthesized static-trimesh colliders, each adding RT BLAS; the `fence` rose 2.62 → 11.65 ms super-linearly. Flagged for follow-up — see Known Issues R6a-stale-13). Exterior 7×7 (radius 3). |
| Skyrim SE         | BSA v105 LZ4  | 100% (18 862)                                | Interior (WhiterunBanneredMare **3211 entities @ 329.8 FPS / 3.03 ms / 1296 draws / fence=1.01**, R6a-stale-13 refresh `4e2ebe8c` 2026-05-28; **+14.6% FPS vs the 287.8 `a9bbe8d1` record at identical 3211 entity count** — Whiterun is the control bench proving the steady-state hot path did not regress over the 125-commit window. Skyrim ships real `bhk` collision so it did not grow the synthesized-collider count that slowed FNV/FO4. The cell loads 246 unique textures across `Skyrim - Textures0..8.bsa` — repro command must list all 9 archives explicitly since the asset-provider's numeric-sibling auto-load gates on a non-digit suffix). |
| Fallout 4         | BA2 v1/v7/v8  | **96.46%** (33 757 / 34 995) · recover 100%  | Interior (MedTekResearch01 **15546 entities @ 90.7 FPS / 11.02 ms / 8304 draws / brd=2.63 ms / fence=4.73**, R6a-stale-13 refresh `4e2ebe8c` 2026-05-28; entities grew +42% vs the 10 913 `a9bbe8d1` record (same synthesized-collider growth as FNV); FPS −11% but **`brd_ms` improved 7.81 → 2.63** — the CPU win held and the frame is no longer CPU-bound, the GPU now pays for the larger scene). FaceGen NIFs dominate the truncation tail (1 235 of 1 238 truncated files). |
...
**Tripwire today** (re-measured 2026-05-28): the old `esm/cell.rs`
...
source file is `crates/renderer/src/vulkan/context/draw.rs` at 3 337
lines (per-frame command recording) — under the 3 500 line threshold
but the closest to it; `vulkan/context/mod.rs` (3 017) and
`byroredux/src/asset_provider.rs` (2 561) follow. Watch draw.rs as
GPU-driven rendering (M52) plumbing lands. R2 (typed subrecord
...
material / particle / collision (see Status above). Per-game
...
- [ ] **R6a-stale-13-collider-cost** (follow-up filed by the refresh above) The FNV/FO4 synthesized-collider entity growth (Prospector 2564 → 3507, MedTek 10 913 → 15546) and its RT cost (Prospector `fence` 2.62 → 11.65 ms, −56% FPS) is a likely-unintended side-effect of #1294's `base_layer` fallback-gate change. Needs a fix-issue to (a) confirm the synthesized-collider count on FNV/FO4 cells is intended (not double-spawning architecture as both render mesh + trimesh collider), and (b) bound the RT-BLAS cost of collider-only geometry — collider trimeshes should likely be excluded from the ray-traced BLAS set since they're physics proxies, not visible surfaces.
- [ ] **R6a-stale-14** (filed 2026-06-01, Session 44 close) Bench-of-record `4e2ebe8c` (2026-05-28) is 106 commits stale, and unlike Session 43 this session shipped live perf changes to the ECS/renderer hot path: FxHash material/instance keys (`ac8d257a`), incremental world-bound + transform propagation (`ad012f9d` / `99166c84`), query storage-downcast cache (`c6e983de`), and distant-terrain LOD streaming (`ce4a0cf8` / `bdd6563b` / `601e2969`). The FNV/Skyrim/FO4 steady-state numbers above are provisional pending a `--bench-frames 300` refresh on all three repro commands (run from each game's `Data/` dir per the CWD note). Depth of field (`400fa68f`) is opt-in/zero-cost at `aperture==0` and excluded from the concern. Carries the still-open R6a-stale-13-collider-cost finding forward — the refresh should confirm whether the synthesized-collider RT growth persists.
...
Ground-truth as of 2026-06-01, verified by `/session-close`.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~218 000                     |
| Rust total lines (all `.rs`, excl. `target/`) | ~230 500               |
| Source files (`.rs`, excl. `target/`)   | 561 total · 530 outside `tests/` dirs |
| Workspace members                       | 21 (19 crates + `byroredux` binary + `tools/byro-dbg`) |
| Tests (last reported by ROADMAP)        | ~2735 passing (Session 44 added +52 over Session 43's ~2683). Session 44 was a large mixed session: a ~45-issue audit bug-bash (`AUDIT_FNV_2026-05-30` + `AUDIT_SAFETY_2026-06-01` + FO4/NIFAL — FNV texture-refcount leaks #1338/#1340/#1343, FO4 BGSM Disney/LUT #1352/#1353, AS scratch-alignment round-up #1386, water caustic floor-ray bias #1388), a perf arc (ECS incremental transform/bound propagation, FxHash hot-path keys, distant-terrain LOD streaming), and stochastic depth of field (#400fa68f). |
| Open issue directories                  | 1332 (`.claude/issues/`)     |
...
| Claim                                                                     | Command                                                                                                                                                                                        |
|---------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Prospector Saloon 3507 entities @ **71.4 FPS / 14.00 ms / fence=11.65 ms** (R6a-stale-13, `4e2ebe8c`, 2026-05-28; entities +37% vs prior record — synthesized-collider growth, see Bench-of-record) | (CWD = `.../Fallout New Vegas/Data`) `cargo run --release -- --esm FalloutNV.esm --cell GSProspectorSaloonInterior --bsa "Fallout - Meshes.bsa" --textures-bsa "Fallout - Textures.bsa" --textures-bsa "Fallout - Textures2.bsa" --bench-frames 300` |
| Skyrim SE WhiterunBanneredMare 3211 entities @ **329.8 FPS / 3.03 ms / 1296 draws / fence=1.01 ms** (R6a-stale-13, `4e2ebe8c`, 2026-05-28; +14.6% vs prior record at flat entity count — the control bench). Must list all 9 texture archives explicitly — numeric-sibling auto-load gates on a non-digit suffix and `Textures0.bsa` ends in a digit. | (CWD = `.../Skyrim Special Edition/Data`) `cargo run --release -- --esm Skyrim.esm --cell WhiterunBanneredMare --bsa "Skyrim - Meshes0.bsa" --bsa "Skyrim - Meshes1.bsa" --textures-bsa "Skyrim - Textures0.bsa" --textures-bsa "Skyrim - Textures1.bsa" --textures-bsa "Skyrim - Textures2.bsa" --textures-bsa "Skyrim - Textures3.bsa" --textures-bsa "Skyrim - Textures4.bsa" --textures-bsa "Skyrim - Textures5.bsa" --textures-bsa "Skyrim - Textures6.bsa" --textures-bsa "Skyrim - Textures7.bsa" --textures-bsa "Skyrim - Textures8.bsa" --bench-frames 300` |
| FO4 MedTekResearch01 15546 entities @ **90.7 FPS / 11.02 ms / 8304 draws / brd_ms=2.63 / fence=4.73** (R6a-stale-13, `4e2ebe8c`, 2026-05-28; entities +42% vs prior record; brd improved 7.81 → 2.63 — no longer CPU-bound) | (CWD = `.../Fallout 4/Data`) `cargo run --release -- --esm Fallout4.esm --cell MedTekResearch01 --bsa "Fallout4 - Meshes.ba2" --bsa "Fallout4 - MeshesExtra.ba2" --textures-bsa "Fallout4 - Textures1.ba2" --textures-bsa "Fallout4 - Textures2.ba2" --textures-bsa "Fallout4 - Textures3.ba2" --textures-bsa "Fallout4 - Textures4.ba2" --textures-bsa "Fallout4 - Textures5.ba2" --textures-bsa "Fallout4 - Textures6.ba2" --textures-bsa "Fallout4 - Textures7.ba2" --textures-bsa "Fallout4 - Textures8.ba2" --textures-bsa "Fallout4 - Textures9.ba2" --textures-bsa "Fallout4 - TexturesPatch.ba2" --materials-ba2 "Fallout4 - Materials.ba2" --bench-frames 300` |
