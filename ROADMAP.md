...
**Last verified**: 2026-05-08 (post-Session 31, cell-load perf bundle
on streaming-in critical path — REFR placement dedup + NPC spawn cache
+ batched DDS uploads + batched StringPool lock + unload_cell single
fan-out + dirty-gated material SSBO + tracing spans — M41-EQUIP scaffold
in `Inventory` / `EquipmentSlots` / `ItemInstancePool` + per-game
ARMO → worn-mesh resolver wired into both spawn paths, REFR Euler→Y-up
composition order pinned, REN-D15 audit closed, NIF-PERF cluster
collapsed `Vec<String>` → `Vec<Arc<str>>` + `read_pod_vec` triple-array
extension across 5 sites, audio reverb-send finally wired to interior /
exterior cell type — see [HISTORY.md](HISTORY.md)).
...
- [ ] **R6a-stale-7** Bench-of-record `6a6950a` is now 419 commits stale. Sessions 24-25 stacked M41.0 Phases 0–4 + R1 MaterialTable refactor; Session 28 added the RenderLayer depth-bias ladder + Frostbite light falloff curve + `env_map_scale` ≠ metalness fix; Session 29 was bug-bash + parser fixes (no rendering hot-path changes that would invalidate the bench, but the rayon parallel parse `#830` may shorten cell-stream latency on radius-3 grids — companion telemetry work needed before that's measurable). Session 30 shipped M44 audio (no rendering-path impact) + cell-streaming hardening + concurrency audit closeouts. Session 31 shipped the cell-load perf bundle (#878–#883, #886) — improvements land on the streaming-in critical path, not the steady-state per-frame path the bench measures, so the existing bench remains representative; plus the M41-EQUIP scaffold (#896 Phases A.0 → B.2) which will materially change the visible-actor mix the moment Megaton-scale NPCs render with default outfits, REN-D15 fixes (cloud scroll, fog night_factor, interior-fill ambient docstring), and the REFR Euler→Y-up composition fix (`Rx · Ry · Rz`, was `Rz · Ry · Rx`) — the rotation fix touches every REFR placement so it's the closest the session got to invalidating the bench, but Prospector is interior-only and uses authored placements that fall in the trivial Euler subspace where the order doesn't matter empirically. Refresh still deferred until M41 lands the visible-actor workload that exercises the new code paths. Not blocking.
...
Ground-truth as of 2026-05-08, verified by `/session-close`.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (non-test)            | ~146 399                     |
| Rust total lines                        | ~152 159                     |
| Source files (non-test)                 | 289                          |
| Workspace members                       | 18                           |
| Tests (last reported by ROADMAP)        | 1811 (Session 30 1729 + Session 31 +82 across M41-EQUIP scaffold — Inventory + EquipmentSlots + ItemInstancePool + per-game ARMO resolver, REN-D15 closeouts — climate-driven fog night_factor + cumulative interior-fill scaling docstring + distinct cloud layer 2/3 scroll velocities, NIF parser perf cluster — Arc<str> refcount-bump on NiUnknown + walk.rs resolvers + bulk-read triangle indices + bulk header arrays + zero-copy morph deltas, audio polish — multi-listener warn debounce + stop_issued flag + reverb-send wired to cell type + VecDeque queue + drain ordering, parser correctness — BSDynamicTriShape silent-import warn + SCOL FULL display name + half-float skin weight renorm + opt-in validate_links, REFR Euler→Y-up composition fix, scheduler panic + reentry doc) |
| Open issue directories                  | 853 (`.claude/issues/`)       |
