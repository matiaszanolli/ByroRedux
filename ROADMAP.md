...
**Last verified**: 2026-05-28 (Session 42 closeout — #1277 NIF translation-layer epic + 5 children closed; Starfield bring-up `no parser` → `walkable Cydonia` in 5 days via #1289 / #1291 / #1292 / #1294 / #1295; Disney BSDF + water caustics #1248-1257 shipped; R6a-stale-12 closeout `c9ad33f0` delivered +33-50% FPS across all three benches (Prospector / Whiterun / MedTek). See [HISTORY.md](HISTORY.md) Session 42 narrative. Bench-of-record `a9bbe8d1` now 112 commits stale — R6a-stale-13 tracker filed in Known Issues).
**Bench-of-record**: Prospector Saloon 161.4 FPS / 6.19 ms @ 2564
entities — at HEAD (`a9bbe8d1`), 2026-05-24, wall-clock bench, 300
frames, RTX 4070 Ti @ 1280×720. Scene is glass-heavy (bottles, pitcher,
marquee sign); RT refraction/reflection cost is representative of a
tough FNV interior. Frame is GPU-bound (fence=2.62 ms, 42% of wall;
brd=1.17 ms; draw=3.03 ms with tlas=0.07 / ssbo=0.03 / cmd=0.04 /
submit=0.09). **Per-pass GPU timer**: gpu_skin_disp=0.000 ms,
gpu_blas_refit=0.001 ms, gpu_taa=0.050 ms — the paired
#1195+#1196 bone-pose dirty gate has reduced the skin-chain cost
from 0.029 + 0.675 = 0.704 ms (R6a-stale-11 baseline) to effectively
zero on the steady-state Prospector frame where NPCs are idle.
**Diff from `d0b52bd5` (120.7 FPS / 8.28 ms / fence=6.37,
R6a-stale-11 closeout)**: **+33.7% FPS / −25.2% wall_ms / −58.9%
fence_ms** on flat entity count — far exceeds the 5% gate in the WIN
direction. Companion benches at the same HEAD: Skyrim Whiterun
287.8 FPS / 3.47 ms / fence=0.71 @ 3211 entities / 1593 draws
(was 211.0 / 4.74 / 2.12 — **+36.4% FPS, −66.5% fence_ms**; the
gain is rasterization-side via #1259+#1260 since Whiterun's NPCs
land in first-sight and don't refit); FO4 MedTek 102.1 FPS / 9.79
ms @ 10 913 entities / 7363 draws / brd=7.81 ms / fence=0.44
(was 67.9 / 14.72 / 7.10 — **+50.4% FPS, −33.5% wall_ms**;
brd_ms went up +0.71 ms but the GPU win dominates, frame still
CPU-bound on `build_render_data` per Tier 11 narrative). **Texture-archive
gotcha fixed in this refresh**: prior Skyrim repro command listed only
`Textures0/1/2.bsa` but SE ships through Textures8; the visual A/B
showed magenta-checker placeholders on furniture / rugs that the
headless bench summary couldn't surface. New repro command lists
all 9 archives explicitly (numeric-sibling auto-load doesn't fire
when the explicit path ends in a digit). See Repro commands below.
...
| Fallout New Vegas | BSA v104      | 100% (14 881)                                | Interior (Prospector 2564 entities @ **161.4 FPS / 6.19 ms / fence=2.62** on RTX 4070 Ti, post-#1195/#1196/#1197 skin-chain dirty gate `a9bbe8d1` 2026-05-24; **+33.7% FPS / −58.9% fence_ms vs `d0b52bd5` baseline** — the paired bone-pose dirty gate drops skin_dispatch+blas_refit from 0.704 ms to ~0). Exterior 7×7 (radius 3). |
| Skyrim SE         | BSA v105 LZ4  | 100% (18 862)                                | Interior (WhiterunBanneredMare 3211 entities @ **287.8 FPS / 3.47 ms / 1593 draws / fence=0.71**, post-#1195/#1196/#1197 + #1259/#1260 `a9bbe8d1` 2026-05-24; **+36.4% FPS / −66.5% fence_ms vs 211.0 baseline** — gain is rasterization-side via blend-pipeline fast path + off-frustum skip since NPCs land in first-sight without refit. The cell loads 246 unique textures across `Skyrim - Textures0..8.bsa` — repro command must list all 9 archives explicitly since the asset-provider's numeric-sibling auto-load gates on a non-digit suffix). |
| Fallout 4         | BA2 v1/v7/v8  | **96.46%** (33 757 / 34 995) · recover 100%  | Interior (MedTekResearch01 10 913 entities @ **102.1 FPS / 9.79 ms / 7363 draws / brd=7.81 ms / fence=0.44**, post-#1195-1197 + #1259/#1260 `a9bbe8d1` 2026-05-24; **+50.4% FPS / −33.5% wall_ms vs 67.9 baseline** — GPU win dominates the +0.71 ms brd cost, frame still CPU-bound on `build_render_data` per Tier 11 narrative). FaceGen NIFs dominate the truncation tail (1 235 of 1 238 truncated files). |
...
- [ ] **R6a-stale-13** Threshold tripped 2026-05-28 at Session 42 close (HEAD `dabf89b2`, 112 commits past `a9bbe8d1` — well past the 30-commit gate). Notable post-bench landings: #1284 SkinSlotPool 3-step bump (32 768 → 196 608 bones, cap 226 → 1 365) + descriptor-pool pin + spill telemetry; #1294 static-trimesh fallback gate-on-`base_layer` (was `final_layer` — silently lost SF colliders, also touches FO4 sub-decomposed architecture); #1292 BSGeometry `geometries\<hash>.mesh` canonical-path resolution (Cydonia spawn-rate 0.27% → 99.7%); #1289 Starfield CDB consumer Phase 1; #1291 XCLL 108-byte split. **Most post-bench commits target the SF / NPC / audit / KCC surface, not the FNV/Skyrim/FO4 steady-state hot path** — Prospector + Whiterun bench expected to hold within noise; #1294 collider-gate change may shift MedTek slightly (FO4 sub-decomposed architecture may now gain colliders that pre-#1294 silently lost). Re-run deferred to next session ahead of any Tier-3 / Tier-5 milestone change. Bench-of-record stays at `a9bbe8d1` until refresh.
...
| Claim                                                                     | Command                                                                                                                                                                                        |
|---------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Prospector Saloon 2564 entities @ **161.4 FPS / 6.19 ms / fence=2.62 ms** (post-#1195/#1196/#1197 skin-chain dirty gate, `a9bbe8d1`, 2026-05-24, wall-clock bench) | `cargo run --release -- --esm "Fallout New Vegas/Data/FalloutNV.esm" --cell GSProspectorSaloonInterior --bsa "Fallout - Meshes.bsa" --textures-bsa "Fallout - Textures.bsa" --textures-bsa "Fallout - Textures2.bsa" --bench-frames 300` |
| Skyrim SE WhiterunBanneredMare 3211 entities @ **287.8 FPS / 3.47 ms / 1593 draws / fence=0.71 ms** (post-#1195-#1197 + #1259/#1260, `a9bbe8d1`, 2026-05-24). Must list all 9 texture archives explicitly — numeric-sibling auto-load gates on a non-digit suffix and `Textures0.bsa` ends in a digit. | `cargo run --release -- --esm "Skyrim Special Edition/Data/Skyrim.esm" --cell WhiterunBanneredMare --bsa "Skyrim - Meshes0.bsa" --bsa "Skyrim - Meshes1.bsa" --textures-bsa "Skyrim - Textures0.bsa" --textures-bsa "Skyrim - Textures1.bsa" --textures-bsa "Skyrim - Textures2.bsa" --textures-bsa "Skyrim - Textures3.bsa" --textures-bsa "Skyrim - Textures4.bsa" --textures-bsa "Skyrim - Textures5.bsa" --textures-bsa "Skyrim - Textures6.bsa" --textures-bsa "Skyrim - Textures7.bsa" --textures-bsa "Skyrim - Textures8.bsa" --bench-frames 300` |
| FO4 MedTekResearch01 10 913 entities @ **102.1 FPS / 9.79 ms / 7363 draws / brd_ms=7.81** (CPU-bound on `build_render_data`, post-#1195-#1197 + #1259/#1260, `a9bbe8d1`, 2026-05-24) | `cargo run --release -- --esm "Fallout 4/Data/Fallout4.esm" --cell MedTekResearch01 --bsa "Fallout4 - Meshes.ba2" --bsa "Fallout4 - MeshesExtra.ba2" --textures-bsa "Fallout4 - Textures1.ba2" --textures-bsa "Fallout4 - Textures2.ba2" --textures-bsa "Fallout4 - Textures3.ba2" --textures-bsa "Fallout4 - Textures4.ba2" --textures-bsa "Fallout4 - Textures5.ba2" --textures-bsa "Fallout4 - Textures6.ba2" --textures-bsa "Fallout4 - Textures7.ba2" --textures-bsa "Fallout4 - Textures8.ba2" --textures-bsa "Fallout4 - Textures9.ba2" --textures-bsa "Fallout4 - TexturesPatch.ba2" --materials-ba2 "Fallout4 - Materials.ba2" --bench-frames 300` |
