...
| **M44** | Audio (3D spatial)            | **Phases 1+2+3+3.5 shipped (2026-05-05).** Phases 1+2+3: `byroredux-audio` crate on [`kira`](https://crates.io/crates/kira) `0.10` — `AudioWorld` + `AudioListener` / `AudioEmitter` / `OneShotSound` ECS components; BSA decode via `StaticSoundData::from_cursor` + `SoundCache`; `audio_system` runs the spatial sub-track model with lazy listener creation, per-emitter `SpatialTrackHandle`, prune-on-Stopped, `spawn_oneshot_at` helper. Real cpal-driven full lifecycle test passes in 0.63s on an FNV `npc_securitron_armswing_02.wav`. **Phase 3.5**: `AudioWorld::play_oneshot(sound, pos, attenuation, volume)` queue API — fire-and-forget for callers without `&mut World` access (e.g. ECS systems). Drained in `audio_system` ahead of entity-based dispatch via the same spatial-track path. Bounded at 256 entries (FIFO drop-oldest). New `FootstepEmitter` component + `FootstepConfig` resource + `footstep_system` registered in `Stage::Update`: walks the player's XZ-plane GlobalTransform delta against `stride_threshold` (1.5 game-units default), fires one `play_oneshot` per crossing. Vertical (Y-axis) motion explicitly excluded — jumping / elevators don't count as steps. The fly-camera entity opts in via `world.insert(cam, FootstepEmitter::new())` in `scene.rs`, also doubling as the `AudioListener`. `--sounds-bsa <path>` CLI flag at startup decodes the canonical FNV `sound\fx\fst\dirt\walk\left\fst_dirt_walk_01.wav` into `FootstepConfig.default_sound`; absence is a clean no-op. **5 new synthetic footstep tests** pin: first-tick seeds last_position without firing, exact-threshold-stride fires once, single-large-jump-fires-once (catches "subtract-threshold-keep-remainder" regression that would machine-gun at high speeds), standing-still never fires, no-default-sound is silent no-op. Workspace 1677 / 1677 passing. **Phases 3.5b–6 pending:** FOOT records → per-material sound lookup (drop the dirt hardcode), REGN ambient soundscapes, MUSC + hardcoded music with crossfade, reverb zones (`kira::ReverbBuilder`) keyed off cell acoustics, raycast-occlusion attenuation. | —                  |
