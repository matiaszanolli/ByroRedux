...
**Last verified**: 2026-09-21 (session close, HEAD `f97775ca` — tests
**8386, 0 failing**, +196 vs Session 87's 8190; Rust `src/` LOC **~619 934**,
+19 882; total `.rs` LOC **~665 225**, +21 267; source files **1158** (1071
outside `tests/`), +32/+28; workspace members **34** (`crates/menuxml`, added
in Session 88); open issue dirs **4509**, +127. Session 89 rebuilt the audit
roster from yield data — 28 → 33 skills, one ownership map
(`.claude/commands/_audit-owners.md`) in place of three drifting copies, every
dimension delta-scoped, total skill text −39% — and then ran the new owners
(`/audit-exterior`, `/audit-gameplay`, `/audit-parsers`, `/audit-tooling`,
`/audit-papyrus`) straight away: 146 issues closed in the window, 94 through
`Fix #N` commits. The playable slice advanced on three fronts — the P2 combat
tail (Draugr attack/hit/death takes + combat sounds), P3's HUD vitals bars and
objective text with a live pixel-verified gate, and the P4 fixture frozen on
MS01 — see the playable-slice section. Renderer additions are opt-in AgX
tonemapping and an auto-exposure meter, plus a volumetric fire quality wave
(BFECC advection, spectral soot, multiple-scatter compensation) gated by a
game-data-independent `--combustion-lab` golden frame. The CI clippy gate
(`cargo clippy --workspace -- -D warnings`) was red under rustc 1.96 mid-window
and is green again at HEAD. The bench-of-record at `4c9a5b36` is now 449
commits stale; **R6a-stale-22** below carries the unmeasured per-frame
additions.
...
Ground-truth as of 2026-09-21 (session close, HEAD `f97775ca`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~619 934                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~665 225                 |
| Source files (`.rs`, excl. `target/`)   | 1158 total · 1071 outside `tests/` dirs (+32 / +28 since the 2026-09-18 close) |
