...
**Last verified**: 2026-06-03 (Session 46 — build failures resolved; R6a-stale-14 bench run completed at HEAD `1c26bc25`). Notable since Session 45: REND-#1447/1448 closed; ReSTIR-DI Phase 1 (`9abbe510`); EFFECT_LIT `lighting_influence` packing (`ea044b68`); 3 perf wins; 4 BGSM/BGEM fixes; `IsCollisionOnly` collider-BLAS gate (`1c26bc25`). See [HISTORY.md](HISTORY.md) Session 46 narrative.
...
- [ ] **R6a-stale-14-collider-partial** (filed 2026-06-03): Prospector fence recovered only 4.6% (11.65→11.12 ms) after `IsCollisionOnly` fix; entity count unchanged at 3516. The `IsCollisionOnly` tag is applied to the same entity that holds both `MeshHandle` (render) and `CollisionShape` (physics), so the fix reduces TLAS instance count for those entities but also removes their RT presence (no RT shadows/GI on FNV/FO4 architecture). BLAS is still built per mesh-handle regardless of `in_tlas`. Root cause of the original 2564→3507 entity growth (and fence 2.62→11.65 ms spike) between R6a-stale-12/13 is not yet confirmed to be solely the synthesized collider code. Needs: (a) instrument entity count broken down by component (render-only, render+collision, collision-only) to confirm how many entities changed; (b) consider NOT tagging render+collision entities with `IsCollisionOnly` — instead create a separate slim entity for the Rapier body that has no `MeshHandle`, so the render entity stays in TLAS cleanly.
