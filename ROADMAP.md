...
| Starfield         | BA2 v2/v3 LZ4 | **re-measurement pending** (last verified aggregate 99.99% · recover 100%, 2026-07-21 sweep — see caveat) | Per-archive as of the 2026-07-21 sweep: Meshes01 **100%** (31 058 NIFs), Meshes02 100% (7 552), MeshesPatch 99.98% (29 849; 6 truncated), LODMeshes **100%** (19 535), FaceMeshes **100%** (1 282). Intervening parser work (≥ #1510 BSShaderType155 tail, #1606 starfield_tail, #754 BSWeakReferenceNode, #722 cloth) cleared the Meshes01/LODMeshes truncation tails; the MeshesPatch tail (was 325 truncated, 1.09%, mis-attributed to closed #746/#747) is fixed by #2105. A residual 6/29 849 files with a distinct, unexplained cause remain truncated. **Caveat (#2201, 2026-07-25):** the Meshes02 figure above was measured 2026-04-28 (`dd203a00`) and was *not* re-verified after #2105 landed; #2201 reports it had in fact fallen to 6.10% (7 091/7 552 truncated) because #2105's 2-byte gap was gated on `SF_FORM_ID` (173) rather than the `SF_WEAK_REF_GAP` (175) it is now gated on. The gate fix is byte-level-verified and pinned by synthetic fixtures at both bsvers, but **no figure in this row has been re-measured against real archives since** — re-run `parse_rate_starfield_all_meshes -- --ignored` with `BYROREDUX_STARFIELD_DATA` set and replace this whole cell before citing any of it. |
...
| ~~R4~~  | ~~SWF/GFx strategic decision~~ | **Closed 2026-07-25 — use pinned Ruffle as the Flash VM with ByroRedux-owned Scaleform host profiles, preserving SWF mod compatibility.** Archive-grounded split: Skyrim `HUDMenu.swf` is AVM1/AS2; Fallout 4 is AVM2/AS3; FO3/FNV's XML menus remain a separate legacy-UI track. `byroredux-ui` exposes `ScaleformProfile::{SkyrimAvm1,Fallout4Avm2}` and a bidirectional `ScaleformHostBridge`: Ruffle `ExternalInterfaceProvider` queues typed ActionScript → engine calls, records unknown methods/callback registrations, and `Player::call_internal_interface` drives engine → ActionScript callbacks. M48 corrected Skyrim's real `GameDelegate` contract (method name is the ExternalInterface method; numeric request ID is argument 0; replies re-enter `respond`) and pinned 74 SkyUI call-site methods / 12 callback requests. The Fallout 4 slice recovered the distinct `BGSCodeObj` lifecycle, pinned 129 calls from reconstructed-vanilla interface source, and injects an eager ABC function table that routes namespaced calls through the same typed bridge without exposing Ruffle's private AVM2 internals. Ten default tests pin both AVMs, generated-ABC execution, nested values, catalog isolation, transport, and diagnostics; two ignored tests rewrite/load installed Skyrim/FO4 HUDs without shipping Bethesda assets. | M20                                             |
| M48     | UI integration               | **Host ABI slices advanced 2026-07-25:** Skyrim/SkyUI 74-method catalog + 12 request contracts and re-entrant response routing; Fallout 4 `BGSCodeObj` create/destroy metadata + 129 reconstructed-source methods + generated AVM2 forwarding adapter and object-aware dispatch. Corpus investigation corrected the old FO4 smoke claim: the standalone installed HUD parses and rewrites, but never reaches frame 1 because Ruffle preload stops at unresolved `ImportAssets`; the archive-backed navigator is therefore the next blocking slice, followed by installed-corpus lifecycle/inventory completion, method behavior and `_global.gfx` stubs, fonts/input/menu lifecycle, and Papyrus/ECS ↔ UI callbacks.                                                                                                                                                                                                 | R4, M20, M47.2                                  |
...
- [x] ~~**R4** SWF/GFx strategic decision~~ — **closed 2026-07-25:** pinned Ruffle VM + Skyrim AVM1 / Fallout 4 AVM2 Scaleform host profiles; bidirectional bridge, Skyrim's 74-method/12-request `GameDelegate` catalog, Fallout 4's 129-method `BGSCodeObj` catalog + injected ABC adapter, ten default tests, and two installed-corpus rewrite/load smokes shipped in `byroredux-ui`
