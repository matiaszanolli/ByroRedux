...
Ground-truth as of 2026-08-18 (Session 69 closeout, HEAD `348f4cd0`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~396 037                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~422 794                 |
| Source files (`.rs`, excl. `target/`)   | 933 total · 869 outside `tests/` dirs |
| Workspace members                       | 27 (24 crates + `byroredux` binary + 2 tools) |
| Tests                                   | **5303 passing, 0 failing, 143 ignored** (`cargo test --workspace --no-fail-fast`, 2026-08-18). The suite is green for the first time since 2026-08-11 — but by deletion, not by fix: the `fire_lights` derived-reach canary went out with the module it guarded (`2325c1de`). See [Known Issues](#known-issues) for what that leaves unverified. Plain `cargo test --workspace` now agrees at 5303, since nothing bails the run. |
| Open issue directories                  | 2963 (`.claude/issues/`)     |
