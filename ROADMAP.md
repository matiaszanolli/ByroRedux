...
| M61      | Wet-surface system                 | A "wet" state on actors/clothing/static geometry driven by water contact (rain volumes, submersion, splashes, footprints exiting a WATAL water plane) that darkens albedo and boosts specular/fresnel toward a thin-film reflective/refractive look, with a drying-over-time falloff. No scoping done yet — surfaced 2026-08-03 while wiring #2240 (WATR wave params); needs its own design doc before work starts, sized like M59's material-slot approach (`GpuMaterial` extension, no `DrawCommand`/shader-lockstep growth) rather than a new render pass. Depends on WATAL's water-plane contact geometry existing to detect "touched by water" in the first place. | M59, WATAL              |
