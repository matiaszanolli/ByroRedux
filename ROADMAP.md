...
**Last verified**: 2026-08-17 (Session 68 closeout — tests 5263, +84, **one
failing** (unchanged: the `fire_lights` canary); Rust `src/` LOC +13 349;
workspace members unchanged at 27; open issue dirs 2823 → 2963. **Thirteen**
audit reports landed in a single 08-16 sweep — the largest to date (physics,
regression, renderer, runtime, safety, save, scripting, Skyrim, SpeedTree,
Starfield, tech-debt, UI, plus the suite summary) — filing #3008 through #3098.
Unlike Session 67 this is a deliberate **refill**: only the UI and renderer
slices were worked down in-session, so the backlog is the sweep's output, not
its residue. The session's other arc gave the playable vertical slice a native
egui inventory/settings UI and, after `/audit-runtime` found all three playable
smoke gates unsound at once, gates that fail when the thing they gate is broken
(`23068af0`, plus the first CI job that runs them). **No milestone opened or
closed.** Bench-of-record is unchanged at **`34074b93` (2026-08-14)** and is now
**49 commits stale**, past its own 30-commit gate — tracked as R6a-stale-20 in
Known Issues; this session touched the main-pass fragment shader, ray-origin
reconstruction, texture-upload batching and the workspace hasher, so a re-run is
owed before the next matrix comparison. CI has also been red on
`main` for at least the last five commits for an unrelated environmental
reason — the runner cannot find `vulkan/vulkan.h` when building
`byroredux-fsr3-sys` — which is tracked in Known Issues below.)
...
Ground-truth as of 2026-08-17 (Session 68 closeout, HEAD `23068af0`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~392 385                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~418 903                 |
| Source files (`.rs`, excl. `target/`)   | 932 total · 868 outside `tests/` dirs |
| Workspace members                       | 27 (24 crates + `byroredux` binary + 2 tools) |
| Tests                                   | **5263 passing, 1 failing, 140 ignored** (`cargo test --workspace --no-fail-fast`, 2026-08-17). The failure is the `fire_lights` derived-reach canary — see [Known Issues](#known-issues). Without `--no-fail-fast` the run bails at that failure and reports only 1217. |
