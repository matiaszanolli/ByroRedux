...
| ~~R4~~  | ~~SWF/GFx strategic decision~~ | **Closed 2026-07-25 — use pinned Ruffle as the Flash VM with ByroRedux-owned Scaleform host profiles, preserving SWF mod compatibility.** Archive-grounded split: Skyrim `HUDMenu.swf` is AVM1/AS2; Fallout 4 is AVM2/AS3; FO3/FNV's XML menus remain a separate legacy-UI track. `byroredux-ui` exposes `ScaleformProfile::{SkyrimAvm1,Fallout4Avm2}` and a bidirectional `ScaleformHostBridge`: Ruffle `ExternalInterfaceProvider` queues typed ActionScript → engine calls, records unknown methods/callback registrations, and `Player::call_internal_interface` drives engine → ActionScript callbacks. The first M48 slice corrected Skyrim's real `GameDelegate` contract (method name is the ExternalInterface method; numeric request ID is argument 0; replies re-enter `respond`), and pinned 74 SkyUI call-site methods / 12 callback requests. Seven default tests pin both AVMs, nested values, catalog isolation, transport, and diagnostics; two ignored tests load installed Skyrim/FO4 HUDs without shipping Bethesda assets. Fallout 4's native AVM2 root-object surface remains a separate inventory rather than inheriting Skyrim's protocol. | M20                                             |
| M48     | UI integration               | **Phase 1 host inventory started 2026-07-25:** Skyrim/SkyUI 74-method catalog + 12 request contracts and re-entrant response routing shipped on R4's bridge. Next: implement method behavior and `_global.gfx` stubs; recover/install Fallout 4's vanilla AVM2 root objects; then archive-backed navigator/fonts, input routing and menu lifecycle, and Papyrus/ECS ↔ UI callbacks.                                                                                                                                                                                                 | R4, M20, M47.2                                  |
...
- [x] ~~**R4** SWF/GFx strategic decision~~ — **closed 2026-07-25:** pinned Ruffle VM + Skyrim AVM1 / Fallout 4 AVM2 Scaleform host profiles; bidirectional ExternalInterface bridge, Skyrim's 74-method/12-request catalog, seven default tests, and two installed-corpus smokes shipped in `byroredux-ui`
