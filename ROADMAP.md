...
**Last verified**: 2026-09-18 (session close, HEAD `5f3c7ec2` — tests
**8190, 0 failing**, +24 vs Session 86's 8166; Rust `src/` LOC **~600 052**
(correcting this session's own mid-session note, which had mistranscribed
~606 503), +2 854; total `.rs` LOC **~643 958**, +2 928; source files
**1126** (1043 outside `tests/`), +4/+4; workspace members unchanged at
**33**; open issue dirs **4382**, +1. Session 87 flipped M42's locomotion
procedures and sandbox seating on by default — walkers had been sliding on
their spawn idle behind seven per-procedure opt-in env gates even though the
package runtimes were complete (M42.10/M42.11) — and derived NPC walk speed
from each clip's own authored stride instead of a fixed 100 u/s, with
separate male/female/child walk clips per body class. The rest of the
session was a lighting and NIFAL material fix tail: Starfield `LIGH`
spotlights now key off the `DAT2+56` Light Type enum instead of a
misidentified flag bit, the falloff-exponent sentinel resolves per layout
generation, Oblivion's HNAM sunlight dimmer finally has a consumer, and three
material bugs (detail-role double-darkening, tint-role skin darkening on
alpha-less DDS, FO4 smooth-spec's blue-channel zeroing) are fixed. Four
issues (#3572, #4422–#4424) were genuinely fixed by commits that referenced
their number without a closing keyword; the close ritual caught and
reconciled all four with citing comments. The bench-of-record at `4c9a5b36`
is now 297 commits stale; **R6a-stale-22** below carries the unmeasured
per-frame additions.
...
Ground-truth as of 2026-09-18 (session close, HEAD `5f3c7ec2`). Every
figure in this table was measured at that HEAD, not carried forward.

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~600 052                      |
| Rust total lines (all `.rs`, excl. `target/`) | ~643 958                 |
| Source files (`.rs`, excl. `target/`)   | 1126 total · 1043 outside `tests/` dirs (+4 / +4 since the 2026-09-17 close) |
