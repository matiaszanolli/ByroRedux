...
**Last verified**: 2026-06-19 (Session 49 closeout — tests 2985, +70; non-test LOC +~4 818; bench-of-record now 281 commits stale, see staleness caveat below). Notable since Session 48: an **RT denoiser overhaul** (#1662) — multi-scatter energy compensation, the long-missing SVGF à-trous spatial pass (closes M37), ReSTIR-DI temporal direct-shadow reservoirs (M37.3 Phase 2), PCG-hash sampling replacing crawling IGN dither; an **FO4 import-consumer arc** making precombined-geometry cells render correctly (#1590 precombines-by-owning-plugin, #1594 BSConnectPoint attach graph, #1592 model-space-normal / alpha-test flags, BGSM precombine material flags, DXT1 punch-through alpha); plus a continued audit bug-bash across NIF collision / Havok (#1334 bhkPlaneShape, #1604 bhkBallSocketConstraintChain, #1652 motion_type enum, #1606 Starfield BSLighting tail), ESM (#1650 Oblivion ACBS, #1538 SCOL FNV/FO3), renderer / LOD, and tech-debt (TD9 commands.rs split). See [HISTORY.md](HISTORY.md) Session 49.
...
- [~] No save/load — playtest iterations require cold cell re-load (M45). **M45 library landed 2026-06-21 (`bd2d0de2`, branch `feat/m45-save-load`)**: full snapshot save/load (`crates/save`) — save + validate + atomic-write + `save.info` verify all wired; round-trip headlessly tested incl. entity-id preservation, StringPool, hierarchy, inventory/equipment, stable FormIdPair. **Remaining = M45.1**: live load-apply into a running Vulkan session (GPU/physics/camera re-instantiation). Until M45.1, `save.info` verifies a slot but doesn't apply it in-engine; the cold-reload workflow is unchanged.
...
Ground-truth as of 2026-06-19 (Session 49 closeout). Last `/session-close` verification was 2026-06-19 (Session 49).

| Metric                                  | Value                        |
|-----------------------------------------|------------------------------|
| Rust source lines (`src/` dirs)         | ~235 913 (Session 49 measure) |
| Rust total lines (all `.rs`, excl. `target/`) | ~249 751 (updated 2026-06-19)  |
| Source files (`.rs`, excl. `target/`)   | 585 total · 550 outside `tests/` dirs |
| Workspace members                       | 21 (19 crates + `byroredux` binary + `tools/byro-dbg`) |
| Tests (last reported by ROADMAP)        | **2985 passing** (Session 49 closeout, 2026-06-19). +70 vs Session 48. |
| Open issue directories                  | 1515 (`.claude/issues/`)     |
