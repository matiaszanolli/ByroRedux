...
- [ ] **REND-#1451** MEDIUM (filed 2026-06-03, Lonesome Road Ulysses Temple + prior cells): Bright near-zone ring around player — the `LIGHT_RANGE_EXTENSION = 2.5` multiplier makes a strong ceiling LIGH contribute 84% of max brightness at `d = authored_radius` (`ratio = 0.4, window = 0.84`). The doc-comment claim "10–30% at d=r" contradicts the formula and appears to describe a different (intended?) target. Bethesda's original `(1 − d/r)^e` formula hard-zeros at `d = r`; our window formula hard-zeros at `d = 2.5r`, creating a visible bright disc at `r < d < 2.5r`. Getting the formula to read 10–30% at `d = r` requires `LIGHT_RANGE_EXTENSION ≈ 1.1–1.2`, not 2.5. Fix requires: (1) verify Bethesda's actual formula from UESP/reverse-engineering data, (2) run a controlled bench comparing Prospector at extension 1.0 / 1.2 / 2.5 — the original 2.5 was tuned for FO4 densely-lit interiors, which may need a different value than FNV. Filed as #1451.
