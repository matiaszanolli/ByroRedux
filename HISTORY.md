...
## Session 90 — "The Hurt Locker": the rebuilt audit roster runs every owner, and the fix wave defuses its findings one by one while the bench matrix quietly reads slower  (2026-09-21 → 2026-09-23, `73aaed7b..5570c221`, 129 commits)

Session 89 rebuilt the audit skills and ran only the five new owners. This session ran everything else. Every owner ran over 2026-09-21/22, including all six per-game audits, and a four-leg volumetrics-deep suite ran on 09-23: 34 reports landed in `docs/audits/`. The window is mostly the fixes those reports drove, taken one issue per commit. A lot of them are bombs in the literal parser sense: MenuXml `<include>` expansion that grew exponentially, HKX string memory that grew quadratically, and a 17 KB clip that decoded to 610 MiB. Four feature commits landed beside the wave. The largest, `b9e961eeb`, carries far more than its docs-only message says. The same commit added a 75-run bench matrix at `cb44d99f6` that nobody folded, and it reads mostly slower than the record (see Net).

- **Audit fan-out and traceability.** 2026-09-21: character, concurrency, ECS, ESM, exterior, gameplay, NIF, NIFAL, parsers, performance, physics, renderer, safety, UI. 2026-09-22: Oblivion, FO3, FNV, Skyrim, FO4, Starfield, audio, legacy-compat, papyrus, regression, runtime, save, scripting, SpeedTree, tech-debt, tooling. 2026-09-23: renderer/safety/performance/concurrency over volumetrics. Issues filed through #4789. 171 closed in the window. 105 closed through in-window `Fix #N` commits, and all 66 others already had their archaeology: a pre-window fix commit, or a close comment naming the commit (bare siblings in `Fix #A, #B` and `Docs #…` sweeps). 186 issues open at close.
- **Renderer.** Per-game PBR ceilings (#4566). AgX clamps in log space, so highlights reach white again (#4578). Tonemap operator ids are generated into `shader_constants.glsl` (#4584). Exposure meter: the UBO is pinned in the block-size table (#4585), each slot adapts over its real interval (#4590), raw debug views are gated (#4591), and the average divides by the real sample count (#4597). Caustics: glass occlusion uses linearized depth (#4588) and the water visibility ray is masked to opaque layers (#4589). The FSR recovery blit reads the image's actual layout (#4592). The device-writes host flush now happens before `end_command_buffer` (#4602). Legacy FX cards rejoin the non-occluder mask (#4576). Debug-view fixes (#4582, #4583). `composite.frag.spv` was recompiled and pinned (#4577). The volumetrics-deep fixes: the froxel XY divisor fits `maxImageDimension3D` (#4781); the stale TLAS is withheld from compute ray queries after a failed build (#4779); froxel jitter is formed in fixed point (#4776); the lapsed combustion residual is dropped (#4775); and the fog params go in declared order (#4773). The end-of-frame scratch shrink moved out of `draw_frame` under a budget (#4767).
- **Parsers and ESM (the untrusted-input bombs).** HKX string memory is linear, and its sample cap is relative to the file's blocks (#4648/#4649/#4655). The MenuXml include bomb and `.fnt` panics are fixed on the `--hud` path (#4650–#4652). BA2 validates `name_table_offset` (#4670). BSA/BA2 count duplicate keys (#4671). VDF recursion is capped and `installdir` contained (#4673). BGSM/BGEM strings decode lossily (#4672) and report version-over-ceiling (#4664). EGM deltas are scaled int16 (#4653) and EGT is planar signed chars (#4668). ESM: TES4 light/medium master bits per game (#4639), Oblivion 8-byte LVLO rows and LVLD 0x80 (#4638), a recalibrated inflation ceiling for 82 Starfield SFTR records (#4640), FO4 terminal text (#4641), LTEX.GNAM as an array (#4642), FO76 HEDR no longer pinned (#4643), GRUP skips clamped to the parent (#4644), and the TRDA payload decoded (#4645). Six corpus-sweep gaps were closed across five titles (#4665).
- **Physics, character, gameplay.** The explosion recovery reads a dynamic-body index and carries a counter (#4682/#4683), with three uncovered edges pinned (#4687); FO3's ~1e12 BU first solve on restore is now its own root-cause issue (#4772). The query pipeline advances incrementally (#4685). NPC locomotion constants are derived from their sources (#4689). A dead combatant's corpse is no longer teleported to the origin (#4693). CHARAL: TPLT resolution is hoisted behind `ResolvedNpc` (#4457); FO76/Starfield claim no NPC stat model (#4453); the player carries `CharacterLevel`/`Background` (#4678) and PlayerOnly rows (#4674); the vital-pool roster lives on the profile (#4679); HUD bars resolve AVIF ids per load (#4675); PlayerRef `0x14` resolves through the shared resolver (#4694); and a charal doc-rot sweep landed (#4454–#4463). Water: the player sampler composes flows additively (#4691), Starfield concentrations normalize at the WATAL boundary (#4285), and a `WaterSurfaceMesh` component is added (`17c01a4e`).
- **NIFAL.** Material sinks, glass provenance split, and once-only script persistence (#4282/#4283/#4334). Flipbook frames resolve through the role table (#4426). The BGSM envmap fill is gated (#4428). Starfield single-channel kinds are parked by name (#4429). A dead NIF texture path yields to the sidecar chain (#4636). Completeness guards now match the code, not the prose (#4411).
- **Features.** `b9e961eeb` shipped five features: (1) Starfield's metric scene data is lifted to Bethesda units once, at both parser boundaries (`nif/import/units.rs`, `esm/records/spatial_units.rs`); (2) immutable geometry is shared by exact content (`mesh/geometry_sharing.rs`); (3) a static BLAS working set; (4) direct interior loads spawn on the authored `COCMarkerHeading` (`cell_loader/interior_spawn.rs`); (5) new Cornell RT-oracle rungs and `scripts/lighting-game-matrix.py`. `ff1b48d7` tracks mesh-upload provenance, with a shared-mesh oracle rung. `5570c221` adds spawn-time skin-seam blending for runtime-assembled Oblivion/FO3/FNV NPCs (`npc_spawn/seam_blend.rs`), head-part textures on RACE, and dismemberment caps identified by body-part data.
- **ECS, CI, tech debt.** The #4572 hierarchy walks are guarded and paired with `HierarchyTraversalGuard` (#4769). The access-declaration guard sees mode and whole names (#4573), and six exclusive systems got their missing Access rows (#4574). CI's validation lane reaches Vulkan (#4596) and clippy runs even when tests are red (#4595). `storage_util.rs` became a module (#4768), and an orphaned renderer test was restored (#4766).
- **Left red at close.** The clippy `-D warnings` gate is red again from two fixes in this window (`a323138d` hkx, `20faaf89` menuxml), on top of the still-open #4765. #4700 was verified against HEAD: Skyrim's production route never inserts `DraugrCombatAnim`, so Session 89's P2 combat tail does not fire on the target game. #4752 (an unauthenticated, default-on debug port with file read/write) is open at HIGH.

Net: tests +135 (8386 → 8521, 0 failing; 233 ignored); Rust LOC +10 637 src / +12 103 total (630 571 / 677 328); source files +15 total / +13 outside `tests/` (1158 → 1173 / 1071 → 1084); workspace members 34 (unchanged); open issue dirs +224 (4509 → 4733). Bench: the uncontrolled `cb44d99f6` matrix has 19 of 25 medians slower than `4c9a5b36` (Cornell −19% to −26% on every config), but every scene fingerprint differs from the record and no same-machine control was run. The record is now 579 commits stale (R6a-stale-22).

---

## Session 89 — "Moneyball": the audit roster is rebuilt from yield data, and the audits it drives feed a 94-issue fix wave  (2026-09-19 → 2026-09-21, `959ff551..f97775ca`, 143 commits)

Every audit run loads its whole skill file, and the skills had rotted in both directions: 1.09 MB (~272k tokens) of text, `_audit-common.md` alone 56 KB loaded on every run, three hand-kept copies of "who owns what" that drifted independently, and no owner at all for the code that had dominated the last five sessions (ground cover, sky, water translation, the gameplay layer, the non-NIF/ESM readers, the SDK and debug tooling). Historical yield data (some 670 reports under `docs/audits/`) showed no skill was dead — the lowest recent average was ~1.6 findings a report — so the work was to cut narrative, add owners, and make every run delta-scoped rather than to delete audits. The rebuilt skills were then run immediately, and the findings they produced are most of this window's fix commits.

- **Audit-skill overhaul.** Audit skills 28 → 33 and total skill text 1 087 178 → 659 835 B (−39%); `_audit-common.md` 55 914 → 9 386 B (`a43b19603`, `41aed20e`). Five new owners: `audit-exterior` (EXAL/SKYAL/WATAL translation + ground cover + sky bake), `audit-gameplay`, `audit-parsers` (bsa/bgsm/sfmaterial/hkx/facegen/menuxml/game-detect), `audit-tooling` (SDK, debug server/protocol, launcher), and `audit-papyrus` split out of a 119 KB `audit-scripting` (now 36 KB + 16 KB). Renderer dimensions 23 → 12. Every dimension now carries `Paths:` / `First step:` / `Guard:` lines, so a run scopes to the dimensions whose paths changed since the last report (the shared "delta-first" protocol) and cites the guard test that already enforces a mechanical invariant instead of re-deriving it. One ownership map (`_audit-owners.md`, 136 path rows) replaces the three shadow copies; `_audit-route.sh` routes a diff through it, `audit-suite` derives its sets from it (`--area` / `--changed`, replacing ~27 hand-kept presets), `audit-regression` selects issues by churn on the files changed since its last sweep instead of "the last 50 closed", and `_audit-validate.sh` now fails on an unowned crate, a dead owner row, or an owner with no skill (replacing the hand-maintained "Crate count: N" literal). The slim-down dropped a vocabulary pin that a source-scan test audits, caught and restored the next day (`33d987ba`) — the guards-audit-the-docs design working as intended. Corrections found on the way: the renderer skill's teardown, TAA-order, bloom and hash-function premises were all stale, the AI systems are default-on (only `BYRO_NO_AI_LOCOMOTION` remains), and a `bench-stats` command it cited never existed.
- **Audit waves the new skills drove.** Exterior (#4480–#4509), Papyrus/pex (#4471–#4479), renderer (#4510–#4542), NIFAL animation/material (#4396–#4407, #4546–#4566), tech-debt (#4567–#4570), character (#4447–#4463) and the 2026-09-11 backlog (#4145–#4207); 146 issues closed in the window, 94 of them through `Fix #N` commits and every one of the remaining 52 carries a close comment naming its commit. A further concurrency / ECS / NIF / performance / renderer / safety sweep dated 2026-09-21 was still unpublished at close.
- **Gameplay slice — P2 tail, P3, P4 fixture.** P3's container browser, take-all, minimal theft rules and visible player equipment (`479163836`); native HUD vitals bars and objective text with a live pixel-verified gate, `p3-hud.sh` (`db39fe00`, `31357d15`); live re-equip reconcile via `equipment_appearance_system` (`24ccc8f7`); the P2 combat tail — Draugr attack/hit/death takes and combat sounds through a new `systems::combat_anim` (`ec3a18d2`, fixture `c5f45613`); the P4 fixture frozen on MS01 `0x00018B4B`. Player `ActorValues` are stamped from the base NPC record (`eb378430`, #4458), loot transfer got its missing production caller (#4464), and `FORMAT_MAJOR` moved to 25 with the required `picked_up` field (`340799d6`, #4465). #4466 records the prevention gap behind it: the bin crate shipped uncompilable across two commits because the default toolchain could not build it and nothing caught it.
- **HUD / UI.** FO3/FNV per-game profiles on the MenuXml HUD driver (`d574d9bd`) and a Fallout 4 Scaleform HUD behind `--hud` with smoke tests (`62fc0bf2`); the frames-in-flight tripwire now covers the HUD overlay rotation (#4516).
- **Renderer correctness.** The water push-constant block now fits its declared 16 B range (#4510/#4534); TAA's raw-view jitter is gated (#4513) with a #3572 doc-rot sweep (#4524/#4530); DDS uploads hardened against untrusted input, a staging-pool capacity fix and real extent validation (#4511/#4512/#4515); every material byte view goes through one bounded `byte_view` (#4445/#4521) and `GpuMaterial` is interned once (#4442); detail/tint GLSL arithmetic pinned (#4520). Water: reflection intensity was four serial authored multipliers (#4543), directional scroll is confined to the flow axis (#4544), caustic deposits are gated on visibility (#4545, `0418ac76`). SSAO's kernel operating point made contact occlusion visible (`09d9bc6f`); model-space normal maps decode through the source-to-renderer basis flip (#3922); authored Environment Map glass classifies as glass again (#4392).
- **Renderer features.** An AgX tonemap operator and an auto-exposure meter pass with live console controls, both opt-in (`--tonemap agx`, `--auto-exposure`; `c5663fe3`, `d5438241`). The volumetric fire quality wave: single-pass BFECC advection recovers the detail semi-Lagrangian transport smears (`10798823`), spectral soot and a phase lobe on froxel local lights (`1e1bca8f`), multiple-scatter compensation (`33d253b7`), and a game-data-independent `--combustion-lab` golden frame, which required generalising the harness from one cube-demo case to named cases (`7cdc0c7f`). Facing-ratio and ReSTIR-light debug modes (`f97775ca`).
- **NIF / NIFAL / animation.** One rotation sanitizer and NaN-inclusive static-pose gates (#4396/#4397, `d53be91b`), plus a finite gate on the static `NiTransform` boundary (#4549); overflowing B-spline rotation samples are skipped, not replaced with identity (#4406); particle spawn cones use the emitter's composed orientation (#4398), birth rates resolve behind opaque sibling controllers (#4467), an authored-zero emitter budget keeps the preset (#4550); GPU morph slots are created where the read gate can reach them (#4399); MSWP re-merge, clamp-mode latch and BGSM winning-slot authority fixes (#4400–#4402); `_msn` slots declare model-space normals at the translate boundary (#4548); the walk_anim/cinematic lock-order cycle is broken (#4546); `BhkPlaneShape` is stated to produce no collider and counted in the spawn census (#4407).
- **Exterior and plugin.** HNAM sun dimmers carry through cross-fade and promotion (#4481/#4485); `env.health` gates `WaterMaterial`, weather fog and every `FogMedium` field (#4483); WRLD/VTXT wire floats are finite-gated (#4484/#4487/#4495); the packed LOD word's 22-bit serial mask fixes precision loss after ~19.4 h uptime (#4498); FO76's baked object-LOD family joins the `BakedBto` scheme (#4488); the exterior smoke harnesses were hardened and wired into playable-smoke (#4489/#4491/#4492/#4506/#4508). CELL `XEZN` is parsed and tested (#4173, `encounter_zone_form` fixtures #4480), and the loose-strings probe is case-insensitive (#4176/#4178).
- **Papyrus / pex.** Construct continuation no longer skips newlines (#4472), a case-insensitive auto-state collision marks exactly one auto state (#4473), lexer diagnostics cover `0x`, exponent shapes and CR-only files (#4479), the `MAX_OPCODE` transmute invariant is pinned at compile time (#4475), and the `is`-to-Cast lowering is bookkept as the third Champollion departure (#4477).
- **Tech debt and infrastructure.** The clippy `-D warnings` gate was red under rustc 1.96 (an undocumented `unsafe` in `texture.rs`, a collapsible-if, menuxml/plugin/bin-crate buckets) and is cleared (`a62d06d6`, `82bc3493`, `92a633ce`); ground-cover telemetry moved into `groundcover_stats.rs` (#4568) and character spawn planning out of `scene.rs` (#4569). `ratatui` 0.28 → 0.30 and `crossterm` 0.29 clear an `lru` Stacked-Borrows advisory (`052891f2`). Engine-facing identifiers drop the Havok brand — `AnimationTarget`, `IdleClipCatalog`, the packed-collision proxy trio (`4dca737e`).

Net: tests +196 (8190 → 8386, 0 failing); Rust LOC +19 882 src / +21 267 total (619 934 / 665 225); source files +32 total / +28 outside `tests/` (1126 → 1158 / 1043 → 1071); workspace members 33 → 34 (`crates/menuxml`, landed in Session 88, first counted here); open issue dirs +127 (4382 → 4509); audit skills 28 → 33 and −39% text; bench-of-record `4c9a5b36` now 449 commits stale (was 297; R6a-stale-22).
...
- **Ambient locomotion (M42.10/M42.11).** Locomotion procedures and sandbox seating now walk by default with a kill-switch, instead of requiring seven separate opt-in env gates (`913fd39d`). NPC walk speed now derives from the authored clip's own stride instead of a fixed 100 u/s that fought whatever gait the clip played, and male/female/child KF-game humanoids each get their own body-class walk clip instead of sharing the male `mtforward.kf` (`774560dc`). `5f3c7ec2` reconciled ROADMAP's Tier table, `docs/engine/npc-spawn-ai-packages.md`'s per-procedure gate descriptions, and `docs/feature-matrix.md`'s AI/behavior entry with the now-live configuration.
- **Lighting.** Starfield `LIGH` spotlights now key off the dedicated `DAT2+56` Light Type enum instead of the `0x200` flag bit, which the now-published xEdit legend confirms is "Focus Spotlight Beam", not a spot-shape bit (`6c45c5bd`). The falloff-exponent sentinel now resolves per layout generation (1.0 for Skyrim, 2.0 for pre-Skyrim) instead of one shared 1.0 reaching every game and flattening FO3/FNV/Oblivion's authored quadratic falloff (`6b4e6252`). Oblivion's HNAM sunlight dimmer, parsed since #537 but never consumed, now translates onto `WeatherDataRes` so exteriors stop rendering the raw undimmed NAM0 sun palette (`df59c636`). The unflagged-ESM-light visibility policy (`ARCHITECTURE|DYNAMIC_ACTOR` vs NIF-direct full-omni) moved from an inline bool into a named, doc-tabled `VisibilityMask::for_legacy_local_light` decision table (`c5a09a78`). The light-overflow warn now cites the real drop policy — `collect_lights` has sorted by descending `gi_priority_score` since #1800, so `MAX_LIGHTS` drops the lowest-scoring tail, not "storage-iteration order" as the stale wording claimed (`8ad6b0de`). The oversized-reach canary on derived combustion lights, deleted wholesale with the `fire_lights` refactor, is back to guard the transported-field reduction (`bfc4f4e6`).
- **NIF / material boundary.** `MaterialInfo::texture_set` no longer double-counts the detail role's sRGB-decoded sample against a linear-space neutral — a producer-declared `detail_neutral` now flows NIF import → translate → `GpuMaterial` → GLSL, `GpuMaterial` grows 428 → 432 B (#4422). The tint role only fires for an alpha-bearing tint DDS now; vanilla `_sk.dds` tints are alpha-less DXT1, so the weighted mix was collapsing Skyrim skin to 1/5–1/200 of its diffuse (#4423). FO4 texture-set slot 7's `_s.dds` now routes to a new `SmoothSpec` role instead of feeding two-channel BC5 into the specular-colour multiply, which was zeroing every direct highlight's blue channel (#4424). A non-canonical 92+-shape `XCLL` (cross-game plugin injection, corrupt authoring) now only takes the ambient-cube-plus-specular/Fresnel arm on games whose canonical size sets actually contain that shape (`efe15eca`). `NiAmbientLight` scoping stays parked — a four-game census (Oblivion+SI, FNV, Skyrim SE, FO4) found zero vanilla instances with non-black color or affected-node scoping (`10d071cd`).
- **Investigation.** #3305 (shadowless creature actors) narrowed to a skinned-BLAS first-sight failure path: an entity whose per-entity BLAS never built on its first frame emits no TLAS instance and stays invisible to RT while raster still draws it, and the refit path defers retry rather than catching a failed first build (`b73d251c`). Still open — needs a RenderDoc capture to confirm, per this project's own no-speculative-render-fixes convention.
- **TAA/FSR parity.** TAA now resolves on the same post-composite scene tap FSR consumes instead of the raw pre-composite direct-HDR attachment, so sky, denoised indirect, volumetrics, caustics and bloom are no longer excluded from temporal resolve on the `--upscaler taa` fallback path (#3572).
- **Issue-citation reconciliation.** Four issues (#3572, #4422, #4423, #4424) were genuinely fixed by this session's commits but referenced their issue number in the commit subject (`(#N)`) rather than a closing keyword; all four closed with comments citing the landing commit as part of this session-close. #4433 (a comment naming a future NIFAL fix) is a legitimate forward-looking reference, left open.

Net: tests +24 (8166 → 8190, 0 failing); Rust LOC +2 854 src / +2 928 total (600 052 / 643 958); source files +4 (1122 → 1126 total, 1039 → 1043 outside `tests/`); open issue dirs +1 (4381 → 4382, net of the four closes this session plus new filings); workspace members unchanged at 33; bench-of-record (`4c9a5b36`) now 297 commits stale (was 277 at Session 86 close, tracked as R6a-stale-22).

---

## Session 86 — "Extraction": VulkanContext splits into sub-structs, inventory and loot persist across reloads, and a same-session regression is caught and fixed  (2026-09-15 → 2026-09-17, `b0068f95..ac1d44f5`, 87 commits)

Session 85 closed a 45-issue audit-fix tail but left `VulkanContext` as a large flat struct after months of feature growth, and the RPG-systems side — inventory, loot, consumables, player feedback — still had no persistence across cell reloads or respawns. This session split `VulkanContext` into logically-scoped sub-structs across five commits, then spent the back half building out gameplay systems: persistent reference state for looted NPCs and containers, leveled loot expansion for Fallout 76, timed restorative consumable effects, and a transient `PlayerNotifications` HUD channel. A long tail of renderer correctness fixes, ground-cover/water/audit doc corrections, and two new compat audit reports (runtime telemetry, Starfield) closed out the window. The close ritual itself caught a live regression: two structural guard tests were red at HEAD, both traced to the same commit and fixed before this entry was written.

- **Renderer: `VulkanContext` decomposition.** Five commits regrouped the context's fields into cohesive sub-structs — reprojection history (`f5267e1b`), per-frame scratch buffers (`59307d4a`), swapchain-lifetime render targets (`10ee872b`), the post-processing chain (`27a163b1`), and overlay/capture state (`bb9493de`) — plus pulled the context's telemetry accessors and pure data types out on their own (`9951c14e`), extracted four per-frame hooks out of `render_one_frame` (`5b4eae2c`), and split `setup_scene` into its seven phases (`c03934bf`). A follow-up fixed the formatting drift the splits introduced (`c1502ef9`) and corrected a stale `ScratchBuffers` handle claim in docs (`112f7d4b`). The golden-frame baseline was regenerated and its staleness guard flipped back on (`b23c92e2`, `970e8b8b`).
- **Gameplay: persistence and inventory.** `PersistentReferenceStates` now captures inventories, equipment and actor state before cell teardown and restores them on respawn, with NPC placement identity stamping so only the looted instance of a shared base actor restores (`d8255b2e`). `NpcLootAppearance`/`RestorePart` defer corpse appearance restoration and hide worn gear until the NPC is fully looted (`0c8ce1f5`). `expand_leveled_loot` resolves container loot into `(form_id, count)` stacks for Fallout 76's LVLO/LVLV/LVIV leveled-list variants, and Oblivion `CLOT` records now enter the inventory (`3cbf3674`).
- **Gameplay: consumables and feedback.** `ConsumableEffect`/`parse_effects` decode ALCH effect chains for Skyrim and FO3/NV, with `MgefRecord.instant_restoration_av` for game-local actor values (`4c005332`, `caea8e62`). `TimedRestorations` + `restoration_system` apply restorative effects over elapsed simulation time, and `HardcoreMode`/`IsHardcore` land for New Vegas (`2e2f40b2`). `PlayerNotifications` replaces the old debug-only `SaveLoadNotifications` channel with an 8-message queue rendered in the HUD without debug panels (`ae25ade3`).
- **Gameplay: spawn, load screens, scripting attach.** Character spawn now prioritizes explicit camera positions and runs capsule overlap checks so NPCs don't spawn inside doors or solid objects (`c03d6d6f`). LSCR loading-screen records parse through a new `LoadScreenRecord`, replacing the `MinimalEsmRecord` stand-in (`40bd9c19`). Scripts now attach on both logical-actor-stub paths (`c60a7b6b`), saves are gated on `SetInChargen` (`2f36373c`), and every shader-consumed `Material` scalar is `mat.set`-able from the debug CLI (`d38d3a12`).
- **NIF / physics.** Ball-and-socket, stiff-spring and chain constraint `CInfo`s now decode (`5172f5e2`). NIF loading was simplified and `spawn_embedded_animation_player` now owns the animation-player hierarchy explicitly; ragdoll solver iterations were increased for stability, and a new `p5-save-restart.sh` smoke test verifies character pose survives a process restart (`00a8a1ee`). FaceGen tint override is now keyed on `FaceTint` heads (#4421), BGEM effect data merges into the NIF payload (#4425), and flipbook frame textures release on cell unload (#4427). Fallout 4 previs headers now recover their owning cell's grid (`00b75ed0`), and `TPLT` template-stat resolution walks the whole inheritance chain instead of stopping partway (#4086).
- **Renderer correctness and perf.** BLAS budget now recomputes after the upscaler recreates (`2798905b`), static-BLAS recovery moved onto its own path and reports batches at their real scope (`fa428397`, `ac8844146`), the pipeline cache persists across blend-variant compiles (`26cea0bb`), instance SSBOs allocate at a working capacity and grow instead of over-provisioning (`5e778119`), bone-palette dispatch narrows to changed slots only (`eb7c8204`), and `GpuMaterial` now hashes its bytes instead of walking fields for change detection (`479ce526`). Ground cover picked up a second LOD-tier bit (`bae4ee96`), pipelines rebuild on a surface-format change (#4307), counter seeds sequence after the zero fill (#4293), the boundary-material guard walks the whole spawner tree (#4302), and blades jitter with a debug-view albedo output (`6431838b`). Flipped normal maps gate on their own alpha channel (`c9d4f007`), each `WTHR` cloud layer gets the mip its own frequency calls for (#4230), cloud height fraction derives from radial altitude (`b20a51cd`), and two new debug render views (`water_term`, `water_normal`) landed (`7c3f182e`).
- **UI/Scaleform.** The Skyrim/AVM1 host `GameDelegate` catalog was measured against and regenerated from the shipped AVM1 corpus sweep (`ba13f8c4`, `900c5d23`).
- **Audits, docs, tech debt.** New compat audit reports for runtime telemetry (`fb4db015`) and Starfield (`8227c852`) were published; the runtime-audit baseline was re-pinned to renderer-static and re-baselined (#4417–#4420). A stale-status sweep corrected docs and code comments across 11 issues (`4127533e`), and audit-skill anchors were repointed at the split NIF-shader module (#4446) and fenced layout paths (`db70595e`). Tech-debt work split the ESM records barrel and NIF shader family files (`eaa94b49`), the scripting `apply_effect` 29-arm match into per-family helpers (`ad62a5ce`), derived the plugin dispatch coverage list with an explicit-remap requirement (`262a49d3`), gave the SDK's `StorageUtil` list verbs their own functions and scalar declarations a table (`d43f7328`), shared renderer shader helpers (`242cbb45`, `13e00a24`), deleted dead code and wired the texture deferred-destroy backlog (`e3131f5e`), and cleared clippy `-D warnings` under rustc 1.96 (`80080256`). Water fixtures were frozen for FO3/Oblivion/FO4 and re-frozen for Skyrim (`6af41ce5`), smoke-test coverage/fixtures were enhanced for FO3/FO4 (`613ad312`), and `LTEX.GNAM` grass mapping plus water shader fixes landed (`7996edf6`).
- **Session-close regression catch.** `/session-close`'s ground-truth pass found `cargo test --workspace` red at HEAD for the first time since Session 77's under-reporting bug — two structural guards, both real, both traced to `3ce970a5a`: `player_controller_system` had picked up an undeclared `GlobalTransform` write (`fly_camera_system` now publishes its pose same-frame for interaction/picking/audio/debug-camera consumers, but the write was never added to the system's scheduler-access declaration, which would make the parallel-scheduler's conflict analyzer blind to it), and the save-shape fingerprint guard had gone red because `VisibilityMask::for_legacy_projection`'s logic changed inside a tuple-struct `impl` block the guard's brace-matching sweeps — a scanner false-positive on a type whose actual on-disk shape (a plain `u8` newtype) never moved. Both fixed and verified in `ac1d44f5` (full workspace suite: 8166 passing, 0 failing).

Net: tests +207 (7959 → 8166, 0 failing after the same-session regression catch); Rust LOC +17 070 src / +17 244 total; source files +29 (1093 → 1122 total, 1010 → 1039 outside `tests/`); open issue dirs +30 (4351 → 4381); workspace members unchanged at 33; bench-of-record (`4c9a5b36`) now 277 commits stale (was 189 at Session 85 close, tracked as R6a-stale-22).
...
- **WATAL — water placement and currents.** The moiré had three causes, each fixed on its own. Particle billboards' `emitter ^ index` ids collided with a water plane's entity id, so the re-emit index is now keyed by entity and mesh (`b6c9d712`). The shoreline mask read LAND rows north-first, which mirrored every exterior water mask north/south (`09432157`). And water draws used a draw-list position as their SSBO slot, not the compacted `instance_map` slot (`00d4ef5d`). Skyrim mesh water now takes its volume from the authored `bhkSimpleShapePhantom`, so a scaled `water1024` no longer reaches 4 344 units deep where 1 536 were authored (`1cc47a16`). A census showed `XWCU` is an `XWCN`-counted array: REFR entry 0 is the current, and CELL-level `XWCU` carries no velocity. The CELL `water_velocity` path was removed and REFR currents applied (`052c0a39`, `ec9ed852`, `1b861bba`). Data textures now resolve as linear (`60cef1c6`).
- **EXAL ground cover.** Phase 3's always-on tier-3 terrain detail layer landed in `triangle.frag` (#4056, `fd0cd577`). It is keyed on intrinsic ground density rather than draw distance, and `GpuTerrainTile` grew 144 → 160 B for it. The same commit brought LOD tiers 1–2 with a bounded FSR reactive mask that replaces full-strength writes (#4297), blade shaping literals named as uncited constants (#4378), and a deterministic reference-capture harness (`scripts/renderer-eval-groundcover.sh`, with follow-ups through `071dfa30`). Blades had received no sun at all, because directional lights were scaled by the point-light falloff exponent (#4291). The blade buffer is now sized from a Rust mirror, and over-cap chunks are cut nearest-first (#4335, #4338). The tier 1–3 visual acceptance review is still pending.
- **SKYAL.** Weather-driven sky illumination (`dc595f9c`): GGX-prefiltered cube mips for rough reflections, nine-coefficient SH irradiance from the baked sky, weather-driven cloud morphology and a palette-preserving Rayleigh/Mie approximation. Every exterior sky-miss site now goes through one gated cube read (#4292).
- **Renderer audit tail.** Draw sort keys: the glass pipeline axis and the `no_sorter` slot (#4191, #4192). Instance-map scratch reuse (#4193). The key-cached sort proposal was re-measured and rejected again (#4194). The FO4 draw split was re-captured in a single run (#4195). BLAS admission is now decided per mesh (#4196), and texture staging is bounded to 128 MiB sub-batches (#4197). MorphSlot LRU is now stamped from entity liveness (#4294).
- **NIF / material boundary.** Emitter planar angle (#4240); `.bto` LOD materials through the full translation boundary (#4245); texture-set precedence (#4235); flip-controller texture roles (#3901); MSWP swaps merged onto the pre-merge material (#4290); window/eye glass signal (#4391); Skyrim effect-shader `env_map_scale` (#4393); Starfield bound-sphere units (#4394); NIF light direction (#4395). FO4 precombines now draw all three LOD bands, which are disjoint sub-meshes, instead of picking one and dropping 12.45% of baked triangles (#4234, `66fff31b`).
- **Skyrim LE (2011).** Alternate game releases with distinct archive names get their own profiles (`66fff31b`). 32-bit Havok packfiles now parse, and a `SkyrimLE` game variant brings per-block baselines (`fb8173fe`). Packfiles outside Havok 2010's MSVC layout are refused (#4332).
- **Scripting / MQ101.** `.pex` string copies are bounded and script scope is keyed on the empty state (#4317, #4319). Pratt-loop depth is bounded and expressions end at a newline (#4320, #4321). Fragment primitives and the lock ledger were corrected (#4318, #4323, #4328–#4330), along with same-frame hits and lock order (#4324, #4325), `SetInChargen` parameter names (#4322), the cinematic-router predicate (#4333), and `Disable()` persistence across reloads (#4326, #4327).
- **Docs and tooling.** Split-module doc paths re-pointed (#4351–#4354). Disproved SpeedTree claims swept (#4118, #4120). The production-LOC counter is now string-aware (#4336). `GpuTerrainTile` docs corrected (#4337, `de9ff4da`). A Built-in Fixes design doc was added (`1b01ad64`).

Net: tests 7864 → 7959 (+95); Rust LOC outside `tests/` ~570 874 → ~580 128 (+9 254); total `.rs` LOC ~614 100 → ~623 786 (+9 686); source files 1082 → 1093 (+11; 999 → 1010 outside `tests/`); open issue dirs 4228 → 4351 (+123). No bench run: the bench-of-record at `4c9a5b36` is now 189 commits stale. This range adds unmeasured per-frame GPU work: the sky-cube prefilter and SH irradiance passes, tier-3 terrain detail sampling in `triangle.frag`, and sun-lit blades.
...
- **SKYAL — sky abstraction layer** (`docs/engine/skyal.md`, `2e31d4dc`). The analytic sky moved into a shared `include/sky.glsl`, verified to match the old output within the noise floor (`b7abdaa5`). Bloom gained a Karis/Jimenez soft-knee bright-pass: a flat 1.71× gain had been flattening the authored zenith gradient and washing the sky toward white (`62a09fd9`). A per-frame sky cubemap bake now feeds `traceReflection` misses and `pathEnvironmentRadiance` (`b54b86b7`, `6db9eac2`), with its own GPU timer bracket `gpu_sky_cube` (`74df367f`). Volumetric clouds are marched into the cube and the visible sky (`c379898f`, `564d0d2f`). Lighting follows Hillaire 2016: two-lobe HG phase, exact hemispheric sky ambient, and octave multiple scattering. Extinction is set to 0.12 m⁻¹, the measured cumulus value; the old value was ~75× too thin. Three diagnoses followed, each proven rather than guessed. Grainy edges were view-step aliasing, fixed with Schneider & Vos's adaptive march (`9ac8a929`). Flat undersides were self-shadow step aliasing, fixed with a geometric light march (`5d5d6ddd`). Striation came from fixed shadow sample distances, fixed with jitter (`fc4600b3`). A wind-axis misread was also fixed (`5317be29`). The unsourced constants (octave falloffs from Skybolt, adopted with the project owner's sign-off) are listed as such in `66e86ce1`.
- **EXAL ground cover visual pass.** Blades glowed because composite added the terrain's GI × albedo on top of their own ambient; blades now write albedo and inherit the ground's GI (`4d54c73a`). A downward TLAS trace per candidate keeps grass off placed geometry such as roads (`c775745b`). Four-blade tufts follow Outerra's density (`fcdf7083`). Blade normals fold per Jahrmann & Wimmer 2017 (`e5f75465`), though no visible difference could be shown at bench distance. Species are now drawn by climate weight through a 256-entry table (`1a7a22cf`). A 184-record census found `GRAS` models are card clumps, and about half aren't grass at all. So blades became the engine's own climate-selected sward, and `GRAS` is reserved for the authored-model tier, §12.12 (`f8a900b3`). A 4× candidate budget raises Skyrim tundra from ~9 to ~36 blades/m² (`673b2145`).
- **MQ101 gameplay.** Chargen-gate effects: `SetInChargen`, `ShowRaceMenu` (auto-accept) and `RequestSave`. Save format moves to v23; fragment coverage 45.9% → 48.4% (`d0dac91b`). A minimal NPC combat AI adds `Faction.SetEnemy`/`Actor.StartCombat` and `npc_combat_ai_system` for Alduin's attack and the keep escape, sharing the player's `HitEvent` damage pipeline (48.4% → 50.0%, `f61ea044`). Still open: `AddRaceSpells` (needs a RACE `SPLO` decoder) and an interactive race menu.
- **Audit-fix tail (43 issues).**
  - Renderer sync: static-batch scratch barrier (#4177), BLAS writes published to the TLAS build (#4179), ground-cover readback barrier (#4181), `pending_uploads` purge on eviction (#4050). `StagingPool` now releases its allocator `Arc` on destroy (#4187); the demo scene tears down cleanly.
  - Save: scripted lock changes persist in a FormID-keyed ledger (#4136), quicksave refused mid cell transition (#4138), session-local continuations dropped (#4139), plus #4140–#4144.
  - ESM: FO4+ `_en` string tables and cp1252 decoding (#4168, #4170), nested-GRUP `group_type` (#4169), terminal text (#4171), TRDA dialogue and localization (#4068, #4072, #4073, #4084).
  - NIF: `NiPixelData` mipmap bounds (#4157, #4165), B-spline rotation normalize guard (#4166), Animation/Particles completeness guards (#4167).
  - Ragdoll access declarations and census fixes (#3964–#3967), renderer doc-rot (#4023–#4049), hazard-window scan (#4021), `capture.sh` readiness gate (#4123), clippy and test sweep (#4130–#4133).

Net: tests 7754 → 7864 (+110); Rust LOC outside `tests/` ~563 207 → ~570 874 (+7 667); total `.rs` LOC ~606 180 → ~614 100 (+7 920); source files 1076 → 1082 (+6); open issue dirs 4219 → 4228 (+9). Bench-of-record `4c9a5b36` is now 136 commits stale; R6a-stale-22 is folded. This range's sky-cube bake, per-pixel cloud march and ground-cover trace are new per-frame GPU work that no bench-of-record run has measured.
...
first track grew compiled Papyrus fragment execution from the initial quest
effects into scene phases, aliases, global/conditional/reference effects and
package/cinematic control. The second made save/load failure visible and
non-destructive. The closeout then restored CI's native Vulkan dependency and
drained two live ECS lock-order cycles.

- **Quest, scene and scripting runtime** — SCEN fragment population and phase
  dispatch landed; quest fragment cascades are queued and bounded; global
  values, `GetStageDone` conditionals, persistent reference `Disable`, actor-
  specific trigger gating, horse tethering and shared package re-evaluation
  gained canonical ECS sinks. Save registration now includes globals,
  continuations and reference-enable state.
- **Save/load reliability and UI** — quicksave/load now publish player-facing
  messages, quickload falls back newest-to-oldest across corrupt slots, typed
  column decoding happens before destructive teardown (#3163), dead actors are
  reconciled even on an unexpected apply failure, and exterior reload uses a
  full-radius bootstrap before FormID delta overlay (#3280).
- **Safety/correctness audit fixes** — the ESM GRUP walker shares a bounded
  recursion budget (#3237); Ball/Capsule/Cylinder collider inputs are bounded
  like Cuboid (#3238); NIF light direction, morph original-index alignment,
  canonical BGSM specular selection, FO4 WATR fog decoding, and mapped morph
  weight synchronization were corrected (#3232/#3233/#3234/#3270/#3244).
- **Signal and regression guards** — CI now compiles past FSR after Vulkan
  headers were installed in every relevant job, exposing separate runner gaps
  (headless wgpu adapter, Python in the shader container, XKB in lavapipe) that
  still keep the overall workflow red. The current ABBA leg additionally
  exposes six acquisition-order failures in binary tests despite #3260/#3303,
  so lock-order signal is improved but not clean. The ECS cached-pointer model
  gained a scoped Miri job, the scheduler's zero-conflict invariant became
  cargo-test reachable, and focused
  guards landed across LVLI, UI capture, AVM1 request IDs, HKX malformed input,
  Material finiteness, NIFAL roles, and storage dirty ordering. #3259 restored
  exhaustive example compilation; #3260 and #3303 broke two lock-order cycles.

Net: tests 5869→5965 (+96, 0 failing); ignored 161→162; Rust `src/` LOC
426 409→433 473 (+7064), total 457 584→464 639 (+7055); source files remain
956 total / 886 outside `tests/`; workspace members remain 27; open issue
directories 3123→3176 (+53). The bench-of-record remains `34074b93`, now 503
commits stale; no GPU is available in this environment for the 75-run refresh.
...
- **M40 Phase 2 — interior↔exterior cell-swap orchestrator (`f6b9911a` / `1e92a471` / `a7cc9184`)** — Stage 1 plumbs `DoorTeleport` extracted from REFR XTEL sub-records into `PlacedRef` (destination form-id + position + rotation triple); reverse-lookup helper `cell_for_refr_form_id(form_id) → Option<CellRef<'_>>` lets the orchestrator resolve the destination cell from the door's target REFR without re-walking the masters. Stage 3a wires the interior-side: `script.activate <door_id>` (Phase 4 console command from M47.0) hits an interior↔interior door, the orchestrator tears the source cell down and loads the destination through the existing `load_cell_with_masters` path. Stage 3b extends to interior↔exterior — destination resolves to `(wrld, grid)` instead of an interior `CellRef`, triggers `load_exterior_grid` with the same radius the engine launched with. Door-teleporter spawn placeholder for cells without authored doors landed in the M28.5 follow-up (`826d3cfb`) so the developer can demonstrate the swap from any test cell.
...
- **Permanent fix: `open_with_numeric_siblings`** — New helper, now in [`byroredux/src/asset_provider/archive.rs`](byroredux/src/asset_provider/archive.rs), wraps the `--bsa` / `--textures-bsa` open path. After the explicit archive opens, scans the parent for `<stem>2.bsa` … `<stem>9.bsa` siblings and opens each that exists. The rule fires only when the explicit path has *no digit immediately before* `.bsa` / `.ba2`, so Skyrim's already-numeric `Skyrim - Meshes0.bsa` / `Meshes1.bsa` is inert (the user already lists each archive). FNV's split is now transparent. Same helper applied to mesh `--bsa` arg for symmetry; harmless on FNV (no `Meshes2.bsa` exists) and groundwork for any future game that splits meshes the same way. Verified end-to-end: original CLI logs `Opened sibling textures archive: '...Fallout - Textures2.bsa'` on every FNV launch.
