...
# source-command-audit-incremental-skill

Use this skill when the user asks to run the migrated source command `audit-incremental-SKILL`.
...
real audit skills (each lives at `.Codex/commands/audit-<NAME>/SKILL.md`).
Use those for the authoritative checklist of any one area.

See `.Codex/commands/_audit-common.md` for project layout, severity,
methodology, deduplication, context rules, and the base finding format.
See `.Codex/commands/_audit-severity.md` for the severity scale + special
