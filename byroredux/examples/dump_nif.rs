...
fn main() {
    ...
    // Parse NIF
    let scene = byroredux_nif::parse_nif(&nif_data).expect("Failed to parse NIF");
    ...
    for (i, block) in scene.blocks.iter().enumerate() {
        ...
        // Try as NiTriShape
        else if let Some(shape) = block
            .as_any()
            .downcast_ref::<byroredux_nif::blocks::tri_shape::NiTriShape>()
        {
        ...
        // Try as NiTriShapeData
            ...
            .as_any()
            .downcast_ref::<byroredux_nif::blocks::tri_shape::NiTriShapeData>()
        {
            println!();
        ...
        // Try as NiTriStripsData
            ...
            .as_any()
            .downcast_ref::<byroredux_nif::blocks::tri_shape::NiTriStripsData>()
        {
            println!();
            println!("  vertices: {}", data.vertices.len());
            println!("  strips: {}", data.strips.len());
            let tri_count: usize = data
                .strips
                .iter()
                .map(|s| if s.len() >= 2 { s.len() - 2 } else { 0 })
                .sum();
    ...
    for (i, m) in meshes.iter().enumerate() {
        ...
        println!(
            "  rotation (quat xyzw): ({:.4}, {:.4}, {:.4}, {:.4})",
            m.rotation[0], m.rotation[1], m.rotation[2], m.rotation[3]
        );
