...
/// and threads the result through each [`NpcSpawnJob::advance`](resumable::NpcSpawnJob::advance)
/// call so the clip lands in the registry at most once per cell.
...
/// patch. Called from each `NpcSpawnJob::advance` success path (both the
/// runtime and pre-baked-FaceGen steps in `resumable.rs`) before returning.
/// BFS over `Children`, mirrors
