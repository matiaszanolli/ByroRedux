...
//! furniture, the first visible autonomous NPC behavior.
//!
...
//! snap the actor's placement-root [`Transform`] onto the seat, and switch
//! its [`AnimationPlayer`] to the generic sit loop. FNV `dynamicidle_*`
//! clips are authored in marker-local space, so placing the root at the
//! marker + playing the loop yields a correct seated pose.
//!
//! v0 scope (documented approximations — see the M42 plan): nearest free
//! chair, seat once, no scoring / scheduling / meals / wander / ownership,
//! no pathing (snap-to-seat), generic sit clip, furniture-rotation facing.
