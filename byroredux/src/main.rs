...
const BENCH_GPU_KEYS: [&str; 22] = [
    ...
    "skin_palette",
    "depth_history_copy",
    // SKYAL sky-cubemap bake — appended, same rule.
    "sky_cube",
    ...
    fn bench_gpu_keys_match_the_reported_bracket_order() {
        assert_eq!(
            ...
            "the bench line reports 22 of gpu_timers.rs's 23 brackets — the \
             ...
             `--bench-groundcover-sampling` bracket is not. The line reported \
             12 of them until #3629/#3667 while claiming a full per-pass \
             breakdown"
        );
...
fn build_debug_ui_snapshot(
    ...
    let metrics = world
        ...
        .map(|m| byroredux_debug_ui::panels::MetricsSnapshotView {
            ...
            gpu_busy_pct: m.gpu_busy_pct,
            gpu_memory_busy_pct: m.gpu_memory_busy_pct,
            driver_vram_used_mb: m.driver_vram_used_mb,
            driver_vram_total_mb: m.driver_vram_total_mb,
            vulkan_heap_used_mb: m.vulkan_heap_used_mb,
            vulkan_heap_budget_mb: m.vulkan_heap_budget_mb,
