...
fn every_component_or_resource_impl_is_saved_or_explicitly_allowlisted() {
    ...
    const NOT_SAVED_BY_DESIGN: &[(&str, &str)] = &[
        ...
        ("PoolRegenConfig", "immutable game-profile regeneration tuning; forward-latent — inserted only inside unit tests today, no production site installs it (see feature-matrix.md's regen-unwired note)"),
        ...
        ("WaterSurfaceMesh", "static world-space triangles of a placed mesh-water surface, rebuilt from the NIF at spawn alongside its WaterVolume, no runtime mutator"),
        ...
        ("AiCombatState", "MQ101 combat-gate runtime (Effect::StartCombat): target is a session-local EntityId, same #4139/#1696 hazard class as ActorCinematicState::vehicle; losing an in-progress scripted combat across a save/reload is the same posture CombatState/MeleeState already take for player combat"),
        ("FactionRelations", "Faction.SetEnemy runtime tracking; plain u32 FormIDs rather than FormIdPairs, and nothing currently reads it for ambient hostility (MQ101 always pairs SetEnemy with an explicit StartCombat, which is what AiCombatState above actually drives) — losing it across a save/reload is a fully inert loss"),
        ...
        ("AnimationTarget", "skeleton_root + consumed_idle_serial are both spawn-time-resolved (serial always starts at 0), rederived identically every load"),
        ("IdleClipCatalog", "process-lifetime IDLE FormID -> animation handle mapping, populated once and read-only afterward — same posture as AnimationClipRegistry"),
        ...
        ("Locked", "XLOC lock data, rederived every cell load (#3098) — and since #4136 rederived from the plugin's authored REFR *as overridden by* `ReferenceLockState`, which IS registered. #3159's Effect::SetLocked/SetLockLevel made this component runtime-mutable and the gap was real for that window; it is closed by persisting the FormID-keyed ledger rather than the component. The ledger is the right home because cell unload despawns the placement root: a saved `Locked` column would have survived a save/load and still lost a scripted unlock across an ordinary in-session cell revisit. Same shape as DoorTeleport/NavmeshTile — genuinely rederived — now that its override source is itself save state"),
        ("CellTransitionInFlight", "per-frame mirror of App::interior_transition (#4138), republished at the head of step_player_save_actions purely so SaveCommand::execute can refuse a save taken mid-transition. Derived state with a one-frame lifetime and no gameplay meaning — a restored value would describe the SAVING session's transition, not the loading one, which is worse than deriving it fresh"),
        ...
        ("Owned", "theft ownership (owner form id + faction-rank/global gates) rederived from authored ESM ownership at spawn/load — same rederived posture as Locked"),
        ...
        ("TerrainCoverInputs", "per-cell ground-cover density inputs (LTEX affinity weights + resolved water height) recomputed by the terrain-spawn path every load (#4054) — same posture as its TerrainCellOrigin sibling"),
