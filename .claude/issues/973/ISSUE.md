# Issue #973: FO4-D4-NEW-08-followup: Apply MSWP material-swap list per-shape at mesh-spawn time

**State**: OPEN
**Labels**: bug, renderer, high, legacy-compat, game:fo4, esm-plugin
...
The per-mesh path-resolution loop is at `byroredux/src/cell_loader.rs:2263-2316`. It iterates `imported.iter()` (one entry per NIF shape), and for each shape pulls a single `material_path` via `ov.and_then(|o| o.material_path).or(mesh.material_path)`.
