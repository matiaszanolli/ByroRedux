...
VulkanContext:   crates/renderer/src/vulkan/context/  (mod.rs, draw.rs, resize.rs, resources.rs, helpers.rs, screenshot.rs, geometry_pass.rs, post_passes.rs, skinned_blas_refit.rs — the last three split out under #1857/the FSR3 frame-tail work; depth_capture.rs — the #3308 depth-attachment readback, a deliberate sibling of screenshot.rs with a single consumer, the `depth.stats` console command)
