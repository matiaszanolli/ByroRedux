...
Mod Runtime:     crates/mod-runtime/src/              (byroredux-mod-runtime, added 2026-08-07: sandboxed executable-mod host — the engine-owned boundary between untrusted community code and semantic host services. Guests are WebAssembly Components; each instance gets its own `Principal`/`CapabilitySet`; no WASI is linked by default, so OS access is absent, not merely unused. lib.rs re-exports; bindings.rs, error.rs, identity.rs (`Principal`/`PrincipalId`/`CapabilityId`/`CapabilitySet`), limits.rs (`SandboxConfig`), runtime/ (`SandboxRuntime`/`ModInstance`/`CompiledMod`/`LifecyclePhase`/`FaultInfo`/`LogEntry`; a directory since #3853 — mod, capabilities, convert, host/, host_state, instance, sandbox). Consumed by the engine through byroredux/src/extensions/ (Binary modules row). NO dedicated owner audit skill — see coverage note below.)
Scripting (M12/M47): crates/scripting/src/            (ECS-native scripting runtime: events, timer, cleanup, condition (M47.1 cond eval), trigger (M47.2 TriggerVolume detection), quest_stages, fragment.rs + fragment/, recurring_update.rs + recurring_update/, registry, vm_state, globals, package, equipment, player_control, dialogue, cinematic (the M47.2 MQ101 slice — hkx consumer), combat (NPC combat state — the runtime side of Faction.SetEnemy / Actor.StartCombat), compatibility (extender-era Papyrus compatibility preflight over decoded PEX calls), obscript + obscript_runtime (bounded structural decoding + execution of legacy compiled SCDA scripts), papyrus_provider/ (manifest-published Papyrus provider functions: lowering + execution, split under #3852), scene.rs + scene/{playback, quest_alias} (SCEN playback + the M47.3 quest-alias substrate); translate/ holds the AST→ECS recognizer chain {mod, source, archetype, compose, effects, tables, recognizers/{mod, quest_stage_gate, rumble, two_state_activator}}; papyrus_demo/ holds hand-verified reference scripts)
...
VulkanContext:   crates/renderer/src/vulkan/context/  (18 files, ~21,231 LOC, #4020 2026-09-11 — mod.rs: struct + top-level impl; draw.rs: draw_frame() top-level driver; resize.rs: swapchain recreate; resources.rs: BLAS-for-mesh build, UI-quad register, misc accessors; helpers.rs: depth-format/render-pass/framebuffer construction; screenshot.rs, geometry_pass.rs, post_passes.rs, skinned_blas_refit.rs — split out under #1857/the FSR3 frame-tail work; depth_capture.rs — the #3308 depth-attachment readback, a deliberate sibling of screenshot.rs with a single consumer, the `depth.stats` console command; init.rs / teardown.rs — the `new()`/`Drop` construction+destruction chain, split out under #1749/TD1-004; build_and_upload_instances.rs, assemble_camera_and_lights.rs, dispatch_skin_and_cluster.rs, sync_and_acquire_frame.rs, begin_frame_recording.rs — five more `draw_frame` sub-phases split out under #3282/TD1-2026-08-24-01 (draw-command→GpuInstance translation + batching; camera/light assembly; skin dispatch + BLAS refit + TLAS build + cluster cull; fence-wait + swapchain acquire; command-buffer open + clear-value/instance-map setup); render_debug.rs — runtime structured-debug-mode + selected-ray-probe control)
...
Tex Registry:    crates/renderer/src/texture_registry/ (mod, lookup, upload, release — split by lifecycle phase under #3737) + crates/renderer/src/texture_registry_tests.rs
...
Binary modules:  byroredux/src/ — the top-level files no other row below covers.
                 · boot/ — engine boot, a directory since #3855: cli.rs (the expand_boot_request seam), world.rs (build_world), registries.rs (install_runtime_registries), and schedule/{mod,early,update,post_update,physics,late}.rs — scheduler construction: every add_system / add_exclusive registration and its declared resource access. The schedule/ files are the authority for "which stage does X run in".
                 ...
                 · interaction.rs (1626 LOC) + ui_input.rs — winit→UI input routing (/audit-ui Dim 7) AND, since 2026-08-15, the canonical player-interaction producer: InputAction/ActionState, camera_ray, activation/pick, and the hold/look action edges every OnActivate consumer reads. Its interaction_system is the first exclusive in Stage::Update (boot/schedule/update.rs). It has outgrown the "/audit-ui Dim 7" framing — the UI-routing half is /audit-ui's, the action/ray/activation half belongs to the gameplay slice below.
                 · studio_host.rs (402 LOC, `21a840d5` 2026-08-25) — the engine-side host adapter for `crates/sdk`: snapshots ECS state for the renderer-independent Studio tooling. Landed without a layout row; see #3457.
                 · extensions/ (`24df5304` 2026-08-31, "host sandboxed extensions natively"; a single 10.6k-LOC file until #3843 split it on 2026-09-11 into mod, install, systems, dispatch, commands, capture, persist, legacy_compat + tests.rs) — the live engine-side consumer of `crates/mod-runtime`'s `SandboxRuntime`: constructs it, calls `compile()`/`instantiate()`, and bridges ECS events/commands to guest WASM components under capability gating (28 named `*_CAPABILITY` constants in `crates/sdk/src/service.rs`). Wired from `main.rs` (`load_requested_extensions`, `queue_session_event`) and `app_events.rs` (`shutdown_extension_host`, `extension_ui_menu_sync`). Landed without a layout row (#3828 / DOC-ROT-1) — was invisible to audit-suite routing despite being an order of magnitude larger than every other file in this list. Owner audit: /audit-safety Dimension 11.
 · debug_load.rs, list_cells.rs, name_lookup.rs, parsed_nif_cache.rs, scene_import_cache.rs, ownership_sample.rs, groundcover_translate.rs, anim_convert.rs, helpers.rs, bench.rs, bench_camera.rs, scheduler_access_tests.rs, groundcover_translate_tests.rs, ownership_sample_tests.rs, workspace_hygiene_tests.rs.
Gameplay Slice:  byroredux/src/{combat,inventory,settings_io}.rs + the action half of interaction.rs — the P2 playable-vertical-slice runtime, added 2026-08-15/16. **NO owner audit skill** (see coverage note below); this is the project's active execution focus, so an audit that ignores it is auditing last month's engine.
                 · combat.rs (1284 LOC) — first melee vertical slice. MELEE_REACH_BU = 180.0, MELEE_COOLDOWN_SECONDS = 0.45, UNARMED_DAMAGE = 8.0. combat_input_system + combat_damage_system are both Stage::Update exclusives (boot/schedule/), joined by reconcile_pending_dead_actors_system (the shared combat/save-load death reconciler, #3119) in Stage::Late; CombatState is a Resource holding cooldown + the CombatTraceEntry smoke evidence, alongside PendingDeathReconciliations. Casts from the active camera, resolves a ragdoll bone through ActorColliderOwner, emits the canonical scripting HitEvent, applies damage to the Health ActorValue and owns the alive→dead transition (Dead + the AI-behavior teardown). Transient HitEvent cleanup stays in the scripting Late stage — do not report that as a leak.
                 · inventory.rs (1096 LOC) — native inventory presentation + player-facing equipment mutation. Canonical state stays Inventory + EquipmentSlots (crates/core); this module only carries immutable item metadata (InventoryCatalog / InventoryItemDefinition, FxHashMap-keyed by form id) and seeds the player from base NPC_ 0x00000007. Form 0x00000014 is the placed player reference, not the base record. A finding that it duplicates canonical inventory state is a real NIFAL-style boundary violation; a finding that it *caches* record metadata is by design.
                 · settings_io.rs (**7 LOC — a re-export shim, not a module**) — the body moved to `crates/settings-io` in `e05b4a9f` (2026-08-30, "Share one settings model between the launcher and the engine"), which is on the un-owned list below. Audit the settings-persistence behaviour THERE; this file only re-exports `load` / `save` / `SettingsPersistence` / `SETTINGS_PATH_ENV`, and its 7 lines do not belong in the Gameplay-slice LOC total.
...
Systems:         byroredux/src/systems.rs (module index) → systems/{animation, audio, billboard, bounds, camera, character, cinematic, combat_ai, debug, escort, follow, guard, light_anim, locomotion, metrics, navmesh_path, particle, patrol, sandbox, travel, wander, water, weather}.rs (character.rs is the PLAYER/character controller — physics, not CHARAL; combat_ai.rs is npc_combat_ai_system, the MQ101 NPC combat slice and a second HitEvent producer beside combat.rs; navmesh_path.rs is NAVM A* pathfinding, docs/engine/navmesh-pathfinding.md; cinematic.rs drives the M47.2 scripted-camera slice) (particle.rs carries apply_emitter_params, fed by the typed NIF emitter pipeline; sandbox/wander/travel/follow/escort/guard/patrol.rs are the M42 AI-package procedure runtimes — see "Sandbox AI" row below; locomotion.rs is their shared `step_toward` walk-to-point primitive)
...
SDK / Studio:    crates/sdk/src/ (25 files, ~14,245 LOC, `e142e3d4` 2026-09-09) — renderer-independent tooling surface; `StudioSession` is a Resource. Grew far past its original `lib.rs + studio.rs` shape (#3457/#3847): now also exposes actor values, equipment/inventory, factions/reputation/relationships, plugin/load-order metadata, form lookup, UI/menu state, input mappings, and a `compatibility/` subdirectory (~5,500 LOC) to provider calls. Its engine-side adapter is byroredux/src/studio_host.rs (402 LOC — Binary modules row). **No owner audit skill** — see the un-owned-subsystems table below; `/audit-ecs` and `/audit-concurrency` have both reached it incidentally (#3445).
NIFAL Translate: byroredux/src/material_translate.rs (translate_material — the SINGLE raw material → ECS Material boundary; per-game material classification happens here, never in the shader) + crates/core/src/ecs/components/material.rs (Material::resolve_pbr; canonical metalness/roughness are plain f32 fields, resolve-once). Spec: docs/engine/nifal.md. See also /audit-nifal.
                 ...
                 · `GpuMaterial` grew 300 → 348 B for the twelve common supplemental role indices, then 348→364→396→432 B (animated shader color/float fields, BGEM glass optics, soft/rim/back Bethesda lighting response), then 432→428 B (#3909), then 428→432 B (#4422 `detail_neutral`) — current size is pinned by `gpu_material_size_is_432_bytes` in `crates/renderer/src/vulkan/material_tests.rs`, not `_348_` or `_428_`. The GLSL mirror in `crates/renderer/shaders/include/bindings.glsl` must match field-for-field, not just in size.
...
| `docs/feature-matrix.md` | What works at runtime per game — cell loading, rendering, NPCs, audio, scripting, physics, UI. Living status document. (Treat it as a status floor, not a record of what exists: re-check each row against the crate that implements it before citing it, and flag any row that contradicts the code. The M45/M47.2 rows were brought current on 2026-06-21.) |
...
Crate count: 29 under `crates/` — audio, bgsm, boot-request, bsa, core,
cxx-bridge, debug-protocol, debug-server, debug-ui, facegen, fsr3-sys,
game-detect, hkx, menuxml, mod-runtime, nif, papyrus, pex, physics, platform,
plugin, renderer, save, scripting, sdk, settings-io, sfmaterial, spt, ui.
...
| Mod Runtime (sandboxed mods) | `crates/mod-runtime/src/` | `/audit-safety` Dimension 11 (added 2026-08-13) | A trust boundary between untrusted WASM guest code and the host. **Live since `24df5304`**: `SandboxRuntime` is constructed and driven by `byroredux/src/extensions/` (`mod.rs`, `install.rs`) — audit it as a live path, per `/audit-safety` Dim 11 |
...
Skyrim LE:     /home/matias/Games/skyrim-original/drive_c/Program Files (x86)/The Elder Scrolls V Skyrim/Data/
               ...
               not SE's v105 LZ4 / 1.71 / v44. Tests read it via BYROREDUX_SKYRIMLE_DATA, falling back to
               this prefix — crates/nif/tests/common/mod.rs, since fb8173fe.)
