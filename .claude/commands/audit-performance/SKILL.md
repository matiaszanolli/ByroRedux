...
**Checklist**: Sort-key effectiveness — `draw_sort_key` returns an 11-tuple `(u8,u8,u8,u32×8)` ordering by pipeline/material/texture before depth; verify it actually minimizes pipeline + descriptor-set rebinds in the draw loop. Draw-call-count vs entity-count ratio (the compat table reports both per cell). Instanced batching — same mesh + multiple transforms should collapse, not emit N draws. Push-constant / per-draw state churn (`cmd_set_depth_bias` on every draw?). The parallel-sort gate: `sort_draw_commands` switches the raster prefix to `par_sort_unstable_by_key` only at `DRAW_SORT_PARALLEL_THRESHOLD` = 3000 commands (`render/mod.rs` ~429); verify the threshold matches the typical Bethesda cell range (400–1500) — a too-low gate pays rayon spin-up on small cells, too-high starves big exterior grids. **GT-presence hoist guard (#1377)**: the static-mesh loop does `if tq.get(entity).is_none() { continue; }` before the vis/wb sibling probes — dropping the hoist re-pays two SparseSet gets per GT-less entity. **Two-sided blend split gate (#1804)**: `needs_two_sided_blend_split(&DrawBatch)` (`crates/renderer/src/vulkan/context/draw.rs`) requires `z_write` in addition to `is_blend && two_sided` (glass keeps it true; particles are the one `z_write:false` batch type) — the split used to run unconditionally, paying a dead FRONT-cull pass every particle batch. A split reappearing on non-depth-writing batches is the regression.
