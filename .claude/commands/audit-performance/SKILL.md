...
- **Early-return rollback (regression guard, #1791/#1796)**: `ctx.skin_dispatch_ran` must be checked after every `draw_frame` return — `draw_frame`'s two early-return guards (empty framebuffers, `ERROR_OUT_OF_DATE_KHR`) used to leave `SkinSlotPool`'s CPU-side commits (pose-hash baseline, first-sight `bind_inverses` drain) irrevocably applied even though the GPU dispatch never ran, freezing RT skinned output/BLAS geometry for N+ frames and permanently corrupting a first-sight entity's skin palette on a resize. `byroredux/src/app_frame.rs` (`render_one_frame` — moved out of *main.rs* by the #2731 split) resets `skin_dispatch_ran` before both guards and sets it true only in `record_skinned_blas_refit`; a bailed frame must roll back the pose-hash commit and requeue drained first-sight `bind_inverses`. The scope guard is `skin_dispatch_ran_rollback_scope_tests` in `byroredux/src/app_frame.rs`, which asserts the rollback check comes *after* the dispatch call.
...
**Entry points**: `crates/renderer/src/vulkan/gpu_timers.rs` (`GpuPerFrameTimers`, `GpuTimerSnapshot`), `crates/core/src/ecs/resources/mod.rs` (`ScratchTelemetry`), `byroredux/src/render/camera.rs` (`assemble_camera`, render-origin snap), `crates/renderer/src/vulkan/context/draw.rs` (per-instance model rebase ~1805, `origin_corrected_prev_view_proj`)
**Checklist**:
