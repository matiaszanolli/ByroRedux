...
- **LOD selection** — one tier per object, not all three (the `a30c088a` fix), chosen by `select_finest_lod` (extracted from an inline `max_by_key` under #3641: highest LOD index wins a triangle-count tie, matching the prior incidental behavior — 49 of 46,422 measured CSG objects hit the tie). A regression here triples precombine geometry; a refactor that inlines the picker without preserving the explicit tie-break silently reverts #3641.
