...
- **Collision-shape translation completeness** (`import/collision/mod.rs::extract_collision`): MUST translate `BhkMultiSphereShape` + `BhkConvexListShape` to `CollisionShape` (the `downcast_ref::<…>()` arms must stay). Regression = a shape that parses but never reaches `extract_collision`.
