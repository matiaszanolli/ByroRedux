...
**Entry points**: `crates/renderer/src/vulkan/acceleration/` (`blas_static.rs`, `blas_skinned.rs`, `tlas.rs`, `predicates.rs`, `constants.rs`, `types.rs`), `crates/renderer/src/vulkan/context/resources.rs` (`restore_missing_static_blas_for_draws`, `build_blas_batched`). (`build_blas_for_mesh` and the single-shot `build_blas` were deleted under #2914 — had no caller anywhere in the workspace; `docs/engine/memory-budget.md` documents the deletion.)
...
- Transform: column-major `mat4` → 3×4 row-major `VkTransformMatrixKHR`. `TRIANGLE_FACING_CULL_DISABLE` gated on `draw_cmd.two_sided` (#416) — a blanket enable is the regression, not the target: pre-#416 every instance disabled backface culling, so shadow/GI rays hit the interior backfaces of closed single-sided architecture from outside (~2x ray cost). Do NOT file the gate as a bug (#3996 — this bullet said "on all instances" and pointed auditors at correct code).
...
- LRU/shrink wiring (regression guards): `shrink_tlas_scratch_to_fit` uses `tlas_scratch_should_shrink` (`TLAS_SCRATCH_SLACK_BYTES` = 256 KB, `predicates.rs`); the 1 MB-slack `tlas_instance_should_shrink` is the *instance-buffer* predicate used by `shrink_tlas_to_fit` (#3996 — both are TLAS-calibrated, but a symbol name in a No-Guessing checklist has to be the right one), called at the END OF `draw_frame` (`context/draw.rs`), NOT at cell-unload (#1226 / REN-LOW L-2 — cell-unload calls the *different* `shrink_blas_scratch_to_fit`; matches `docs/engine/memory-budget.md`); the three `missing_blas` cause-counters (skinned/rigid/ssbo_evicted) all increment, and besides the rate-limited (once/sec) `log::warn!` in `build_tlas_instances` they are read **every frame** by a registered command. There is still NO `mem.stats` command (#1228 / REN-LOW L-1), but the telemetry gap itself closed in `9c805cd7` (2026-08-14). **Regression guard, not a gap to re-report**: verify the `integrity_snapshot()` -> `fill_rt_integrity_stats` -> `RtIntegrityStats` -> `rt.integrity` chain still runs per-frame from `app_events.rs`, and that `RtIntegrityStats::verdict` still requires `tlas_emitted == tlas_eligible` with all three counters zero. #3999 extended the same chain with five BLAS-residency figures (`blas_total_bytes`, `blas_static_bytes`, `blas_pending_destroy_bytes`, and the two pending counts). #3996 — this clause denied that any registered command read them, for three weeks after that stopped being true, and produced a false finding in TWO consecutive sweeps (`REN-2026-08-30-D1-01`, `REN-2026-09-05-D1-02`), the second explicitly recommended for filing. Grep the METHOD name `integrity_snapshot`, not the struct `TlasIntegritySnapshot` — greping the struct is what let both sweeps miss the call; post-TLAS `rt_flag` patch in `draw_frame` keeps cell-load frames from rendering RT-disabled (#1227). Of #1793's two original gaps, only one is still open: a synchronous multi-cell burst (`--grid`) can false-evict a not-yet-drawn entry via the shared `frame_counter` bump (`blas_static.rs` still carries the "Deferred pending a `--grid` + low-VRAM-budget repro" note — search that phrase; the `:228-238` range this cited rotted off it, #3996) — gated behind `static_blas_bytes > budget`, unreachable on the 12 GB dev card. The other half — "a permanently-missing rigid BLAS has no recovery path" — is **closed**: `VulkanContext::restore_missing_static_blas_for_draws` (`resources.rs`, landed `8e7582ed` 2026-08-16) is exactly that per-frame build primitive, called every frame from `app_frame.rs` before `draw_frame`. Verify it still runs before `draw_frame` and still calls `mark_static_blas_used` **before** `handles.retain(|h| !accel.has_blas(h))` (pinned by the source-position test `static_blas_recovery_covers_both_sources_and_protects_current_draws` in `resources.rs` — the `resources.rs:473-497` line range this doc previously cited has rotted off the ordering it once pointed at; anchor on the test name, not a line range); recast only the `--grid` half, don't re-report either as new.
...
**Checklist**:
- `instance_custom_index` (NOT *gl_InstanceID*, which this codebase never uses) indexes `GpuInstance[]`; `materials[instance.material_id]`, vertex/index SSBOs (Set 1 bindings 8/9 per shader-pipeline.md) use the same offsets the Rust upload writes.
...
- Glass / IOR refraction:
  ...
  - The glass ray budget is `qualityTier`-gated (`budgetTier = min(rayBudget.qualityTier, 3u)` in `triangle.frag`), not a `GLASS_RAY_BUDGET` shader-side cap — no `.frag`/`.comp`/`.vert` reads that constant; it only derives the per-tier `glass_ray_limit` telemetry value in `ray_budget.rs`. The budget `atomicAdd(rayBudget.rayBudgetCount, glassRayCost)` overshoots unconditionally by design (#1438) and is telemetry only — verify the negative pin in `shader_constants.rs` (`!src.contains("old + glassRayCost <= rayBudget.glassRayLimit")`) still holds and no CPU reads the counter.
  ...
  - **Thin-glass gate (regression guard, #883f57cd).** `MAT_FLAG_THIN_GLASS` (bit 11) forces non-occluding glass (open window panes, display-case fronts) onto the zero-ray Fresnel/framebuffer-transmission path: `glassIORAllowed = isGlass && !isThinGlass && reflectionGlassRayEnabled && !isWindow`. Occluding/thick glass (bottles, canopies) keeps the full Snell/RT path at every RT LOD; `triangle_frag_keeps_glass_identity_and_ior_across_rt_lods` pins both the live gate and the absence of an RT-LOD cutoff. BGEM classification is pinned by `bgem_uses_thin_glass_behavior` / `closed_bgem_glass_does_not_select_thin_surface_behavior` / `legacy_bgem_effect_cards_do_not_become_glass` (`asset_provider/tests/bgsm_merge.rs`) — thin only for non-occluding transmissive shells, never plain closed BGEM glass or effect cards.
...
- **ReSTIR-DI surface-identity tag now uses the stable surface ID (regression guard, #883f57cd).** The reservoir's surface tag is `uint surfaceId = inst.surfaceId & RESERVOIR_SURFACE_MASK` (mask `0x3FFFFF`), replacing the old `fragInstanceIndex + 1` — so spatial-reuse validity survives per-frame draw-order/batch reordering instead of going stale whenever the draw list re-sorts. Test: `restir_history_uses_stable_surface_id_not_instance_order` (`shader_contract_tests.rs`).
- BC1 punch-through alpha (regression guard, #ae285062): a pure-blend mesh whose BC1/DXT1 diffuse decodes index-3 texels as `a==0` (an RGB-fidelity encoder choice, NOT transparency) must NOT leak into blend-discard / decalWeight / finalAlpha. `triangle.frag` pins `texColor.a = 1.0` when `INSTANCE_FLAG_DIFFUSE_ALPHA` (bit 8) is clear and no alpha test is active. The CPU bit is set in `context/build_and_upload_instances.rs` (#4020 — moved here from `draw.rs` under #3282/TD1-2026-08-24-01) from `format_has_alpha` (which excludes `BC1_RGBA`). Regression = a BC1-blend mesh speckling/pinholing again.
...
**Checklist**:
- Sizes pinned by tests — confirm they hold and match shader-pipeline.md: `GpuInstance` = **160 B** (`gpu_instance_is_160_bytes_std430_compatible`; grew 112→128 with the `skinned_vertex_address` + `_reserved` vec4 slot (#2219), then **128→160 on 2026-08-23 (#3231) with `morph_delta_address` + `morph_weight_address` + `morph_target_count` + reserved padding** for GPU morph-target blending — the `_160_` test name is current; `_128_` and `_112_` are both gone), `GpuCamera` = **368 B** (`gpu_camera_is_368_bytes` — grew 320→336 with the `render_origin` vec4 (#markarth-precision / #1492), then 336→352 on 2026-08-16 (`8e7582ed`) with the appended `render_debug` uvec4, then **352→368 with the appended `exterior_sky_tint` vec4 (#3323, the live exterior sky transmitted through interior window portals)**; the `_368_` test name is current, `_352_`/`_336_`/`_320_` are all gone. `render_debug` is APPENDED at the end; enumerate direct declarations with `rg "uniform CameraUBO" crates/renderer/shaders` and separately inspect fragment consumers that obtain the shared declaration through `include/bindings.glsl`, rather than trusting a transcribed count), `GpuMaterial` = **432 B** (`gpu_material_size_is_432_bytes`; NOTE the size grew 260→…→300 via #804/#1249/#1250, then 300→348 on 2026-07-27 (`1d94eb24`) when the twelve common supplemental texture roles landed, then 348→364 on 2026-08-23 (#2221) with the `shader_color_r/g/b`/`shader_float` animated-sink fields, then **364→396 on 2026-08-25 with BGEM v21+ glass optics (`glass_fresnel_color`/`glass_refraction_scale`/`glass_blur_scale`/`glass_blur_scale_factor`), then 396→432 same day with the Bethesda soft/rim/back lighting response (`lighting_effect_1/2`, `subsurface_rolloff`, `rimlight_power`, `backlight_power`, `fresnel_power`, `grayscale_to_palette_scale`, `lighting_mask_map_index`, `back_lighting_map_index`)** — the `_432_` test name is current; `_396_`/`_364_`/`_348_`/`_300_`/`_260_` are all gone. The dedup hash now walks 108 scalar fields (`hash_gpu_material_fields`, #1368), up from 92 — re-count with `awk` over the function body rather than trusting this figure, it moves with every field the struct gains. The role indices remain the largest single growth in this struct's history: re-check the GLSL mirror in `crates/renderer/shaders/include/bindings.glsl` field-by-field, not just the size.
- **`GpuInstance.surface_id`** (`u32`, offset 108, regression guard #883f57cd) repurposes the old `_pad_albedo` padding into a stable per-entity surface identity (`draw_cmd.entity_id.wrapping_add(1)` — 0 reserved for background/synthetic), used by TAA/SVGF disocclusion and ReSTIR-DI reuse to survive per-frame draw-order reshuffling. `surface_id`'s own offset (108) is unaffected by the #3231 growth — the new morph-address fields were appended after `_reserved` (offset 120), not before it. Pinned by `gpu_instance_field_offsets_match_shader_contract` (asserts offset 108, plus `skinned_vertex_address` at 112 and `_reserved` at 120) plus `restir_history_uses_stable_surface_id_not_instance_order` / `gbuffer_history_uses_stable_surface_id_but_caustics_keep_draw_lookup`.
...
- `struct GpuInstance` is declared once in `crates/renderer/shaders/include/bindings.glsl` (pulled into `triangle.frag` via `#include`) and **hand-mirrored in 4 standalone shaders** — verify lockstep via `grep -rlE '^struct GpuInstance' crates/renderer/shaders/` → `include/bindings.glsl`, `triangle.vert`, `ui.vert`, `water.vert`, `caustic_splat.comp` (5 declaration sites; `ui.vert`/`water.vert` reading wrong offsets is the recurring trap, #785/#1498). **Anchor the grep to a declaration** (`^struct GpuInstance`, not a bare `"struct GpuInstance"`) — the unanchored form now returns 7 (#4031, 2026-09-06): `skin_vertices.comp`'s comment noting that shader has no `struct GpuInstance`, plus `volumetrics_inject.comp`, which acquired three `GpuInstance`-naming comments under `fa5c4191`. There is also a **sixth real mirror** the anchored grep cannot see by name: `volumetrics_inject.comp` declares `struct GpuBoundaryInstance`, which reads the very same per-frame `GpuInstance` SSBO (`VolumetricsPipeline::write_boundary_geometry` writes the scene `instance_buffer` into its binding 19) and is pinned separately by `gpu_boundary_instance_stride_matches_gpu_instance` (#3829). Use `grep -rhoE '^struct [A-Za-z_]+' crates/renderer/shaders/` and inspect anything binding set 1 / binding 4 or 19 so a renamed mirror cannot rot this count the same way again. Every mirror must carry the `surfaceId` field (renamed from the old albedo pad, #883f57cd) — `grep -L surfaceId` across the 5 named-`GpuInstance` sites should return nothing. Per `feedback_shader_struct_sync.md`.
- Flag constants are the single source of truth in `crates/renderer/src/shader_constants_data.rs`, emitted into `crates/renderer/shaders/include/shader_constants.glsl` and `#include`d — never hand-written shader-side: `INSTANCE_FLAG_*` (`NON_UNIFORM_SCALE` bit 0, `ALPHA_BLEND` bit 1, `CAUSTIC_SOURCE` bit 2, `TERRAIN_SPLAT` bit 3, `FLAT_SHADING` bit 7, `DIFFUSE_ALPHA` bit 8 — the BC1 punch-through gate, #ae285062), `MATERIAL_KIND_*`, `MAT_FLAG_*` (bits 0–9, including `PBR_BSDF` bit 5, `TRANSLUCENCY` bit 6, `MODEL_SPACE_NORMALS` bit 7, `TRANSLUCENCY_THICK_OBJECT` bit 8, `TRANSLUCENCY_MIX_ALBEDO` bit 9 — all canonical post-#1357, the `BGSM_*` prefix is gone; plus `THIN_GLASS` bit 11 — bit 10 unused/reserved — gating occluding-vs-non-occluding glass, #883f57cd; plus **`SOFT_LIGHTING` bit 13, `RIM_LIGHTING` bit 14, `BACK_LIGHTING` bit 15** (2026-08-25, Bethesda lighting-response fields) — bit 12 is `MSN_HAS_AUTHORED_Z`), and the `DBG_*` bits (**35 catalogued entries as of 2026-08-29**, `0x1`…`0x80000000` — up from 24 at `8eaade44`, so the bitmask now reaches the top of the `u32`; value-pinned by the shared catalog. Read `DBG_BITS` rather than trusting any figure quoted here — it has moved twice since the figure was first written, and a 32-bit-exhausted mask is itself the next finding).
- Capacity constants match memory-budget.md: `MAX_INSTANCES = 0x40000`, `MAX_MATERIALS = 16384` (`scene_buffer/constants.rs`), `MAX_INDIRECT_DRAWS = MAX_INSTANCES`. Over-cap material intern returns id 0 + one-shot `warn!` (no SSBO-index corruption, #797); upload truncates to `min(table.len(), MAX_MATERIALS)` — the unique-material count, **not** `interned_count()` (#4045; that accessor is the total-call-count denominator of the dedup ratio, one to two orders of magnitude larger on a real cell).
...
- **`AllocatorResource` ECS-ordering**: must be removed from the `World` BEFORE `VulkanContext::drop()` — the allocator holds a live `Arc<Device>`; a `World` outliving the context fires the allocator Drop against a destroyed device (use-after-free). Verify the drop/remove ordering in `byroredux/src/app_events.rs` (the `remove_resource` at ~line 59 — moved out of *main.rs* by the #2731 split), and that it survives a panic-unwind path (#1406). Allocator-independent destroys are hoisted out of the allocator-guarded Drop block (#1483).
- BLAS scratch high-water-mark reuse never shrinks mid-life (verify no use-after-free); of the two shrink fns, only `shrink_blas_scratch_to_fit` runs at cell-unload (`cell_loader/unload.rs`, plus `context/resize.rs`); `shrink_tlas_to_fit` and `shrink_tlas_scratch_to_fit` both run at the end of `draw_frame`. Slack constants from memory-budget.md (REN-LOW L-2).
...
- **Global geometry SSBO rebuild is resumable and gated (regression guards, #3298 / #3372 / #3443).** `MeshRegistry::rebuild_geometry_ssbo` (`crates/renderer/src/mesh.rs`) no longer rebuilds the whole global vertex/index SSBO in one frame: `advance_geometry_rebuild` + `next_geometry_rebuild_chunk` walk it in chunks across frames, `geometry_rebuild_in_progress` reports the in-flight state, `geometry_rebuild_needs_idle` decides when a chunked rebuild must serialize against the device instead of duplicating buffers, and `rebuild_geometry_ssbo_atomic_fallback` is the whole-frame path it falls back to. Compaction is two-phase on purpose: `plan_geometry_compaction` must NOT publish offsets — only `apply_compaction_plan` does, once the compacted buffer actually binds (#3372, a published-early offset points every draw at the wrong geometry). Pinned by `planning_compaction_does_not_publish_offsets`, `applying_the_plan_publishes_the_compacted_offsets`, `chunked_rebuild_consults_the_idle_threshold_before_duplicating`, `large_rebuilds_idle_only_when_replacing_existing_buffers`. Regression = a single-frame rebuild reinstated, or an offset publish hoisted out of `apply_compaction_plan`.
...
**Entry points**: `byroredux/src/material_translate.rs` (`translate_material` — the single `ImportedMesh → Material` boundary), `crates/core/src/ecs/components/material.rs` (`Material`, `resolve_pbr`, `EmissiveSource`), the particle slice `crates/nif/src/import/walk/mod.rs` (`extract_emitter_params`/`extract_emitter_rate`) → `byroredux/src/systems/particle.rs` (`apply_emitter_overlays` — the single overlay boundary; `apply_emitter_params` is its kinematic+lifetime+size delegate) → `byroredux/src/render/particles.rs` (`emit_particles`). Spec: `docs/engine/nifal.md`. See also `/audit-nifal`.
...
**Checklist**:
- **Single boundary**: `translate_material` has three production callers — `byroredux/src/scene/nif_loader.rs` (loose NIF), `byroredux/src/cell_loader/spawn.rs` (REFR placement) and `byroredux/src/cornell.rs` (the Cornell RT test harness). The invariant is that no `Material {…}` literal is constructed *outside* `translate_material`, not that the caller count stays at any particular number — a further legitimate caller is not a finding, an independently-built `Material` downstream is (#3904).
...
- Particle slice: authored emitter rate/size override the preset but NOT color via the `apply_emitter_params` delegate (`apply_emitter_params_overrides_kinematics_and_size_not_color`); the BGEM effect-shader payload (`BSEffectShaderProperty`) is packed through the same `apply_emitter_overlays` boundary as every other overlay (regression guard, #2610/#3589 — previously hand-copied at each spawn site outside the boundary, the exact divergence class #1513 exists to prevent); render assembly reads `ParticleEmitter` post-overlay.
...
**Checklist**:
- `intern` produces stable `material_id`s within a frame; identical materials collapse to one entry. Over-cap returns id 0 + warn-once, surfaced via the `ctx.scratch` command (#7823eb59/#797 — NOT `mem.stats`/`mem`, neither of which exists; REN-LOW L-1/L-6). **Corrected (#4045):** per-frame SSBO is sized to `min(table.len(), MAX_MATERIALS)` — the unique-material count. `MaterialTable::interned_count()` is a real accessor but names the wrong quantity: its own doc is "total `intern()` calls this frame (hits + misses)", the denominator of the dedup ratio, one to two orders of magnitude larger than the unique count on a real cell; `upload_materials` (`scene_buffer/upload.rs`) never calls it.
- **Corrected (#4045):** `Eq` is byte-level (`GpuMaterial::as_bytes`, depends on the Dim-3 scalar-fields + zeroed-pad invariant) — but `GpuMaterial` has no `Hash` impl, derived or manual, and hasn't since #781. The dedup **key** is the field-walking `hash_gpu_material_fields` (`material.rs`); `DrawCommand::material_hash` must stay in lockstep with it, pinned by `material_hash_matches_gpu_material_field_hash` (`context/mod.rs`).
...
- Composite reassembly: direct + SVGF-denoised indirect + albedo (+ TAA-resolved HDR when TAA on), ACES tone-map is NOT in `composite.frag` — it lives in `presentation.frag` (`aces()`, applied to `graded * params.exposure`), so composite emits linear HDR and bloom is added there, still upstream of the tone-map (Dim 16). Fog applied to direct only, not indirect. SSAO modulates indirect only.
...
- Composite writes the offscreen HDR scene image (`HDR_FORMAT`, final layout `SHADER_READ_ONLY_OPTIMAL`), NOT the swapchain — the FSR upscale then the `presentation.rs` pass consume it, and presentation is what owns the swapchain attachment and its `PRESENT_SRC_KHR` final layout (Dim 23).
**Output**: `/tmp/audit/renderer/dim_8.md`

#### Dimension 9: GPU skinning compute + BLAS refit (M29)
**Entry points**: `crates/renderer/src/vulkan/skin_compute.rs`, `crates/renderer/src/vulkan/morph_compute.rs` (`MorphSlot` — the #3231 GPU morph-target delta/weight buffers `GpuInstance` now carries addresses for), `crates/renderer/shaders/skin_vertices.comp`, `skin_palette.comp`, `acceleration/blas_skinned.rs`, `byroredux/src/render/skinned.rs`, `byroredux/src/render/bone_palette_overflow_tests.rs`.
**Checklist**:
- `VERTEX_STRIDE_FLOATS = 26` (104 B/vertex, since the `[f32;4]` tangent lane #783/M-NORMALS and the `cd2b5fe4` colour `vec3`→`vec4` widening) is defined in `crates/renderer/src/shader_constants_data.rs`, consumed by `skin_compute.rs` via `use crate::shader_constants::VERTEX_STRIDE_FLOATS` (NOT a hardcoded `25`), pinned against `size_of::<Vertex>()` by the assert in `skin_compute.rs` — drift corrupts every skinned vertex.
...
- `SkinPushConstants` — 32 B / six fields (`morph_delta_address`, `morph_weight_address`, `vertex_offset`, `vertex_count`, `bone_offset`, `morph_target_count`; grew from the pre-#3231 12 B/three-`u32` shape), pinned by `push_constants_size_is_32_bytes` — matches the GLSL push-constant struct, well under the 128 B Vulkan-guaranteed minimum.
- Skinned output buffer usage flags are `STORAGE_BUFFER` + `ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_KHR`. `VERTEX_BUFFER` is deliberately ABSENT: `b99ae91e` ("Fix #681 (MEM-2-6)") *removed* it because M29.3 raster still inline-skins in `triangle.vert` and nothing binds the slot buffer as a VBO. Correct as-is — re-add only alongside a Phase-3 raster bind path (REN-LOW L-13).
...
- **`MorphSlot` eviction drains outside the compute/AS guard (regression guard, #3374).** Draining evicted morph slots while holding the `skin_compute` + `accel_manager` guard was a lock-scope defect; the drain now happens after those guards release. Regression = a `destroy` call moved back inside the guarded block. Paired with #3469's cached skinned device address — `skin_compute.rs` caches it rather than re-querying per frame, so a re-added per-frame `get_buffer_device_address` in the draw path is the drift.
...
- G-buffer pipeline writes all eight color attachments (locations 0–7: HDR, normal, motion, mesh_id, raw_indirect, albedo, FSR reactive mask, FSR transparency & composition mask; depth is attachment 8); composite inputs match G-buffer + denoiser outputs; SSAO/cluster-cull compute descriptor layouts correct.
...
**Checklist**:
- Reset-before-record, begin/end balanced (command buffer + render pass), AS build recorded outside the render pass, SVGF/compute after RP end and before composite, then the fixed `record_post_passes` tail (svgf → caustic splat → volumetrics → taa → ssao → composite → bloom → FSR upscale → presentation — bloom moved to AFTER composite by #2796/REN-D16-01, see Dim 16), then egui, then optional screenshot / depth copy. **The Scaleform UI quad moved into the presentation pass (#3426, `b28acb0c`)** — it draws immediately after the tone-map triangle, in the same subpass, at output resolution onto the swapchain, and its pipeline is owned by `PresentationPipeline` (the former `VulkanContext::pipeline_ui` and its geometry-pass lifecycle are retired). Both descriptor sets are rebound before that draw because the tone-map draw binds a layout-incompatible set 0. A finding that places the UI quad at the tail of the geometry pass is written against the pre-#3426 shape — see `/audit-ui` for the full rationale.
...
- **Two-sided blend split keys on material kind, not depth state (regression guard, #1804 → `883f57cd` → #2165).** `needs_two_sided_blend_split` (`draw.rs`) is `is_blend && b.two_sided && b.order_dependent_glass`, where `order_dependent_glass` is set at batch formation from `is_refractive_glass`. Both earlier spellings were wrong in opposite directions: `&& b.z_write` (#1804) excluded the FO4 BGEM glass that motivated the split (commonly `z_write: false`), and dropping the limb entirely (`883f57cd`) re-included every two-sided blended particle batch, whose FRONT-cull pass rasterizes zero fragments and which then falls out of indirect grouping (#2165). Guarded by `splits_when_glass_and_z_write_false` / `splits_when_blended_two_sided_glass_and_z_write` plus the negative pair `does_not_split_two_sided_blended_particles` / `does_not_split_non_glass_regardless_of_z_write` (all in `draw.rs`; the old inverted-assertion name *splits_when_z_write_false* is gone) — a re-added `z_write` term OR a dropped `order_dependent_glass` term is the regression.
...
- History weight is a flat `alpha = 0.1` (`taa.comp`) plus a per-pixel surface-consistency disocclusion test — octahedral-decoded `dot(currNormal, prevNormal) < 0.85` rejects history across a surface change. #1497's `static_frames`-driven progressive alpha FLOOR was deleted by `e5d02f83`; that hazard cannot recur, so do not re-report it. Regression guard: `taa_comp_keeps_history_bounded_and_rejects_unstable_surfaces` (`taa.rs`).
...
- Froxel grid is DERIVED, not fixed: `froxel_extent(render_extent, config)` = `render_extent.{width,height}.div_ceil(froxel_xy_divisor)` × `froxel_z_slices`; read the live defaults from `upscaling.rs::VolumetricsConfig::default` and compute the native/FSR extents rather than trusting quoted dimensions. Dispatch covers exactly that grid at the inject `local_size` (`WORKGROUP_X/Y/Z` = 8³). Size the per-FIF footprint from the live image set and formats allocated by `VolumetricsPipeline::new` — it is not a single RGBA16F buffer. Inject does a single `TerminateOnFirstHit` shadow ray per froxel; integrate multiplies transmittance across the walk; HG `g` clamped to (−0.999, 0.999).
...
- Bloom added **before** ACES tone-map (HDR add), but **runs AFTER composite in `record_post_passes` (regression guard, #2796/REN-D16-01)** — reads and writes composite's own assembled scene (sky + GI + caustics + direct), NOT the pre-composite raw HDR G-buffer attachment, which never contained sky/GI/caustics; a bloom pass reordered back before composite (or rebound to the raw HDR attachment) is the regression. Intensity constant is `BLOOM_INTENSITY` (0.15) in `shader_constants_data.rs` emitted into `include/shader_constants.glsl` — NOT hand-written in `composite.frag`; 5 down-mips + 4 up-mips is `BLOOM_MIP_COUNT` / `BLOOM_MIP_COUNT - 1` (`bloom.rs`); source is composite's un-tone-mapped assembled HDR (NOT the TAA output, and NOT the pre-composite HDR — descriptor-binding regression pattern); disable path short-circuits both dispatch and composite addition.
...
- **Parallax height channel rides bit 31 of `parallaxMapIndex` (regression guard, #3530).** `Material::parallax_height_in_alpha` is canonical NIFAL state (Oblivion's `APPLY_HILIGHT2` route reads height from the normal map's alpha, since Oblivion ships no `_p.dds`); the renderer only *transports* it as the top bit of the parallax index. **Every shader reader must mask bit 31** — not cosmetic: `textures[0x8000000N]` is a wildly out-of-bounds bindless index. **Both** POM marchers — the raster one and the secondary-ray one — must honour the channel, or reflections disagree with the raster pass. Regression = an unmasked read, or one marcher honouring the flag and the other not.
...
- **Clustered-light path rescales diffuse and sheen together (regression guard, #2243/`c4cb2614`).** `shadowableLightRadiance` (`lighting.glsl`) keeps the legacy non-`/π` Lambert convention and must scale the *whole* Disney lobe by `PI` — `diffuseBrdf = (dd.diffuse + dd.sheen) * PI * (1.0 - metalness)` — not diffuse alone; scaling only `dd.diffuse` leaves sheen weighted π× too low relative to diffuse. **#4012 — do not look for a second composition convention to keep this in step with.** `shadowableLightRadiance` is now the ONLY `disneyDiffuseSplit` call site: every directional, point and spot source arrives through it, and `triangle.frag`'s duplicate synthetic no-light sun/BRDF arm (which this bullet used to cite as keeping `(dd.diffuse + dd.sheen) * (1.0 - metalness)`) has been deleted. Regression: `disney_sheen_keeps_its_relative_weight_in_canonical_direct_path` (`shader_contract_tests.rs`) reverting to a diffuse-only `* PI`, and asserting the deleted arm stays deleted.
- **DALC bounded-path escape converts irradiance to radiance (regression guard, #2244/`c4cb2614`).** `pathEnvironmentRadiance` (`lighting.glsl`) samples `sampleDalcCube` — an authored *irradiance* cube — and must divide by `PI` before returning it as environment radiance for a path escaping the TLAS (`sampleDalcCube(rayDir) * (1.0 / PI)`); the raw, unconverted sample over-brightens Skyrim DALC-lit GI misses by π×. Regression: `bounded_path_converts_dalc_irradiance_to_environment_radiance` (`shader_contract_tests.rs`) reverting to a bare `sampleDalcCube` return.
...
- Interior XCLL emits an ordinary directional `GpuLight` (type `2.0`) with `VisibilityMask::FULL`, so it follows the same cone-sampled, visibility-tested shadow path as an exterior directional. `directional_source_contract_tests` in `byroredux/src/render/lights.rs` pins the source selection and upload contract; disocclusion single-sample fallback is not black.
...
**Checklist**:
- `weather_system` advances game time monotonically (`GameTimeRes::tick`); sun arc from CLMT TNAM hours (not hardcoded); TOD color easing matches legacy; weather fade blends AFTER the TOD lookup (`WeatherTransitionRes`); all 4 cloud layers active with world-XY parallax scaled by TOD wind.
- Sky gradient (zenith→horizon) from active TOD palette in the non-RT miss-fill, consistent with the GI miss "sky fill" (Dim 2); fog applied to direct only (Dim 8). Interior XCLL retains its 0.6 source calibration and authored directional fade, but uploads as an ordinary directional `GpuLight` (type `2.0`) with `VisibilityMask::FULL`; `directional_source_contract_tests` in `byroredux/src/render/lights.rs` is the regression pin.
...
- **Game clock survives worldspace transitions (regression guard, `7a851ab9`).** `GameTimeRes` is the single persistent clock driving TOD (day + hour + rate, save/load round-tripped); `apply_worldspace_weather` reads the live hour via `bootstrap_game_hour` and re-installs the resource only through `ensure_game_time` (insert-if-absent), never an unconditional reset. Regression: a worldspace load re-seeding `GameTimeRes` unconditionally, snapping an in-progress session's hour/day back to the process default — pinned by `bootstrap_hour_prefers_the_persistent_live_clock` / `insert_procedural_fallback_resources_preserves_advanced_game_time` (`world_setup.rs`).
...
- `perturbNormal` default-on (#787/#788); `DBG_BYPASS_NORMAL_MAP` (0x10) runtime opt-out still recognized; the `DBG_*` catalog (35 entries as of 2026-08-29, not the 24 an older revision of this file quoted) pinned in lockstep (Dim 3).
...
- **Depth capture (#3308).** `crates/renderer/src/vulkan/context/depth_capture.rs` is a deliberate sibling of `screenshot.rs` — request flag set from outside, copy recorded inside the frame's command buffer, readback at the top of the *next* frame once the fence proves the GPU is done. Unlike the screenshot bridge it carries no owner tag and no capture generation (single consumer, the `depth.stats` command in `byroredux/src/commands/depth.rs`), so a stale result is simply an older frame's depth — that asymmetry is documented and intentional, not a missing guard. Audit it for the same fence-before-read discipline the screenshot path has, and for its staging buffer being destroyed on teardown.
- **GPU-timer brackets honour `_active` (regression guard, #2821).** Every reader of a timer bracket must consult the bracket's `_active` flag rather than assuming a recorded interval; a skipped pass leaves a bracket whose interval is meaningless. Paired with the FSR destroy-path leak closed in #2829 — check `frame_upscaler.rs`'s destroy actually releases what `create` allocated.
- `dispatches_skipped` is a **skin-coverage** counter (`skin_compute.rs`, incremented in `draw.rs` when the bone palette is unchanged), surfaced via console telemetry (NOT `mem.stats`, which does not exist — REN-LOW L-1) — NOT a `GpuPerFrameTimers` field. Issued dispatches = total − skipped (#1194).
...
**Checklist**:
- `canonical_light_animation_flags(game, source_flags)` is the per-game→shared-behavior boundary for animation (mirrors the NIFAL translate pattern, Dim 6) — **FO4 and FO76** mask raw LIGH flags to `FLICKER | PULSE` only, dropping raw bit `0x400` (the Shadow-Spotlight flag, NOT a slow-pulse animation); **Starfield masks to `0`** (its restructured `DAT2` subrecord has no named Flags field in SF1Edit, so no bit meaning is positively evidenced — #2251); every other game masks to the full `SHARED_LIGHT_ANIMATION_MASK` (`FLICKER | FLICKER_SLOW | PULSE | PULSE_SLOW`). It now has a deliberate sibling, `canonical_light_shadow_flags` (#2250 / REN-D22-01), applying the same match-arm pattern to `LIGHT_FLAG_SHADOW_MASK` (`0x400`/`0x800`/`0x1000`) — audit the two as a mirrored pair; a per-game divergence added to one and not the other is the drift to look for. Regression guards (`byroredux/src/systems/light_anim.rs`): `shadow_spotlight_bit_never_leaks_into_animation_on_any_game` (the consolidated, any-game successor to the FO4-only *fallout4_shadow_spotlight_is_not_slow_pulse*, now removed), `fallout4_real_flicker_and_pulse_map_to_shared_behavior`.
...
**Checklist**:
- **FFI safety** — `crates/fsr3-sys` is a real live FFI boundary (unlike `cxx-bridge`'s placeholder). Every `unsafe fn` needs a `# Safety` contract stating who owns the pointed-to memory and for how long. Cross-check with `/audit-safety` Dimension 1 rather than duplicating the finding.
- **Resource-state contract** — FSR requires specific image layouts on every input (color, depth, motion vectors, reactive mask, transparency & composition mask) and on the output. `BYRO_VALIDATION=1` validated this clean over 900 frames; a change to the frame tail can silently break it because layout errors are invisible to `cargo test`. Treat any barrier/layout edit here as needing a validation-layer run, not reasoning — this is exactly the "speculative Vulkan fix" trap.
- **Dispatch-failure fallback** — the fallback path exists and is telemetry-reported. Verify it actually renders (not a black frame) and that `BYRO_FSR_FORCE_DISPATCH_FAIL=1` still exercises it; a fallback that regressed to a hang or blank output is worse than no fallback.
- **Runtime preset switching** — `r.upscaler` / `--upscaler` switch presets live. Confirm resources are re-created (not reused at the wrong render resolution) and that the swap can't land mid-frame.
- **`native-aa` is expected to be a net loss** on every scene (−9 % … +4 %) — it reconstructs at full output resolution. It exists to separate reconstruction quality from upscaling quality. **Do not report its cost as a performance bug.**
- **Jitter / motion-vector agreement** — FSR consumes the same Halton jitter and motion vectors as TAA (Dim 13). Verify one jitter source, not two; a divergence shows up as smearing that is easy to misattribute to the denoiser (Dim 8).
- **Exposure** — `exposure.rs` feeds FSR's auto-exposure. Confirm the value handed to FSR matches what composite/tone-map uses, or reconstruction fights the tone mapper.
- **The FP32 permutation is unexercised** — it needs a GPU without `shaderFloat16` and the dev box has one. Carried scope, not a finding; note it as untested rather than reporting it as a defect.
- **Bench-harness stability** — `scripts/fsr-bench-matrix.sh` + `scripts/fsr_bench_report.py` must stay byte-stable across commits for cross-commit comparisons to mean anything. Flag any edit to either that wasn't itself re-benched.
