...
- Bloom added **before** ACES tone-map (HDR add); intensity constant is `BLOOM_INTENSITY` (0.15) in `shader_constants_data.rs` emitted into `include/shader_constants.glsl` — NOT hand-written in `composite.frag`; 5 down-mips + 4 up-mips is `BLOOM_MIP_COUNT` / `BLOOM_MIP_COUNT - 1` (`bloom.rs`); source is the un-tone-mapped HDR (NOT the TAA output — descriptor-binding regression pattern); disable path short-circuits both dispatch and composite addition.
