...
**Checklist**:
- `intern` produces stable `material_id`s within a frame; identical materials collapse to one entry. Over-cap returns id 0 + warn-once, surfaced via the `ctx.scratch` command (#7823eb59/#797 — NOT `mem.stats`/`mem`, neither of which exists; REN-LOW L-1/L-6). Per-frame SSBO sized to `min(intern_count, MAX_MATERIALS)`.
