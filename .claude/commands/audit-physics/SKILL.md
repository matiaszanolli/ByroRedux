...
- Open: #4134 (KCC/ground-probe capsules use a floor-only `.max(1e-3)` instead of `clamp_shape_extent`), #3477 (`collect_newcomers` rescans every collider row per tick; perf-owned). NIFAL-owned: #4408 (#4407 CLOSED — `726a2e930` fixed the alpha-tested mesh justification).
...
**Guards** (default lane): `config.rs::trimesh_flag_bits_match_rapier_definitions` (Rapier flag pin), `convert.rs::huge_finite_*_clamps_to_sane_ceiling` (extent clamp; release-only), `non_finite_cuboid_extent_clamps_instead_of_reaching_rapier` (release-only — the #4686 debug-lane twin `non_finite_extents_clamp_in_the_debug_lane_too` covers the default lane). Aim at what they cannot see:
...
- **Explosion recovery**: each substep snapshots ALL dynamic-body poses (not just active islands, so a freshly activated ragdoll's first solve is covered; check the per-substep `Vec` cost on a body-heavy exterior); `restore_invalid_dynamic_bodies` reverts any body that is non-finite or moved more than `MAX_DYNAMIC_SUBSTEP_DISPLACEMENT` (2,048 BU) in one substep, sleeps it, logs at error, and zeroes the accumulator. Verify the snapshot precedes every substep, the bound is a delta not a coordinate (far exteriors), and the zeroed accumulator does not eat the frame's wake. #4683: recovery is COUNTED (`recovery_counts()`, surfaced in `phys.stats`/`ragdoll.status` with live joint counts) — assert on the counter, not just post-recovery state. #4687: already-non-finite bodies are parked (not left awake), restored poses are propagated to colliders + the query pipeline same-frame, and EVERY invalidated articulation is detached, not just the first. Note: multibody (ragdoll) joints are DETACHED on restore by design (`remove_multibody_articulations`; `playable-vertical-slice.md` 1386-1390) — a joint-restoration claim is the pre-#4687 error. Tests: `world.rs::invalid_dynamic_body_reverts_to_its_last_valid_substep_pose`, `finite_but_impossible_dynamic_jump_reverts_to_its_last_valid_pose`, `recovery_counter_counts_a_real_substep_explosion`, `recovery_snapshot_index_tracks_dynamics_not_the_arena`, the three `#4687` edge tests.
...
- Writeback drives `GlobalTransform` from `body_pose` (not `Transform` — re-verified #4692); the Z-up→Y-up conversion happens upstream, not again here. FO4+ is blocked on the opaque `BhkSystemBinary`; `summarize_collision_authoring` must let the loader report "authored but opaque" rather than "no collision".
...
- The `LOCOMOTION_NPC_*` constants are DERIVED from their sources (#4689): KCC offset reads `ContactConfig::DEFAULT`, the step trio reads `CharacterController::HUMAN`, the capsule pair shares one const with `install_fallback_actor_collider`; pinned by `npc_locomotion_constants_match_their_sources` (which also restates the `> 2 * default_contact_skin_bu` invariant). Verify the derivation wasn't reverted to literals.
...
- **Two samplers must agree**: the dynamic path (`apply_buoyancy_with_scratch`) and the kinematic player's `player_water_state` in `character.rs`. Every water input added to one (plane flow, placed `WaterCurrentVolume` marker, wave height, damage) must exist in the other, and both COMPOSE plane + marker ADDITIVELY (#4691 — the player's old plane-wins `or_else` let a barrel feel two sources while the swimmer felt one; pinned by `player_water_state_falls_back_to_a_placed_current_volume`'s both-sources arm).
...
- Cost model (#4685/#4682 updated): the step advances the query pipeline INCREMENTALLY (`Some(&mut query_pipeline)` into `pipeline.step`; rapier 0.22 never full-rebuilds a handed pipeline), the post-loop full rebuild only runs on collider-dirty frames that step nothing, and the recovery snapshot reads a maintained dynamic-body index rather than the arena — the audit's 'budget from the rebuild' and 'snapshot walks the arena' attributions are both historical. `step_cost_rationale_is_scoped_to_history_and_names_the_real_cost_centre` pins the in-code comment; re-measure with real TriMesh-heavy content before citing a number.
