...
- **Schedule gating, all seven procedures**: each `active_package_is_*` (`crates/plugin/src/esm/records/misc/ai.rs`) picks the first package scheduled-active at `GameTimeRes.hour` — verify an NPC with a non-matching package active at the current hour (e.g. an `AtBar` schedule) does NOT get that procedure's Behavior marker for that hour. Since an NPC's active package is always a single winning `PackRecord`, the seven checks must be mutually exclusive per actor — verify `npc_spawn.rs`'s `if runs_*` chain never inserts two Behavior components on one actor.
