...
**Checklist**:
- `GpuMaterial` is exactly **300 bytes** (`gpu_material_size_is_260_bytes` asserts `300` at `material.rs:1157`). The test NAME is historical — deliberately kept for grep continuity across the audit corpus, NOT the asserted value (rationale at `material.rs:41-42`); the Rust doc comments and the GLSL mirror in `triangle.frag` are self-aware ("asserted value is 300"). Size history: 272 B → 260 B (#804 / R1-N4 dropped `avg_albedo_r/g/b`) → 296 B (#1249 Disney lobe `subsurface`/`sheen`/`sheen_tint`) → 300 B (#1250 +4 B for `anisotropic`, the GGX ax/ay aspect driver). Any field add/remove must update the Rust struct, the GLSL `struct GpuMaterial` (in `triangle.frag` — the only shader that declares it), and the `gpu_material_field_offsets_match_shader_contract` offsets in lockstep
...
**Entry points**: `crates/renderer/shaders/triangle.frag` (Disney lobe functions + gate sites; flag `#define`s at `triangle.frag:204-208` are the source of truth — `MAT_FLAG_*` bits 5-9 are NOT in `shader_constants_data.rs` today), `crates/renderer/src/vulkan/material.rs` (Disney material preset table from #1251). **Naming note (Stage 3 refactor `ae364e29`)**: the `BGSM_` prefix was dropped from these flags; the `TRANSLUCENCY` / `MODEL_SPACE_NORMALS` / `TRANSLUCENCY_THICK_OBJECT` / `TRANSLUCENCY_MIX_ALBEDO` variants all dropped `BGSM_`. Older audit reports cite the old names.
**Checklist**:
- **Gate is `MAT_FLAG_PBR_BSDF` (bit 5) only** — verify the consumer sites in `triangle.frag` ALL test this single flag (no path lights FNV / FO3 / Skyrim legacy materials with the Disney lobe). Gate-site grep target: `if ((mat.materialFlags & MAT_FLAG_PBR_BSDF) != 0u)`. **Current count: 2** (was 3 in older reports) — `:2610` (Disney diffuse consumer in the main `diffuseBrdf` branch) and `:2861` (second Disney diffuse consumer in the deferred-specular path); the F0-derivation gate is no longer flag-gated at a separate site (F0 is selected via `dielectricF0FromIor(mat.ior)` at `:1760` unconditionally). Drift here is a per-game lighting regression — FNV's Lambert-only world would suddenly get Burley retro-reflection
...
- **#1147 Phase 2b gate sibling** (`fe22e64c`, 2026-05-23): PBR / SSS / model-space-normals are ALSO gated separately in `triangle.frag` (NOT all behind `MAT_FLAG_PBR_BSDF`). Verify:
...
### Dimension 22: NIFAL Material Canonical Translation (2026-05-28, `3ce98db8`)
...
### Dimension 23: Debug Overlay & GPU Telemetry Passes (egui `06af4698` 2026-05-24, GPU timers #1194 `e5774b19` 2026-05-21)
