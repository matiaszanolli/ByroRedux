...
**Entry points**: `crates/renderer/shaders/triangle.frag` (Disney lobe functions + 4 gate sites; flag `#define`s at `triangle.frag:183-187` are the source of truth — `MAT_FLAG_*` bits 5-9 are NOT in `shader_constants_data.rs` today), `crates/renderer/src/vulkan/material.rs` (Disney material preset table from #1251). **Naming note (Stage 3 refactor `ae364e29`)**: the `BGSM_` prefix was dropped from these flags; `MAT_FLAG_PBR_BSDF` became `MAT_FLAG_PBR_BSDF`, and the `TRANSLUCENCY` / `MODEL_SPACE_NORMALS` / `TRANSLUCENCY_THICK_OBJECT` / `TRANSLUCENCY_MIX_ALBEDO` variants similarly dropped `BGSM_`. Older audit reports cite the old names.
**Checklist**:
- **Gate is `MAT_FLAG_PBR_BSDF` (bit 5) only** — verify the 3 consumer sites in `triangle.frag` ALL test this single flag (no path lights FNV / FO3 / Skyrim legacy materials with the Disney lobe). Gate-site grep target: `if ((mat.materialFlags & MAT_FLAG_PBR_BSDF) != 0u)`. Expected count: 3 — `:1652` (F0 derivation: authored specular for PBR vs metallic-workflow F0 for legacy), `:2447` (Disney diffuse consumer in the main `diffuseBrdf` branch), `:2669` (second Disney diffuse consumer in the deferred-specular path). Drift here is a per-game lighting regression — FNV's Lambert-only world would suddenly get Burley retro-reflection
