...
- Permanent diagnostic bit catalog (declared in `crates/renderer/shaders/triangle.frag`, see the `DBG_*` const block): `DBG_BYPASS_POM = 0x1`, `DBG_BYPASS_DETAIL = 0x2`, `DBG_VIZ_NORMALS = 0x4`, `DBG_VIZ_TANGENT = 0x8`, `DBG_BYPASS_NORMAL_MAP = 0x10`, `DBG_RESERVED_20 = 0x20` (formerly `DBG_FORCE_NORMAL_MAP`, no-op post-#1035), `DBG_VIZ_RENDER_LAYER = 0x40`, `DBG_VIZ_GLASS_PASSTHRU = 0x80`, `DBG_DISABLE_SPECULAR_AA = 0x100`, `DBG_DISABLE_HALF_LAMBERT_FILL = 0x200`. The Rust mirror lives in `crates/renderer/src/shader_constants_data.rs::DBG_*` — both are pinned in lockstep by `crates/renderer/src/shader_constants.rs::tests::triangle_frag_dbg_bits_match` (post-#1119). Audit for drift: any added bit must not collide; any dropped bit must not orphan shader code
