...
**Checklist**:
- `GpuMaterial` is now exactly **300 bytes** (the `gpu_material_size_is_260_bytes` test asserts `300` at `material.rs:1157` — the test name and the `material.rs:38`/`:79` doc comments are STALE-NAMED "260" and were deliberately left so a future size shift updates them in lockstep with the assertion; the `triangle.frag:174` comment says "296 B"). Size history: 272 B → 260 B (#804 / R1-N4 dropped `avg_albedo_r/g/b`) → 296 B (Disney lobe `subsurface`/`sheen`/`sheen_tint`) → 300 B (#1250 +4 B for `anisotropic`, the GGX ax/ay aspect driver). Any field add/remove must update both Rust + GLSL `struct GpuMaterial` in lockstep — flag the misleading "260" test name + the 260/296 B doc comments as drift candidates
