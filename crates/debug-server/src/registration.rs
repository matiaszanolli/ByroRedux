...
fn register_component<T>(registry: &mut ComponentRegistry, name: &'static str, field_names: Vec<&'static str>)
where
    T: Component + Serialize + DeserializeOwned,
{
    let desc = ComponentDescriptor {
        ...
        set_json: Box::new(|world_any: &dyn std::any::Any, entity: u32, value: serde_json::Value| {
            ...
            Err("whole-component replacement not yet supported; use field-level set".to_string())
        }),
        ...
        set_field: Box::new(|world_any: &dyn std::any::Any, entity: u32, field: &str, value: serde_json::Value| {
            ...
            let mut json = serde_json::to_value(&*comp)
...
pub fn register_all(registry: &mut ComponentRegistry) {
    ...
    register_component::<Camera>(
        registry,
        "Camera",
        vec!["fov_y", "near", "far", "aspect"],
    );
    register_component::<LightSource>(
        registry,
        "LightSource",
        vec!["radius", "color", "flags"],
    );
    register_component::<Material>(
        registry,
        "Material",
        vec![
            "emissive_color", "emissive_mult", "specular_color", "specular_strength",
            "glossiness", "uv_offset", "uv_scale", "alpha", "env_map_scale",
            "normal_map", "texture_path", "glow_map", "detail_map", "gloss_map",
            "dark_map", "vertex_color_mode", "alpha_test", "alpha_threshold",
            "alpha_test_func",
        ],
    );
    register_component::<LocalBound>(
        registry,
        "LocalBound",
        vec!["center", "radius"],
    );
    register_component::<WorldBound>(
        registry,
        "WorldBound",
        vec!["center", "radius"],
    );
    ...
    register_component::<MeshHandle>(
        registry,
        "MeshHandle",
        vec!["0"],
    );
    register_component::<TextureHandle>(
        registry,
        "TextureHandle",
        vec!["0"],
    );
    register_component::<BSXFlags>(
        registry,
        "BSXFlags",
        vec!["0"],
    );
    register_component::<BSBound>(
        registry,
        "BSBound",
        vec!["center", "half_extents"],
    );
    register_component::<AnimatedVisibility>(
        registry,
        "AnimatedVisibility",
        vec!["0"],
    );
    register_component::<AnimatedAlpha>(
        registry,
        "AnimatedAlpha",
        vec!["0"],
    );
    register_component::<AnimatedColor>(
        registry,
        "AnimatedColor",
        vec!["0"],
    );
