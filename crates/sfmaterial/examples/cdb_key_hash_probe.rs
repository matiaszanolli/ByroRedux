...
//!
//! This is the in-tree reproduction of the spike's key result — the
//! rotated-column / function identification that established the 3,032 /
//! 3,084 (98.3%) key match `docs/audits/SF_CDB_PHASE2_SPIKE_2026-08-29.md`
//! rests on (#4435: the probe was originally a `_tmp_`-prefixed scratch
//! example deleted four days after the spike was written; it is restored
//! here as a named diagnostic, following the `sf_smoke` convention, so the
//! derivation stays re-runnable).
//!
//! Usage: `cargo run --release -p byroredux-sfmaterial --example
//! cdb_key_hash_probe -- <paths.txt>` — one material path per line, as
//! dumped off real Starfield NIFs (the original producer was
//! `crates/nif/examples/_tmp_sf_matpath_dump.rs`, recoverable via
//! `git show a823c13a1^:crates/nif/examples/_tmp_sf_matpath_dump.rs`), so
//! the test is against strings the engine will actually be asked to
//! resolve. (Split across two examples so neither crate needs a new
...
fn main() {
    ...
    for inst in &cdb.instances {
        ...
        if o.class_name != "BSMaterial::Internal::CompiledDB" {
            continue;
        }
        let Some(Value::Map(pairs)) = o.fields.get("HashMap") else {
            continue;
        };
    ...
    println!(
        "CDB keys: {} pairs, col Dir={} distinct, col Ext={} distinct\n",
        keys.len(),
        col_a.len(),
        col_b.len()
    );
    ...
    let list = std::env::args()
        .nth(1)
        .expect("usage: cdb_key_hash_probe <paths.txt>");
    ...
    let variants: Vec<(&str, fn(&str) -> (String, String))> = vec![
        ...
        ("dir+stem, forward slash, no data prefix", |p| {
            split(p, '/', false)
        }),
        ("dir+stem, backslash, data\\ prefix", |p| {
            split(p, '\\', true)
        }),
    ];
    ...
    for (vname, vf) in &variants {
        for (hname, hf) in &hashes {
            ...
            for p in &paths {
                ...
                if keys.contains(&(hs, hd)) {
                    hit_ab += 1;
                } // Dir col = stem
                if keys.contains(&(hd, hs)) {
                    hit_ba += 1;
                } // Dir col = dir
            }
    ...
    for (e, n) in &hit_by_ext {
        println!("     HIT  .{e}: {n}");
    }
    for (e, v) in &miss_by_ext {
        println!("     MISS .{e}: {}", v.len());
        for x in v.iter().take(6) {
            println!("            {x}");
        }
    }
    ...
    for p in paths.iter().take(500) {
        ...
        if dir_col.contains(&crc32(s.as_bytes(), 0, 0)) {
            stem_in_dircol += 1
        }
    }
...
fn split(p: &str, sep: char, data_prefix: bool) -> (String, String) {
    ...
    let stem_full = match t.rsplit_once(sep) {
        Some((_, f)) => f,
        None => &t,
    };
    let dir = match t.rsplit_once(sep) {
        Some((d, _)) => d.to_string(),
        None => String::new(),
    };
    let stem = match stem_full.rsplit_once('.') {
        Some((s, _)) => s.to_string(),
        None => stem_full.to_string(),
    };
    (dir, stem)
}
