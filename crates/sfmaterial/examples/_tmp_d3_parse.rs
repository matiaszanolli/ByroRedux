...
fn walk(v: &Value, counts: &mut BTreeMap<String, usize>, depth: usize, maxd: usize) {
    match v {
        ...
        Value::Map(p) => {
            if depth < maxd {
                for (k, vv) in p {
                    walk(k, counts, depth + 1, maxd);
                    walk(vv, counts, depth + 1, maxd);
                }
            }
        }
        Value::Ref(r) => {
            if depth < maxd {
                walk(&r.inner, counts, depth + 1, maxd);
            }
        }
        _ => {}
    }
...
fn main() {
    ...
    let cdb = match ComponentDatabaseFile::parse(&bytes) {
        Ok(c) => c,
        Err(e) => {
    ...
    eprintln!(
        "parsed in {:?}: {} classes, {} top-level instances",
        t0.elapsed(),
        cdb.classes.len(),
        cdb.instances.len()
    );
    ...
    for c in cdb
        .classes
        .iter()
        .filter(|c| c.name.to_lowercase().contains("material"))
        .take(60)
    {
        eprintln!(
            "  {} (type_id {:#x}, {} fields, flags {:?})",
            c.name,
            c.type_id,
            c.fields.len(),
            c.flags
