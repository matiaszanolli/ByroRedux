...
fn main() {
    ...
    if which == "probe_no_strt" {
        ...
        println!("[probe_no_strt] probe_header = {:?}", ComponentDatabaseFile::probe_header(&b));
        println!("[probe_no_strt] peek_magic  = {}", ComponentDatabaseFile::peek_magic(&b));
        println!("[probe_no_strt] full parse  = {:?}", ComponentDatabaseFile::parse(&b).is_ok());
    }

    if which == "probe_truncated" {
        // Claim 1 chunk (i.e. BETH only) -> chunk table is empty; probe passes.
        let b = hdr(1);
        println!("[probe_truncated] probe_header = {:?}", ComponentDatabaseFile::probe_header(&b));
    }
}
