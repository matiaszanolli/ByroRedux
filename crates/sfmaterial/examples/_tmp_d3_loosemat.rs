...
fn main() {
    ...
    for f in ["qog-pawnshop - main.ba2", "avontechshipyards - main.ba2", "sp2_factionrequisitionkiosks - main.ba2"] {
        let a = Ba2Archive::open(format!("{d}{f}")).unwrap();
        let mats: Vec<String> = a.list_files().iter().filter(|p| p.to_ascii_lowercase().ends_with(".mat")).map(|s| s.to_string()).collect();
