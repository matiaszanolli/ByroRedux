...
/// 7. FixedBytes(4) (`13013`, 4 payload bytes of `0xAB`)
/// 8. String (`2000`, length-prefix 18 + ASCII `trees/oak/bark.dds`)
/// 9. ArrayBytes{stride=32} (`10002`, count=4 + 128 bytes `0xDE 0xAD 0xBE 0xEF` ×32)
/// 10. ArrayBytes{stride=32} (`10003`, count=2 + 64 bytes)
...
fn build_synthetic_spt() -> Vec<u8> {
    ...
    // 13013 — FixedBytes(4) of 0xAB (#4122 corrected the 7-byte reading).
    bytes.extend_from_slice(&13013u32.to_le_bytes());
    bytes.extend_from_slice(&[0xABu8; 4]);
    ...
    // 10002 — ArrayBytes{stride=32}, count=4 (#4122 corrected stride 1
    // to 32: 8 f32s per entry, byte-verified against the corpus).
    bytes.extend_from_slice(&10002u32.to_le_bytes());
    bytes.extend_from_slice(&4u32.to_le_bytes());
    for _ in 0..32 {
        bytes.extend_from_slice(&[0xDE, 0xAD, 0xBE, 0xEF]);
    }
    // 10003 — ArrayBytes{stride=32}, count=2 (#4122 corrected stride 8
    // to 32: same 8-f32 entry shape as 10002).
    bytes.extend_from_slice(&10003u32.to_le_bytes());
    bytes.extend_from_slice(&2u32.to_le_bytes());
    for _ in 0..2 {
        bytes.extend_from_slice(&[
            0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, // stride-32 entry bytes 0..8
            0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, // stride-32 entry bytes 8..16
            0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, // stride-32 entry bytes 16..24
            0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, // stride-32 entry bytes 24..32
        ]);
    }
...
const PINNED_BYTES: &[u8] = &[
    ...
    // Tag 13013 — FixedBytes(4) of 0xAB (#4122).
    0xD5, 0x32, 0x00, 0x00, 0xAB, 0xAB, 0xAB, 0xAB,
    ...
    // Tag 10002 — ArrayBytes(stride=32), count=4, payload
    // {DE AD BE EF} ×32 (#4122).
    0x12, 0x27, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, //
    ...
    0xDE, 0xAD, 0xBE, 0xEF, 0xDE, 0xAD, 0xBE, 0xEF, 0xDE, 0xAD, 0xBE, 0xEF, 0xDE, 0xAD, 0xBE, 0xEF,
    0xDE, 0xAD, 0xBE, 0xEF, 0xDE, 0xAD, 0xBE, 0xEF, 0xDE, 0xAD, 0xBE, 0xEF, 0xDE, 0xAD, 0xBE, 0xEF,
    // Tag 10003 — ArrayBytes(stride=32), count=2, two 32-byte entries
    // (the 32-byte entry pattern repeated ×4) (#4122).
    0x13, 0x27, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, //
...
fn parser_decodes_every_dispatch_arm_against_pinned_fixture() {
    ...
    match value {
        ...
        _ => panic!("tag 13013 must decode as Fixed(4)"),
    }
    ...
    match value {
        SptValue::ArrayBytes {
            ...
            assert!(bytes.chunks(4).all(|c| c == [0xDE, 0xAD, 0xBE, 0xEF]));
        }
        _ => panic!("tag 10002 must decode as ArrayBytes(stride=32)"),
    }
    ...
    match value {
        ...
        _ => panic!("tag 10003 must decode as ArrayBytes(stride=32)"),
    }
}
