//! TOML loader for the [`GameProfileRegistry`].
//!
//! The registry resource itself lives in `byroredux-core` (so the debug-server
//! can read it without depending on this crate); this module parses
//! `assets/debug_profiles.toml`, the per-user override
//! `~/.byroredux/profiles.toml`, and the `[roots]` table detection writes,
//! into core's [`GameProfileEntry`] values. Every file missing = an empty
//! registry, never an error.
//!
//! Lives here rather than in the engine binary because the launcher needs the
//! *same* registry the engine will launch from: two readers of one file would
//! be free to disagree about which archives a game needs, which is exactly the
//! class of bug a pre-launch validator exists to catch (see
//! `docs/engine/launcher.md` §12 Q6).
...
/// Apply `[roots]` entries over an already-merged registry, replacing only
/// each entry's `root`.
///
/// Split out of [`load_default`] because that function reads the real user's
/// home directory; this is the part with the actual rule in it. An override
/// naming no known profile is reported and skipped individually, so one stale
/// key cannot cost the user every other detected path.
fn apply_root_overrides(
    roots: BTreeMap<String, String>,
    out: &mut BTreeMap<String, GameProfileEntry>,
) {
    for (key, root) in roots {
        match out.get_mut(&key) {
            Some(entry) => entry.root = root,
            None => eprintln!(
                "[roots] {key:?} names no known profile; ignoring (known keys: {:?})",
...
pub fn load_default() -> GameProfileRegistry {
    ...
    // Root overrides from the launcher's detection pass (`[roots]` in the
    // per-user file). Applied *after* both merges and touching only `root`,
    // which is the whole reason detection writes a narrow table instead of a
...
#[cfg(test)]
mod root_override_tests {
    use super::*;

    fn registry() -> BTreeMap<String, GameProfileEntry> {
        BTreeMap::from([(
            "fnv".to_owned(),
            GameProfileEntry {
                name: "Fallout New Vegas".into(),
                subdir: "Fallout New Vegas/Data".into(),
                esm: "FalloutNV.esm".into(),
                default_bsas: vec!["Fallout - Meshes.bsa".into()],
                ..GameProfileEntry::default()
            },
        )])
    }

    /// The reason detection writes `[roots]` rather than a full profile block:
    /// the override sets the path and leaves the curated archive lists intact,
    /// so a later engine update that adds an archive still takes effect.
    #[test]
    fn an_override_sets_the_root_and_preserves_everything_else() {
        let mut out = registry();
        apply_root_overrides(
            BTreeMap::from([("fnv".to_owned(), "/detected/FNV/Data".to_owned())]),
            &mut out,
        );
        let entry = &out["fnv"];
        assert_eq!(entry.root, "/detected/FNV/Data");
        assert_eq!(entry.esm, "FalloutNV.esm");
        assert_eq!(entry.default_bsas, vec!["Fallout - Meshes.bsa".to_owned()]);
        assert_eq!(entry.subdir, "Fallout New Vegas/Data");
    }

    /// `resolve_profile_root` prefers an explicit `root`, so an override is
    /// what actually redirects a `--game <key>` launch off the default games
    /// root — the point of the whole write-back.
    #[test]
    fn an_override_redirects_profile_root_resolution() {
        let mut out = registry();
        apply_root_overrides(
            BTreeMap::from([("fnv".to_owned(), "/detected/FNV/Data".to_owned())]),
            &mut out,
        ...
        assert_eq!(
            resolve_profile_root(&out["fnv"], Path::new("/wrong/games/root")),
            PathBuf::from("/detected/FNV/Data")
        );
    }

    /// One stale key must not cost the user their other detected paths.
    #[test]
    fn an_unknown_key_is_skipped_without_disturbing_the_rest() {
        let mut out = registry();
        apply_root_overrides(
            BTreeMap::from([
                ("morrowind".to_owned(), "/nope".to_owned()),
                ("fnv".to_owned(), "/detected/FNV/Data".to_owned()),
            ]),
            &mut out,
        );
        assert_eq!(out.len(), 1);
        assert_eq!(out["fnv"].root, "/detected/FNV/Data");
    }
}
