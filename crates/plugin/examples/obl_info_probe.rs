...
struct WalkStats {
    dial_count: usize,
    info_count: usize,
    info_with_trdt: usize,
    info_with_qsti: usize,
    trdt_len_hist: std::collections::BTreeMap<u32, usize>,
    byte0_hist: std::collections::BTreeMap<u8, usize>,
    emotype_hist: std::collections::BTreeMap<u32, usize>,
    samples: usize,
}
...
fn main() {
    ...
    fn walk(b: &[u8], start: usize, end: usize, stats: &mut WalkStats) {
        let mut p = start;
        let top = start == 0;
        while p + 24 <= end {
            let ty = &b[p..p + 4];
            ...
            if ty == b"GRUP" {
                ...
                walk(b, p + 24, p + group_size, stats);
                p += group_size;
            } else {
                ...
                if (ty == b"INFO" || ty == b"DIAL") && !compressed {
                    ...
                    while s + 6 <= body_end {
                        ...
                        if ty == b"INFO" && st == b"TRDT" {
                            has_trdt = true;
                            *stats.trdt_len_hist.entry(ssz as u32).or_insert(0) += 1;
                            if ssz >= 1 {
                                *stats.byte0_hist.entry(b[sd]).or_insert(0) += 1;
                            }
                            if ssz >= 4 {
                                *stats.emotype_hist.entry(rd_u32(b, sd)).or_insert(0) += 1;
                            }
                            if stats.samples < 8 {
                                let n = ssz.min(16);
                                let hex: Vec<String> =
                    ...
                    if ty == b"INFO" {
                        stats.info_count += 1;
                        if has_trdt {
                            stats.info_with_trdt += 1;
                        }
                        if has_qsti {
                            stats.info_with_qsti += 1;
                        }
                    } else {
                        stats.dial_count += 1;
                    }
    ...
    walk(&data, 0, data.len(), &mut stats);

    eprintln!("\n=== Oblivion.esm INFO/DIAL raw probe ===");
    eprintln!("DIAL records         : {}", stats.dial_count);
    eprintln!("INFO records         : {}", stats.info_count);
    eprintln!("INFO with TRDT       : {}", stats.info_with_trdt);
    eprintln!("INFO with QSTI       : {}", stats.info_with_qsti);
    eprintln!("TRDT length histogram: {:?}", stats.trdt_len_hist);
    eprintln!("TRDT byte[0] histogram (what code reads as response_type):");
    for (k, v) in &stats.byte0_hist {
        eprintln!("    byte0 = {} ({:#04x}) -> {} INFOs", k, k, v);
    }
    eprintln!("TRDT EmotionType(u32@0) histogram (top 12):");
    let mut emo: Vec<_> = stats.emotype_hist.iter().collect();
