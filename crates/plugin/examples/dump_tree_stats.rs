...
fn dump_one(path: &str, label: &str) {
    ...
    let modb_vals: Vec<f32> = trees
        .iter()
        .filter(|t| t.bound_radius > 0.0)
    ...
    let mean = if modb_vals.is_empty() {
        0.0
    } else {
        modb_vals.iter().sum::<f32>() / modb_vals.len() as f32
    };
    ...
    if !modb_vals.is_empty() {
        ...
        for t in trees.iter().filter(|t| t.bound_radius > 0.0).take(5) {
            eprintln!(
                "    {} | MODB={:.2} | OBND={:?}",
                t.editor_id, t.bound_radius, t.bounds
            );
        }
    }
}
fn main() {
    ...
    dump_one(
        "/mnt/data/SteamLibrary/steamapps/common/Fallout 3 goty/Data/Fallout3.esm",
        "FO3",
    );
    dump_one(
