...
fn main() -> anyhow::Result<()> {
    ...
    let esm = args
        .next()
        .expect("usage: ESM WORLD gx_lo gx_hi gy_lo gy_hi");
    ...
    for (wkey, cells) in &index.cells.exterior_cells {
        ...
        for g in &grids {
            ...
            for r in &cell.references {
                ...
                if ml.contains("roof")
                    || ml.contains("thatch")
                    || ml.contains("architecture")
                    || ml.contains("riverwood")
                    || ml.contains("house")
                    || ml.contains("\\rw")
                    || ml.contains("farm")
                    || ml.contains("mill")
        ...
        println!(
            "{:>5} {:>5} {:>10} {:>10} {:>10} {:>10} {:>10}  base_mesh",
            "gx", "gy", "form", "base", "x", "y", "z"
        );
        for (gx, gy, fid, base, p, mesh, known) in roofs.iter().take(40) {
            println!(
                "{:>5} {:>5} {:>10X} {:>10X} {:>10.0} {:>10.0} {:>10.0}  {}{}",
                gx,
                gy,
                fid,
                base,
                ...
                p[2],
                mesh,
                if *known { "" } else { " [MISSING-STAT]" }
            );
        }

        println!("\n--- TOP 25 highest-Z REFRs (any mesh) ---");
        println!(
            "{:>5} {:>5} {:>10} {:>10} {:>10}  base_mesh",
            "gx", "gy", "form", "base", "z"
        );
