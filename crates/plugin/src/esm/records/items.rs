...
pub enum ItemKind {
    ...
    Armor {
        /// Body parts covered (bitfield, FNV uses bit positions for biped slots).
        biped_flags: u32,
        /// Damage Threshold (FNV) — flat damage absorbed.
        dt: f32,
        /// Damage Resistance (older games) — percentage absorbed.
        dr: u32,
        /// Health (used for condition/durability).
        health: u32,
        /// Equip slot mask.
        slot_mask: u16,
    },
...
pub fn parse_weap(form_id: u32, subs: &[SubRecord]) -> ItemRecord {
    ...
    for sub in subs {
        match &sub.sub_type {
            // FNV WEAP DATA: value(i32), health(i32), weight(f32), damage(i16),
            // clip(i16) — the simple stats block.
            ...
            // DNAM is the big weapon stats blob — many fields, version-dependent
            // layout. We pull a small set of well-known offsets used in FNV.
            // FNV/FO3 DNAM layout (selected fields):
            //   0  ulong   anim type
            //   1  byte    anim multiplier
            //   2  ulong   reach
            //   ...
            // The full schema is huge; for this milestone we extract the values
            // game systems actually consume.
            b"DNAM" if sub.data.len() >= 24 => {
            ...
            b"CRDT" if sub.data.len() >= 8 => {
                // Critical data: chance(u16), unused(u16), mult(f32)
...
pub fn parse_armo(form_id: u32, subs: &[SubRecord]) -> ItemRecord {
    let mut common = CommonItemFields::from_subs(subs);
...
pub fn parse_ammo(form_id: u32, subs: &[SubRecord]) -> ItemRecord {
    let mut common = CommonItemFields::from_subs(subs);
    ...
    for sub in subs {
        match &sub.sub_type {
            // DATA (FNV AMMO): speed(f32), flags(u8), pad(u8)x3, value(i32),
            // clipRounds(u8)
            b"DATA" if sub.data.len() >= 13 => {
            ...
            // DAT2 (FNV AMMO): projPerShot(u32), proj(formID), weight(f32),
            // consumedAmmo(formID), consumedPercentage(f32)
            b"DAT2" if sub.data.len() >= 16 => {
...
mod tests {
    ...
    fn weap_extracts_common_and_kind_fields() {
        ...
        let item = parse_weap(0x100, &subs);
    ...
    fn armo_extracts_dt_and_biped_flags() {
        ...
        let item = parse_armo(0x200, &subs);
        match item.kind {
            ItemKind::Armor {
                biped_flags,
                dt,
