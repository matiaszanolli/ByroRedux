//! `parse_esm` and its load-order-aware sibling — the single-pass GRUP
//! walker behind every ESM read.
//!
//! Split out of `records/mod.rs` under #4219, which had grown into a
//! re-export barrel with a 435-line parsing entry point bolted on: the
//! setup, the 19-arm top-level GRUP dispatch and the post-walk assembly
//! all in one body, in a file whose other job is to be a table of
//! contents. The barrel is now only a barrel, and this module is only the
//! walk. Nothing moved but the text — `mod.rs` re-exports both entry
//! points, so every caller's path is unchanged.

use super::{
    common, dispatch_actor, dispatch_container, dispatch_global, dispatch_items,
    dispatch_misc_gameplay_a, dispatch_misc_gameplay_b, dispatch_misc_stub,
    dispatch_world_placement, EsmIndex, MaterialSwapRecord, MovableStaticRecord, PkinRecord,
    ScolRecord,
};
use crate::esm::cell::support::{
    parse_ltex_group, parse_movs_group, parse_mswp_group, parse_pkin_group, parse_scol_group,
    parse_txst_group,
};
use crate::esm::cell::walkers::parse_cell_group;
use crate::esm::cell::wrld::parse_wrld_group;
use crate::esm::cell::{CellData, EsmCellIndex, StaticObject, TextureSet};
use crate::esm::reader::{EsmReader, FormIdRemap, GameKind};
...
pub(crate) fn character_rules_profile(game: GameKind, hedr_version: f32) -> CharacterRulesProfile {
    match game {
...
pub fn parse_esm_with_load_order(data: &[u8], remap: Option<FormIdRemap>) -> Result<EsmIndex> {
    ...
    let mut worldspaces: HashMap<String, crate::esm::cell::WorldspaceRecord> = HashMap::new();
