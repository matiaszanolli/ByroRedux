...
impl VulkanContext {
    ...
    pub fn draw_frame(
        ...
        // ── Depth pre-pass (#779) ───────────────────────────────────
        //
        // Populates depth with alpha-test discards correctly so the
        // main pass can `loadOp = LOAD` the depth and early-Z safely
        // (`layout(early_fragment_tests) in;` on triangle.frag).
        // Without this, the rectangular depth footprint of alpha-
        // tested foliage / fences / hair would block geometry behind
        // their transparent texels, producing visible "ghost
        // rectangle" artifacts. See D1-M3 / PERF-N6 in the 2026-04-20
        // and 2026-05-01 audits.
        //
        // The pre-pass iterates the same batches as the main pass but
        // skips Blended ones (they don't write depth). Per-mesh
        // dispatch (cmd_draw_indexed) is used uniformly — the indirect
        // amortization that the main pass uses optimizes per-call host
        // cost, but the depth-only pass has zero shader work past
        // alpha-test discard, so per-call cost dominates only for
        // huge batch counts.
        unsafe {
            let prepass_clear = [vk::ClearValue {
                depth_stencil: vk::ClearDepthStencilValue {
                    depth: 1.0,
                    stencil: 0,
                },
            }];
            let prepass_begin = vk::RenderPassBeginInfo::default()
                .render_pass(self.depth_prepass_render_pass)
                .framebuffer(self.depth_prepass_framebuffers[frame])
                .render_area(vk::Rect2D {
                    offset: vk::Offset2D { x: 0, y: 0 },
                    extent: self.swapchain_state.extent,
                })
                .clear_values(&prepass_clear);
            self.device
                .cmd_begin_render_pass(cmd, &prepass_begin, vk::SubpassContents::INLINE);
            ...
            // Same descriptor sets as the main pass (set 0 = bindless
            // textures for alpha-test sampling, set 1 = scene/material
            // /instance SSBOs). The prepass pipeline shares the
            // triangle pipeline layout for exactly this reuse.
            ...
            // Bind the global VB/IB once for all prepass draws —
            // identical to the main pass's #294 amortization.
            let prepass_global_bound = if let (Some(gvb), Some(gib)) = (
            ...
            // Track dynamic state to elide redundant cmd_set_* calls.
            // Same defaults as the main pass loop so the first batch
            // emits explicit values regardless of NIF state.
            let mut prepass_last_two_sided: Option<bool> = None;
            let mut prepass_last_is_decal = false;
            let mut prepass_last_z_test = true;
            let mut prepass_last_z_write = true;
            let mut prepass_last_z_function: u8 = u8::MAX;
            // Initial depth-bias to zero — matches main-pass behaviour.
            self.device.cmd_set_depth_bias(cmd, 0.0, 0.0, 0.0);
            self.device.cmd_set_cull_mode(cmd, vk::CullModeFlags::BACK);
            // Bind the BACK-cull prepass pipeline as the default; the
            // loop swaps to NONE-cull when the first two-sided batch
            // arrives. Initial bind here so the first non-two-sided
            // batch doesn't redundantly rebind.
            let mut prepass_pipeline_two_sided = false;
            ...
            for batch in &batches {
                // Skip blend draws — they don't write depth in the
                // main pass either (depth_write=false per
                // create_blend_pipeline at pipeline.rs).
                let two_sided = match batch.pipeline_key {
                    PipelineKey::Blended { .. } => continue,
                    PipelineKey::Opaque { two_sided } => two_sided,
                };

                // Honour z_write — sky domes / glow halos with
                // z_write=0 must skip depth write here too, otherwise
                // they'd stamp depth that the main pass's early-Z
                // would then incorrectly cull subsequent geometry
                // against.
                if !batch.z_write {
                    continue;
                }

                if prepass_last_two_sided != Some(two_sided) {
                    let pipe = if two_sided {
                        self.depth_prepass_pipeline_two_sided
                    } else {
                        self.depth_prepass_pipeline
                    };
                    if prepass_pipeline_two_sided != two_sided {
                        self.device.cmd_bind_pipeline(
                            cmd,
                            vk::PipelineBindPoint::GRAPHICS,
                            pipe,
                        );
                        prepass_pipeline_two_sided = two_sided;
                    }
                    self.device.cmd_set_cull_mode(
                        cmd,
                        if two_sided {
                    ...
                    prepass_last_two_sided = Some(two_sided);
                }

                if batch.is_decal != prepass_last_is_decal {
                    ...
                    prepass_last_is_decal = batch.is_decal;
                }
                if batch.z_test != prepass_last_z_test {
                    self.device.cmd_set_depth_test_enable(cmd, batch.z_test);
                    prepass_last_z_test = batch.z_test;
                }
                if batch.z_write != prepass_last_z_write {
                    self.device.cmd_set_depth_write_enable(cmd, batch.z_write);
                    prepass_last_z_write = batch.z_write;
                }
                if batch.z_function != prepass_last_z_function {
                    self.device.cmd_set_depth_compare_op(
                        cmd,
                        gamebryo_to_vk_compare_op(batch.z_function),
                    );
                    prepass_last_z_function = batch.z_function;
                }

                if prepass_global_bound {
                ...
                } else if let Some(mesh) = self.mesh_registry.get(batch.mesh_handle) {
                    // Per-mesh fallback — early startup before global
                    // VB/IB exists.
                    self.device
