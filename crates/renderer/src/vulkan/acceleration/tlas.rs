        ...
        for (i, draw_cmd) in draw_commands.iter().enumerate() {
            ...
            let blas_address: vk::DeviceAddress = if draw_cmd.bone_offset != 0 {
                ...
                let Some(Some(blas)) = self.blas_entries.get_mut(mesh_handle) else {
                    ...
                    // rigid BLAS from retained mesh buffers before entering
                    // `draw_frame`. Reaching this arm therefore means that
                    // recovery failed or the source mesh was ineligible; keep
                    // the integrity counter loud instead of publishing a TLAS
                    // instance with an invalid device address.
