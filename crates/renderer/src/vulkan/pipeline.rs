...
/// Depth pre-pass fragment SPIR-V bytes (#779). The prepass pipeline
/// pairs this with [`TRIANGLE_VERT_SPV`] (NOT a separate prepass vertex
/// shader): sharing the vertex shader between prepass and main pass
/// guarantees bit-identical FP math, so depth values match exactly
/// under `LESS_OR_EQUAL`. A separate vertex shader compiled to
/// different SPIR-V can drift one ULP and the main pass's early-Z
/// then rejects valid fragments — G-buffer writes vanish, lighting
/// blows up. See revert at 649996a for the failure mode.
///
/// `depth_prepass.frag` declares input locations 1, 4, 5 — a strict
/// subset of triangle.vert's outputs. Vulkan silently drops the
/// other 7 unused outputs.
pub const DEPTH_PREPASS_FRAG_SPV: &[u8] =
    include_bytes!("../../shaders/depth_prepass.frag.spv");
...
/// Depth pre-pass pipeline pair (#779). Reuses the triangle pipeline
/// layout (set 0 = bindless textures, set 1 = scene/material/instance
/// SSBOs) so existing per-frame descriptor binds work unchanged. Bound
/// to `depth_prepass_render_pass`, which has zero color attachments —
/// so this pipeline omits the color-blend state.
///
/// **Pairs `TRIANGLE_VERT_SPV` with `DEPTH_PREPASS_FRAG_SPV`.** Sharing
/// the vertex shader (rather than a separate prepass.vert) guarantees
/// bit-identical FP math — `gl_Position` from prepass is byte-equal
/// to the main pass's `gl_Position` for the same vertex, so depth
/// values match under `LESS_OR_EQUAL`. The prepass.frag declares only
/// the input locations it needs (a subset of triangle.vert's outputs);
/// Vulkan silently drops the unused outputs.
///
/// Returns `(opaque, opaque_two_sided)` — alpha-blend draws skip the
/// pre-pass entirely (they don't write depth), so we don't need a
/// blend variant.
pub fn create_depth_prepass_pipeline(
    ...
    let vert_module = load_shader_module(device, TRIANGLE_VERT_SPV)?;
    let frag_module = load_shader_module(device, DEPTH_PREPASS_FRAG_SPV)?;
    ...
    // Render pass has zero color attachments → empty blend state. The
    // pColorBlendState pointer is OPTIONAL when the subpass has no
    // color attachments per the Vulkan spec, but ash's builder requires
    // a value, so we provide an empty array.
    let color_blend_attachments: [vk::PipelineColorBlendAttachmentState; 0] = [];
    let color_blending = vk::PipelineColorBlendStateCreateInfo::default()
        .logic_op_enable(false)
        .attachments(&color_blend_attachments);

    // Match triangle pipeline's dynamic state set so the existing
    // per-batch `cmd_set_depth_*` / `cmd_set_cull_mode` / viewport /
    // scissor calls work unchanged in the prepass loop.
    let dynamic_states = [
        vk::DynamicState::VIEWPORT,
        vk::DynamicState::SCISSOR,
        vk::DynamicState::DEPTH_BIAS,
    ...
    let pipeline_infos = [
        ...
        vk::GraphicsPipelineCreateInfo::default()
            ...
            .render_pass(render_pass)
            .subpass(0),
        // [1] Opaque NONE-cull (foliage / cloth / glass billboards).
        vk::GraphicsPipelineCreateInfo::default()
            .stages(&shader_stages)
            .vertex_input_state(&vertex_input)
            .input_assembly_state(&input_assembly)
    ...
    let pipelines = unsafe {
        device
            .create_graphics_pipelines(pipeline_cache, &pipeline_infos, None)
            .map_err(|(_, err)| err)
            .context("Failed to create depth pre-pass pipelines (#779)")?
    };

    log::info!("Depth pre-pass pipelines created (BACK + NONE cull) — #779");
