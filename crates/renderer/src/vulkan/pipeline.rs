...
/// Depth pre-pass SPIR-V bytes (#779). Mirrors triangle.vert's transform +
/// skinning + jitter math; depth_prepass.frag does only alpha-test discard
/// so the pre-pass leaves a correct depth buffer for the main pass to
/// early-Z against. See #779.
pub const DEPTH_PREPASS_VERT_SPV: &[u8] =
    include_bytes!("../../shaders/depth_prepass.vert.spv");
...
/// so this pipeline omits the color-blend state. Same vertex input as
/// triangle.vert (the prepass shader is a strip-down with identical
/// transform math).
///
...
pub fn create_depth_prepass_pipeline(
    ...
    let vert_module = load_shader_module(device, DEPTH_PREPASS_VERT_SPV)?;
