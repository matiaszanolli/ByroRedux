...
/// Every production file in this directory, concatenated.
///
/// #3737 split `texture_registry.rs` into this directory, and
/// `vulkan/context/mod.rs`'s #3682 guard reads the registry as text. Two
/// of its four assertions are *negative* — "this field has not come back
/// as a hash map keyed by `TextureHandle`" — so a guard left pointing at
/// `mod.rs` alone would go on passing while the shape it forbids
/// reappeared in `lookup.rs` or `upload.rs`. A negative assertion over a
/// corpus that just shrank is the "passes by not looking" failure, so the
/// guard reads this instead of one file.
#[cfg(test)]
pub(crate) const SOURCES: &str = concat!(
    include_str!("lookup.rs"),
    include_str!("release.rs"),
    include_str!("upload.rs"),
    // LAST, and load-bearing: this is the only module here carrying a
    // `#[cfg(test)]` item, and a consumer that strips test text by brace
    // matching is not string-aware — an unbalanced `{` inside a literal
    // runs the strip into whatever follows. Nothing follows.
    include_str!("mod.rs"),
);
