...
// Selected only for DrawCommand::allows_early_fragment_tests(). The shared
// late-test module must retain discard-before-depth semantics for cutouts,
// effects and refraction. Never enable this globally (#779).
// Build triangle_early.frag.spv from this file with -DBYRO_OPAQUE_EARLY_TESTS=1.
#ifdef BYRO_OPAQUE_EARLY_TESTS
...
// Reservoir indices refer to the previous frame's priority-sorted light list.
uint remapReservoirLight(uint packed) {
    uint oldIndex = reservoirLightIndex(packed);
    return oldIndex < MAX_LIGHTS ? previousLightToCurrent[oldIndex] : 0xFFFFFFFFu;
}
    ...
    if (glassIORAllowed) {
        ...
        if (totalInternalReflection) {
            ...
            if (!refractionResolved) {
                ...
                vec3 tColor = rayHitAlbedo(tMat, tUV, tAlbedo, refrMip, dbgFlags);
    ...
    // Clustered lighting with streaming RIS shadow sampling.
        ...
        if (useRestir) {
            ...
            if (useTemporal && stableTemporalSurface
                ...
                uint rpLightIndex = remapReservoirLight(rp.lightAndSurface);
            ...
            if (useSpatial) {
                ...
                for (int sp = 0; sp < SPATIAL_SAMPLES; sp++) {
                    ...
                    uint rnLightIndex = remapReservoirLight(rn.lightAndSurface);
