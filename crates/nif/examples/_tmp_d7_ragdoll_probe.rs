...
fn main() {
    let bsa = std::env::args().nth(1).expect("usage: <bsa> <skeleton path>");
    let path = std::env::args().nth(2).expect("usage: <bsa> <skeleton path>");
    ...
    println!("bodies={} constraints={}", rag.bodies.len(), rag.constraints.len());
    ...
    for s in 0..n {
        ...
        while let Some(p) = q.pop_front() {
            for &c in &adj[p] {
                if !seen[c] { seen[c] = true; q.push_back(c); }
            }
        }
    }
    ...
    for c in &rag.constraints {
        match &c.kind {
            ImportedJointKind::Ragdoll { cone_max, twist_min, twist_max, plane_min, plane_max, .. } => {
                if *cone_max == 0.0 && *twist_min == 0.0 && *twist_max == 0.0 { degenerate += 1; }
                if (plane_min.abs() - plane_max.abs()).abs() > 1e-3 { plane_asym += 1; }
            ...
            ImportedJointKind::LimitedHinge { min_angle, max_angle, .. } => {
                if *min_angle == 0.0 && *max_angle == 0.0 { degenerate += 1; }
                println!("  Hinge   [{min_angle:.3},{max_angle:.3}]  {} <-> {}", rag.bodies[c.body_a].bone_name, rag.bodies[c.body_b].bone_name);
            }
        }
    }
    println!("degenerate-limit joints: {degenerate}; asymmetric-plane ragdoll joints: {plane_asym}");
