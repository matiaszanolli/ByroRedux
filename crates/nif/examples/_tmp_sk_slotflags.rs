            ...
            for block in scene.blocks.iter() {
                ...
                if !get(2).is_empty() && !tint_family {
                    if f2 & F2_GLOW_MAP != 0 {
                        note(
                            &mut cases,
                            &mut samples,
                            "AA slot2 authored, GLOW_MAP SET (correct)",
                            format!("{name} ty={ty} tex={}", get(2)),
                        );
                    } else {
                        note(
                            &mut cases,
                            &mut samples,
                            "AB slot2 authored, GLOW_MAP CLEAR (mis-roled)",
                            format!("{name} ty={ty} ec={ec:?} tex={}", get(2)),
                        );
                        if ec_lit {
                            note(
                                &mut cases,
                                &mut samples,
                                "AC slot2 GLOW_MAP CLEAR + lit emissive_color (LIVE)",
                                format!(
                                    "{name} ty={ty} ec={ec:?} em={} tex={}",
                                    sh.emissive_multiple,
                                    get(2)
                                ),
                            );
                        }
                    ...
                    if f2 & F2_RIM_LIGHTING != 0 && f2 & F2_GLOW_MAP == 0 {
                        ...
                        if ec_lit {
                            note(
                                &mut cases,
                                &mut samples,
                                "slot2 RIM->glow_map WITH LIT emissive_color",
                                format!(
                                    "{name} ty={ty} ec={ec:?} em={} tex={}",
                                    sh.emissive_multiple,
                                    get(2)
                    ...
                    if f2 & F2_SOFT_LIGHTING != 0 && f2 & F2_GLOW_MAP == 0 {
                        ...
                        if ec_lit {
                            note(
                                &mut cases,
                                &mut samples,
                                "slot2 SOFT->glow_map WITH LIT emissive_color",
                                format!(
                                    "{name} ty={ty} ec={ec:?} em={} tex={}",
                                    sh.emissive_multiple,
                                    get(2)
                ...
                if !get(3).is_empty() && ty != 4 && f1 & F1_FACEGEN_DETAIL != 0 {
                    note(
                        &mut cases,
                        &mut samples,
                        "slot3 FACEGENDETAIL->parallax",
                        format!("{name} ty={ty} tex={}", get(3)),
                    );
                }
                ...
                if !get(7).is_empty() && msn && ty != 11 && f2 & F2_BACK_LIGHTING != 0 {
                    note(
                        &mut cases,
                        &mut samples,
                        "slot7 BACKLIGHT+MSN->specular",
                        format!("{name} ty={ty} tex={}", get(7)),
                    );
                }
                ...
                if !get(7).is_empty() && !msn && ty != 11 {
                    note(
                        &mut cases,
                        &mut samples,
                        "slot7 no-MSN -> DROPPED",
                        format!(
                            "{name} ty={ty} f2bl={} tex={}",
                            f2 & F2_BACK_LIGHTING != 0,
                            get(7)
                        ),
                    );
                }
                ...
                if !get(6).is_empty() && ty != 11 && ty != 4 {
                    note(
                        &mut cases,
                        &mut samples,
                        "slot6 non-MLP -> DROPPED",
                        format!(
                            "{name} ty={ty} f2mlp={} tex={}",
                            f2 & F2_MULTILAYER != 0,
                            get(6)
                        ),
                    );
                ...
                if f2 & F2_MULTILAYER != 0 && ty != 11 {
                    note(
                        &mut cases,
                        &mut samples,
                        "F2_MULTILAYER but ty!=11",
                        format!("{name} ty={ty} s6={}", get(6)),
                    );
                }
                ...
                if tint_family && (!get(4).is_empty() || !get(5).is_empty()) {
                    note(
                        &mut cases,
                        &mut samples,
                        "tint-family slot4/5 authored -> DROPPED",
                        format!("{name} ty={ty} s4={} s5={}", get(4), get(5)),
                    );
                }
            }
        }
