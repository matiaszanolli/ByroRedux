        ...
        for name in &names {
            ...
            for block in scene.blocks.iter() {
                ...
                match p.global_vertex_data.as_ref() {
                    ...
                    None => { if is_dyn { dyn_no_buf += 1; } }
                }
            }
        }
    }
    println!("dyn_with_buf={dyn_with_buf} dyn_no_buf={dyn_no_buf}");
    println!("DYN partition-buffer attrs: {:?}", buf_attrs.iter().map(|(k,v)|(format!("0x{k:03x}"),*v)).collect::<Vec<_>>());
    println!("PLAIN partition-buffer attrs: {:?}", plain_buf_attrs.iter().map(|(k,v)|(format!("0x{k:03x}"),*v)).collect::<Vec<_>>());
}
