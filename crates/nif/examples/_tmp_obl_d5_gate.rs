...
fn main() {
    let mesh_bsa = std::env::args().nth(1).expect("usage: <meshes bsa> <textures bsa>");
    let tex_bsa = std::env::args().nth(2).expect("usage: <meshes bsa> <textures bsa>");
    ...
    for name in &files {
        ...
        for m in &imported.meshes {
            ...
            if mat.material_kind >= 100 { continue }
            if metal >= 0.3 { continue }
            if mat.env_map_scale > 0.3 { continue }
            if mat.textures.smooth_spec.is_some() { continue }
            // derive normal path the way spawn.rs does
            let base = mat.textures.base_color.and_then(|s| pool.resolve(s)).map(|s| s.to_string());
            let normal_path = match mat.textures.normal.and_then(|s| pool.resolve(s)) {
                Some(p) => p.to_string(),
                None => match &base {
                    Some(b) => match b.rfind('.') { Some(d) => format!("{}_n{}", &b[..d], &b[d..]), None => format!("{b}_n.dds") },
                    None => continue,
                },
            };
            let key = normal_path.to_ascii_lowercase().replace('/', "\\");
            let key = if key.starts_with("textures\\") { key } else { format!("textures\\{key}") };
            if !alpha_map.contains_key(&key) { normal_missing += 1; continue }
            gate_pass_pre_alpha += 1;
            let Ok(nb) = tex.extract(&key) else { continue };
            if nb.len() < 92 { continue }
            ...
            *classifier_hist.entry(format!("{classifier_rough:.2}")).or_insert(0) += 1;
            if r <= 0.10 && mirror_samples.len() < 8 { mirror_samples.push(format!("{name} g={g}")); }
        }
    }
