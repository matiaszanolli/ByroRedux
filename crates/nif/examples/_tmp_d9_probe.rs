...
fn main() {
    ...
    if mode == "list" {
        for path in &args[1..] {
            ...
            let n: Vec<String> = files.iter().map(|s| s.to_string())
                .filter(|f: &String| { let l = f.to_ascii_lowercase(); l.ends_with(".bgsm") || l.ends_with(".bgem") || l.ends_with(".mat") })
                .collect();
            println!("{path}: total={} bgsm/bgem/mat={}", files.len(), n.len());
            for f in n.iter().take(5) { println!("   {f}"); }
        }
        return;
    }
    // sample mode
    ...
    let mut n = 0u64;
    for path in &args[1..] {
        let Ok(a) = Ba2Archive::open(path) else { continue };
        let names: Vec<String> = a.list_files().into_iter()
            .filter(|f| f.to_ascii_lowercase().ends_with(".nif"))
            .map(|s| s.to_string()).take(3000).collect();
        for name in names {
            ...
            for i in 0..scene.blocks.len() {
                if let Some(sp) = scene.get_as::<BSLightingShaderProperty>(i) {
                    if !sp.material_reference { continue }
                    let nm = sp.net.name.as_deref().unwrap_or("");
                    if nm.to_ascii_lowercase().ends_with(".bgsm") || nm.to_ascii_lowercase().ends_with(".bgem") {
                        n += 1;
                        if samples.len() < 15 { samples.push(format!("{name} :: {nm}")); }
                    }
                }
            }
        }
