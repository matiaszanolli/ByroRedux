...
fn main() {
    ...
    for path in std::env::args().skip(1) {
        ...
        for name in arc.list_files().into_iter().filter(|n| n.to_ascii_lowercase().ends_with(".nif")).map(|s| s.to_string()) {
            ...
            for (i, block) in scene.blocks.iter().enumerate() {
                let Some(co) = block.as_any().downcast_ref::<BhkCollisionObject>() else { continue };
                *flag_hist.entry(co.flags).or_default() += 1;
                ...
                if extract_collision(&scene, BlockRef(i as u32)).is_none() { continue; }
                match l {
                    15 => if !noncol.contains(&name) { noncol.push(name.clone()) },
                    5 => if weapon.len() < 12 && !weapon.contains(&name) { weapon.push(name.clone()) },
                    6 => if !proj.contains(&name) { proj.push(name.clone()) },
                    8 => { biped_files.insert(name.clone()); *biped_per_file.entry(name.clone()).or_default() += 1; }
                    _ => {}
                }
            }
        }
    ...
    println!("\nNONCOLLIDABLE files ({}):", noncol.len()); for f in &noncol { println!("  {f}"); }
    println!("\nPROJECTILE files ({}):", proj.len()); for f in proj.iter().take(20) { println!("  {f}"); }
    println!("\nWEAPON sample: {weapon:?}");
    println!("\nBIPED files = {}", biped_files.len());
    let mut v: Vec<_> = biped_per_file.into_iter().collect();
    v.sort_by_key(|(_, c)| std::cmp::Reverse(*c));
    for (f, c) in v.into_iter().take(10) { println!("  {c:4}  {f}"); }
}
