...
fn main() {
    ...
    for nd in &imported.nodes {
        let q = Quat::from_xyzw(nd.rotation[0], nd.rotation[1], nd.rotation[2], nd.rotation[3]);
        let t = Vec3::new(nd.translation[0], nd.translation[1], nd.translation[2]);
        let local = GlobalTransform { translation: t, rotation: q, scale: nd.scale };
        let g = nd.parent_node.and_then(|p| rest.get(p))
            .map(|p| GlobalTransform::compose(p, t, q, nd.scale))
            .unwrap_or(local);
        if let Some(nm) = nd.name.as_ref() { rest_by_name.entry(nm.to_string()).or_insert(g); }
        rest.push(g);
    }

    println!("{:<20} {:>28} {:>28} {:>10}", "bone", "body root pose", "bone rest pose", "|local_t|");
    ...
    for b in &rag.bodies {
        ...
        println!("{:<20} {:>8.2},{:>8.2},{:>8.2} {:>8.2},{:>8.2},{:>8.2} {:>10.2}",
            b.bone_name, b.translation.x, b.translation.y, b.translation.z,
            r.translation.x, r.translation.y, r.translation.z, local_t.length());
    }
