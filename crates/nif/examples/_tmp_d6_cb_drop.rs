    ...
    for p in &files {
        ...
        for b in &scene.blocks {
            ...
            for cb in &seq.controlled_blocks {
                ...
                let Some(i) = scene.get_as::<NiTransformInterpolator>(idx) else { continue };
                f_total += 1;
        ...
        if f_drop > 0 {
            ...
            if f_drop == f_total {
                files_all_dropped += 1;
                if sample.len() < 8 { sample.push(format!("{p} ({f_drop}/{f_total})")); }
            }
        }
    }
    println!("controlled blocks -> NiTransformInterpolator: {cb_total}");
    println!("  dropped (null data_ref): {cb_dropped}  ({:.1}%)", 100.0*cb_dropped as f64/cb_total.max(1) as f64);
