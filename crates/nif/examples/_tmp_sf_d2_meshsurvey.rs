...
fn main() {
    ...
    let names: Vec<String> = a
        ...
        .map(|s| s.to_string())
        .collect();
    println!("{}: {} .mesh entries (of {})", path, names.len(), a.file_count());
        ...
        match BSGeometryMeshData::parse_from_bytes(&bytes) {
            Ok(d) => {
                ...
                *wpv.entry(d.weights_per_vert).or_default() += 1;
                ...
                if !(ok(d.uvs0.len()) && ok(d.uvs1.len()) && ok(d.colors.len())
                    && ok(d.normals_raw.len()) && ok(d.tangents_raw.len()))
                {
                    ragged += 1;
                }
            ...
            Err(e) => { parse_err += 1; if parse_err<=5 { println!("  ERR {} : {} ({} bytes, head {:02x?})", name, e, bytes.len(), &bytes[..bytes.len().min(24)]); } }
        }
    }
    println!("sampled={} parse_err={} sentinel={} ragged={} lods_present={} max_verts={}",
        n, parse_err, sentinel, ragged, lods_present, max_verts);
    println!("versions: {:?}", vers);
    println!("weights_per_vert: {:?}", wpv);
    println!("skinned={} rows==verts:{} rows!=verts:{} bone_idx_max={}",
        skinned, skin_rows_eq_verts, skin_rows_ne_verts, bone_idx_max);
    for e in &examples_skinned { println!("  ex: {}", e); }
}
