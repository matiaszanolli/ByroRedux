...
fn main() {
    ...
    let mut inc =
        |m: &mut BTreeMap<&'static str, u64>, k: &'static str| *m.entry(k).or_insert(0) += 1;
    ...
    for name in &files {
        ...
        for m in &imported.meshes {
            let mat = &m.material;
            meshes += 1;
            if mat.material_path.is_some() {
                inc(&mut c, "material_path");
                let e = samples.entry("material_path").or_default();
                if e.len() < 5 {
                    e.push(name.clone());
                }
            }
            ...
            if mat.metalness_override.is_some() {
                inc(&mut c, "metalness_override");
            }
            if mat.wireframe {
                inc(&mut c, "wireframe");
                let e = samples.entry("wireframe").or_default();
                if e.len() < 5 {
                    e.push(name.clone());
                }
            }
            if mat.flat_shading {
                inc(&mut c, "flat_shading");
                let e = samples.entry("flat_shading").or_default();
                if e.len() < 5 {
                    e.push(name.clone());
                }
            }
            ...
            if mat.has_alpha {
                inc(&mut c, "alpha_blend");
            }
            if mat.alpha_test {
                inc(&mut c, "alpha_test");
            }
            if !mat.z_test {
                inc(&mut c, "z_test_off");
            }
            if !mat.z_write {
                inc(&mut c, "z_write_off");
            }
            if mat.textures.base_color.is_some() {
                inc(&mut c, "tex_base");
            }
            if mat.textures.normal.is_some() {
                inc(&mut c, "tex_normal");
            }
            if mat.textures.dark.is_some() {
                inc(&mut c, "tex_dark");
                let e = samples.entry("tex_dark").or_default();
                if e.len() < 5 {
                    e.push(name.clone());
                }
            }
            if mat.textures.detail.is_some() {
                inc(&mut c, "tex_detail");
            }
            if mat.textures.smooth_spec.is_some() {
                inc(&mut c, "tex_gloss");
            }
            if mat.textures.emissive.is_some() {
                inc(&mut c, "tex_glow");
            }
            if mat.textures.height.is_some() {
                inc(&mut c, "tex_parallax");
                let e = samples.entry("tex_parallax").or_default();
                if e.len() < 5 {
                    e.push(name.clone());
                }
            }
            if mat.textures.environment.is_some() {
                inc(&mut c, "tex_env");
            }
            if mat.textures.decals.iter().any(|d| d.is_some()) {
                inc(&mut c, "tex_decalslot");
            }
            if mat.specular_strength == 0.0 {
                inc(&mut c, "spec_strength_zero");
            }
            if mat.env_map_scale != 0.0 {
                inc(&mut c, "env_map_scale_nonzero");
            }
            if mat.uv_scale != [1.0, 1.0] || mat.uv_offset != [0.0, 0.0] {
                inc(&mut c, "uv_transform");
            }
            *clamp.entry(mat.texture_clamp_mode).or_insert(0) += 1;
            if mat.alpha_test {
                *atfunc.entry(mat.alpha_test_func).or_insert(0) += 1;
            }
            if mat.has_alpha {
                *blend
                    .entry((mat.src_blend_mode, mat.dst_blend_mode))
                    .or_insert(0) += 1;
            }
