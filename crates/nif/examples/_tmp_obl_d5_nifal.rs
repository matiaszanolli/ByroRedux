...
fn main() {
    ...
    for name in &files {
        ...
        for m in &imported.meshes {
            ...
            *gloss_scalar.entry(format!("{:.0}", mat.glossiness)).or_insert(0) += 1;
            if !mat.glossiness.is_finite() { nan_gloss += 1; }
            ...
            *kind.entry(mat.material_kind).or_insert(0) += 1;
            ...
            if has_gloss && !has_normal { gloss_no_normal += 1; }
            if has_gloss && has_normal { gloss_and_normal += 1; }
            if !has_normal { no_normal_slot += 1; }
            let e = mat.emissive_color;
            if e[0] > 0.0 || e[1] > 0.0 || e[2] > 0.0 {
                emis_nonzero += 1;
                if format!("{:?}", mat.emissive_source) == "None" {
                    emis_nonzero_src_none += 1;
                    if none_esrc_samples.len() < 5 { none_esrc_samples.push(name.clone()); }
                }
            }
        }
    }
