        ...
        for name in &names {
            ...
            for block in scene.blocks.iter() {
                if let Some(s) = block.as_any().downcast_ref::<BsTriShape>() {
                    shapes_total += 1;
                    if matches!(s.kind, BsTriShapeKind::Dynamic { .. }) {
                        dyn_shapes += 1;
                    }
                    if s.data_size == 0 && s.skin_ref.index().is_some() {
                        recon_candidates += 1;
                    }
                }
            }
            ...
            for m in &imported {
                ...
                if m.normals.len() != n {
                    mesh_norm_len_mismatch += 1;
                }
                if !m.uvs.is_empty() && m.uvs.len() != n {
                    mesh_uv_len_mismatch += 1;
                }
                if m.uvs.is_empty() {
                    mesh_uv_len_mismatch += 0;
                }
                if m.colors.len() != n {
                    mesh_col_len_mismatch += 1;
                }
                if m.tangents.is_empty() {
                    mesh_no_tangents += 1;
                } else if m.tangents.len() != n {
                    mesh_tan_len_mismatch += 1;
                } else {
                    for i in 0..n {
                        let t = m.tangents[i];
                        let nl = m.normals[i];
                        let tl = (t[0] * t[0] + t[1] * t[1] + t[2] * t[2]).sqrt();
                        if tl > 1e-4 && (tl - 1.0).abs() > 0.05 {
                            bad_tan_norm += 1;
                        }
                ...
                if let Some(sk) = &m.skin {
                    meshes_skinned += 1;
                    if sk.vertex_bone_weights.is_empty() {
                        skin_w_missing += 1;
                    } else if sk.vertex_bone_weights.len() != n {
                        skin_len_mismatch += 1;
                    }
                    if sk.vertex_bone_indices.is_empty() {
                        skin_idx_missing += 1;
                    }
                }
            }
