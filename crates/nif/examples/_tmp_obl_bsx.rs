//! TEMP scratch (audit 2026-08-16): Oblivion BSXFlags bit-5 blast radius.
//! Counts pre-FO4 NIFs with BSXFlags bit 5 set and how many still carry
//! real geometry/collision AFTER the per-node `is_editor_marker` name
//! filter — i.e. the set the cell loader drops wholesale.
...
fn main() {
    ...
    let mut samples: Vec<(String, usize, usize, usize, u32, u32)> = Vec::new();
    ...
    for (n, m, t, c, b, v) in samples.iter() {
        println!("  {n}\tmeshes={m}\ttris={t}\tcolliders={c}\tbsx=0x{b:X}\tbsver={v}");
    }
}
