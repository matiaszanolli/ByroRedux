...
fn main() {
    ...
    for path in std::env::args().skip(1) {
        ...
        for name in &files {
            ...
            let Ok((hdr, data_start)) = byroredux_nif::header::NifHeader::parse(&bytes) else {
                continue;
            };
                                    ...
                                    *sub_count_hist.entry(n).or_default() += 1;
                                    ...
                                    if layers.len() > 1 {
                                        ...
                                        if mixed_layer_examples.len() < 10 {
                                            mixed_layer_examples.push(format!(
                                                "{name} layers={:?}",
                                                layers.iter().map(|&l| fol(l)).collect::<Vec<_>>()
                                            ));
                                        }
                                    }
                                    if mats.len() > 1 {
