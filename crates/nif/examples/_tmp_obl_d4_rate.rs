...
fn main() {
    ...
    for name in &files {
                        ...
                        match interp.data_ref.index() {
                            Some(d) => match scene.get_as::<NiFloatData>(d) {
                                Some(fd) => match fd.keys.keys.first() {
                                    Some(k) if k.value.is_finite() && k.value > 0.0 && k.value < 3.0e38 => "OK keyed".to_string(),
                                    Some(k) => format!("first key rejected ({}) nkeys={} const={}", k.value, fd.keys.keys.len(), interp.value),
                                    None => format!("NiFloatData 0 keys, const={}", interp.value),
                                },
                                None => format!("data_ref -> not NiFloatData ({}), const={}", scene.blocks.get(d).map(|x| x.block_type_name()).unwrap_or("?"), interp.value),
                            },
                            None => {
                                if interp.value.is_finite() && interp.value > 0.0 && interp.value < 3.0e38 { "OK const".to_string() }
                                else { format!("data NULL + const rejected ({})", interp.value) }
                            }
                        }
                    } else {
                        format!("interp is {} (not NiFloatInterpolator)", b.block_type_name())
                    }
                }
            },
        };
        ...
        *reasons.entry(key.clone()).or_insert(0) += 1;
        ...
        if e.len() < 4 { e.push(format!("{name} :: {reason}")); }
    }
    for (k, v) in &reasons {
        println!("{v:5}  {k}");
        for s in &samples[k] { println!("         {s}"); }
    }
}
