...
fn probe(b: &[u8]) -> Result<(u32, usize, bool, bool, bool), String> {
    ...
    if scale <= 0.0 {
        return Ok((ver, 0, false, true, true));
    }
    ...
    if wpv != 0 {
        r.skip((ntw as usize / wpv as usize) * wpv as usize * 4)
            .ok_or("weights")?;
    }
    ...
    let _ = after_lods;
    if ends_after_lods {
        return Ok((ver, nv as usize, true, false, sane));
    }
    ...
    if r.o != b.len() {
        return Err(format!("trailing {} bytes", b.len() as i64 - r.o as i64));
    }
...
fn main() {
    ...
    let mut ends_after_lods = 0usize;
    let mut has_meshlets = 0usize;
    ...
    println!(
        "{} sampled={} ends_at_lods(no meshlet tail)={} full_tail={} sentinel={} ragged={}",
