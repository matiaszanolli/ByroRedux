...
use byroredux_nif::blocks::properties::{NiFlagProperty, NiStencilProperty, NiTexturingProperty, NiVertexColorProperty};
...
fn main() {
    ...
    for name in &files {
        ...
        for b in scene.blocks.iter() {
            if let Some(fp) = b.as_any().downcast_ref::<NiFlagProperty>() {
                match fp.block_type_name() {
                    "NiShadeProperty" => { shade_blocks += 1; if !fp.enabled() { shade_zero += 1; } }
                    "NiWireframeProperty" => { wire_blocks += 1; }
                    _ => {}
                }
            }
        ...
        for b in scene.blocks.iter() {
            let Some(shape) = b.as_any().downcast_ref::<NiTriShape>() else { continue };
            shapes += 1;
            ...
            for (i, pr) in shape.av.properties.iter().enumerate() {
                ...
                let Some(pb) = scene.blocks.get(idx) else { continue };
                match pb.block_type_name() {
                    "NiMaterialProperty" => { if mat_pos.is_none() { mat_pos = Some(i); } }
                    "NiVertexColorProperty" => {
                        if vcp_pos.is_none() { vcp_pos = Some(i); }
                    ...
                    "NiTexturingProperty" => {
                        if let Some(tp) = pb.as_any().downcast_ref::<NiTexturingProperty>() {
                            if let Some(bt) = tp.base_texture.as_ref() { clamps.push((bt.flags & 0xF) as u8); }
                        }
                    }
                    _ => {}
                }
            ...
            if clamps.len() > 1 {
                ...
                if clamps.windows(2).any(|w| w[0] != w[1]) {
                    clamp_conflict += 1;
                    if samples2.len() < 5 { samples2.push(format!("{name} clamps={clamps:?}")); }
                }
            }
                    ...
                    Some(mp) if mp < vp => {
                        ...
                        if let Some((vm, lm)) = vcp_mode {
                            if !(vm == 2 && lm == 1) {
                                ...
                                if samples.len() < 8 { samples.push(format!("{name} vm={vm} lm={lm} matpos={mp} vcppos={vp}")); }
                            }
                        }
                    }
                    _ => { vcp_before += 1; }
