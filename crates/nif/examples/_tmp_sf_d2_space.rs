...
fn main() {
    ...
    let nifs: Vec<String> = ar
        .list_files()
        .into_iter()
        .filter(|n| n.to_ascii_lowercase().ends_with(".nif"))
        .map(|s| s.to_string())
        .collect();
    let mut contains_raw = 0;
    let mut contains_swap = 0;
    let mut contains_neither = 0;
    let mut n = 0;
    let mut bmm_raw = 0;
    let mut bmm_swap = 0;
    let mut bmm_neither = 0;
    let mut shown = 0;
    let mut metric_ok = 0;
    let mut metric_bad = 0;
    let mut ratios: Vec<f32> = Vec::new();
    for name in nifs.iter().step_by(stride) {
        let Ok(bytes) = ar.extract(name) else {
            continue;
        };
        ...
        for blk in &scene.blocks {
            ...
            let Ok(mb) = ar.extract(&format!("geometries\\{mesh_name}.mesh")) else {
                continue;
            };
            ...
            for p in &d.vertices {
                ...
                ds = ds.max(
                    ((p[0] - cs[0]).powi(2) + (p[1] - cs[1]).powi(2) + (p[2] - cs[2]).powi(2))
                        .sqrt(),
                );
                for i in 0..3 {
                    mn[i] = mn[i].min(p[i]);
            ...
            match (dr <= tol, ds <= tol) {
                ...
                _ => {
                    ...
                    if shown < 6 {
                        println!(
                            "  NEITHER {} r={} dr={} ds={} c={:?} aabb={:?}..{:?}",
                            name, r, dr, ds, c, mn, mx
                        );
                        shown += 1;
                    }
                }
    ...
    println!(
        "n={} sphere contains: raw={} swap={} neither={}",
        n, contains_raw, contains_swap, contains_neither
    );
    ratios.sort_by(|a, b| a.partial_cmp(b).unwrap());
    if !ratios.is_empty() {
        println!(
            "dr/r ratio: min={} p50={} p95={} max={}",
            ratios[0],
            ratios[ratios.len() / 2],
    ...
    println!(
        "metric-space (verts/69.969) inside sphere: ok={} bad={}",
        metric_ok, metric_bad
    );
    println!(
        "bound_min_max fits: raw={} swap={} neither={}",
        bmm_raw, bmm_swap, bmm_neither
    );
}
