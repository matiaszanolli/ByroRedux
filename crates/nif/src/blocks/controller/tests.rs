...
fn bhk_blend_controller_dispatches_via_parse_block() {
    ...
    let mut stream = NifStream::new(&data, &header);
    let block =
        crate::blocks::parse_block("bhkBlendController", &mut stream, Some(data.len() as u32))
...
fn ni_light_float_controller_dispatches_preserving_rtti() {
    for type_name in [
        ...
        let mut stream = NifStream::new(&data, &header);
        let block = crate::blocks::parse_block(type_name, &mut stream, Some(data.len() as u32))
