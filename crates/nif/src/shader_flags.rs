...
// ── ShaderFlags typed view (#1277 Task 6) ───────────────────────────────
//
// The on-disk shader-flag storage diverges three ways across games:
//
//   1. FO3 / FNV — `BSShaderFlags` (F1) + `BSShaderFlags2` (F2) u32 pair
//      on `BSShaderPPLightingProperty` / `BSShaderNoLightingProperty`.
//      Bit vocabulary in [`fo3nv_f1`] / [`fo3nv_f2`].
//
//   2. Skyrim LE/SE / FO4 — `SkyrimShaderPropertyFlags1/2` (SLSF1/2) or
//      `Fallout4ShaderPropertyFlags1/2` (F4SF1/2) u32 pair on
//      `BSLightingShaderProperty` / `BSEffectShaderProperty`. Bit vocabulary
//      in [`skyrim_slsf1`] / [`skyrim_slsf2`] / [`fo4_slsf1`] / [`fo4_slsf2`]
//      (mostly overlapping; per-bit doc comments pin the differences).
//
//   3. FO76 / Starfield (BSVER >= `FO4_CRC_FLAGS`/132) — same conceptual
//      flag set but encoded as `Vec<u32>` CRC32 hash arrays
//      (`sf1_crcs` / `sf2_crcs`). The legacy u32 fields are stored as
//      literal zeros per the parser branch at `blocks/shader.rs:604`.
//      Bit vocabulary in [`bs_shader_crc32`].
//
// Today consumers test each case with separate `is_decal_from_legacy_*` /
// `is_decal_from_modern_*` helpers (in `import/material/mod.rs`) and the
// raw `flags1` / `flags2` / `sf1_crcs` / `sf2_crcs` fields. The choice of
// which helper to call requires knowing the BSVER context — a wrong call
// silently returns the wrong answer (flags2 bit 21 is `Alpha_Decal` on
// FO3/FNV but `Cloud_LOD` on Skyrim and `Anisotropic_Lighting` on FO4).
//
// `ShaderFlags` is the typed wrapper that makes the storage choice
// type-visible: a single `match` on the variant pattern-matches the three
// cases and dispatches to the correct bit vocabulary. Consumers that
// adopt it can't accidentally test a FO3 bit-21 decal flag on a Skyrim
// shader.
//
// Backwards compat: the four raw fields on `BsShaderBase` are kept; this
// type is an opt-in borrowed view. Existing callers stay green.

use crate::version::NifVariant;

/// Typed, NifVariant-aware view over the four shader-flag storage slots
/// a `BSLightingShaderProperty` / `BSEffectShaderProperty` /
/// `BSShaderPPLightingProperty` / `BSShaderNoLightingProperty` carries.
///
/// Borrowed — does not own the data. Construct via [`ShaderFlags::classify`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ShaderFlags<'a> {
    /// FO3 / FNV: u32 pair with [`fo3nv_f1`] / [`fo3nv_f2`] bit vocab.
    LegacyFo3Fnv { sf1: u32, sf2: u32 },
    /// Skyrim LE/SE / FO4: u32 pair with Skyrim/FO4 bit vocab
    /// (mostly overlapping — see per-bit docs in [`skyrim_slsf1`] etc.).
    LegacySkyrimFo4 { sf1: u32, sf2: u32 },
    /// FO76 / Starfield (BSVER >= 132): CRC32 arrays replace typed u32.
    Crc32 {
        sf1_crcs: &'a [u32],
        sf2_crcs: &'a [u32],
    },
...
impl<'a> ShaderFlags<'a> {
    /// Pick the variant from raw shader-property fields + the NifVariant
    /// that produced them. CRC32 wins when the arrays are non-empty
    /// (BSVER >= 132 populates them and zeros the typed u32 fields per
    /// the parser branch at `blocks/shader.rs:604`); otherwise the
    /// FO3/FNV vs Skyrim/FO4 split is decided by the variant's
    /// [`NifVariant::has_shader_property_fo3_fields`] predicate.
    pub fn classify(
        variant: NifVariant,
        flags1: u32,
        flags2: u32,
    ...
    /// True when storage is the FO76+/Starfield CRC32-arrays format.
    /// Diagnostic-only — consumers should prefer `is_decal()` etc. to
    /// avoid branching on storage.
    pub fn is_crc32(&self) -> bool {
        matches!(self, ShaderFlags::Crc32 { .. })
    }

    /// Test the `Decal` predicate using the correct bit vocabulary for
    /// the variant. Wraps the existing `is_decal_from_*` helper logic
    /// behind a single type-dispatched call.
    ///
    /// Note: pre-2026-05 the SLSF1 bit-26 / bit-27 numeric values
    /// coincided across all three FO3/FNV/Skyrim/FO4 vocabularies (per
    /// the compile-time assertions in `import/material/mod.rs`), so the
    /// legacy arms share an implementation. The FO3/FNV-only `flags2`
    /// bit 21 (`Alpha_Decal`) is tested in the FO3/FNV arm only — it's
    /// `Cloud_LOD` on Skyrim and `Anisotropic_Lighting` on FO4, both of
    /// which must NOT classify as decal.
    pub fn is_decal(&self) -> bool {
        match self {
            ShaderFlags::LegacyFo3Fnv { sf1, sf2 } => {
                sf1 & (fo3nv_f1::DECAL | fo3nv_f1::DYNAMIC_DECAL) != 0
                    || sf2 & fo3nv_f2::ALPHA_DECAL != 0
            }
    ...
    /// Test the `Double_Sided` predicate. FO3/FNV has no SLSF2
    /// double-sided bit — that case is detected via `NiStencilProperty`
    /// instead and is OUT OF SCOPE for this method (returns false). The
    /// callsite that wants legacy two-sided detection must also check
    /// the stencil property; future work could plumb that through here.
...
#[cfg(test)]
mod shader_flags_tests {
    use super::*;

    #[test]
    fn classify_picks_crc32_when_arrays_populated() {
        let crcs = vec![bs_shader_crc32::DECAL];
        let flags = ShaderFlags::classify(NifVariant::Fallout76, 0, 0, &crcs, &[]);
        assert!(matches!(flags, ShaderFlags::Crc32 { .. }));
        assert!(flags.is_crc32());
    }

    #[test]
    fn classify_picks_legacy_fo3fnv_for_fnv_variant_with_empty_crcs() {
        let flags = ShaderFlags::classify(NifVariant::FalloutNV, 0, 0, &[], &[]);
        assert!(matches!(flags, ShaderFlags::LegacyFo3Fnv { .. }));
        assert!(!flags.is_crc32());
    }

    #[test]
    fn classify_picks_legacy_skyrim_fo4_for_skyrim_variant() {
        let flags = ShaderFlags::classify(NifVariant::SkyrimSE, 0, 0, &[], &[]);
        assert!(matches!(flags, ShaderFlags::LegacySkyrimFo4 { .. }));
        let flags = ShaderFlags::classify(NifVariant::Fallout4, 0, 0, &[], &[]);
        assert!(matches!(flags, ShaderFlags::LegacySkyrimFo4 { .. }));
    }

    #[test]
    fn is_decal_picks_correct_vocabulary_per_variant() {
        // FO3/FNV: bit 26 of flags1.
        let flags = ShaderFlags::classify(NifVariant::FalloutNV, fo3nv_f1::DECAL, 0, &[], &[]);
        ...
        let flags = ShaderFlags::classify(NifVariant::SkyrimSE, 0, 0x0020_0000, &[], &[]);
        assert!(
            !flags.is_decal(),
            "Skyrim flags2 bit-21 (Cloud_LOD) must NOT classify as decal"
        );

        // FO4: flags2 bit 21 is Anisotropic_Lighting, NOT decal.
        let flags = ShaderFlags::classify(NifVariant::Fallout4, 0, 0x0020_0000, &[], &[]);
        assert!(
            !flags.is_decal(),
            "FO4 flags2 bit-21 (Anisotropic_Lighting) must NOT classify as decal"
        );
        ...
        assert!(
            flags.is_decal(),
            "FO76 CRC32 DYNAMIC_DECAL hash in sf2_crcs must classify as decal"
        );

        // Empty: no decal.
        let flags = ShaderFlags::classify(NifVariant::FalloutNV, 0, 0, &[], &[]);
        assert!(!flags.is_decal());
    }
    ...
    fn is_two_sided_picks_correct_vocabulary_per_variant() {
        ...
        // the is_two_sided docstring.)
        let flags = ShaderFlags::classify(NifVariant::FalloutNV, 0, 0xFFFF_FFFF, &[], &[]);
        assert!(
            !flags.is_two_sided(),
            "FO3/FNV must NOT report two-sided from shader flags — \
             that's NiStencilProperty's job"
        );
    ...
    #[test]
    fn classify_with_crc_arrays_overrides_legacy_fields_even_on_legacy_variant() {
        // Edge case: a malformed plugin that ships both legacy u32 AND
        // CRC arrays. Storage rule wins — CRC arrays mean BSVER >= 132,
        // so the legacy fields should be ignored even on (e.g.) FNV
        // classification (which probably means the variant detection
        // mis-classified, but the data clearly came from a CRC-era file).
        let crcs = vec![bs_shader_crc32::DECAL];
        let flags = ShaderFlags::classify(NifVariant::FalloutNV, 0xFFFF_FFFF, 0, &crcs, &[]);
        assert!(matches!(flags, ShaderFlags::Crc32 { .. }));
        assert!(flags.is_decal());
    }
}
