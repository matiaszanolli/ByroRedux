...
| ~~`glass_roughness_scratch` / `glass_dirt_overlay`~~ | `BGEM` (v21+/v22) | **NOT PARKED — routed since #2109 (CLOSED).** Both are named roles on `MaterialTextureSet<T>` (`crates/nif/src/import/types.rs`), carry `supplemental_texture_slot::GLASS_ROUGHNESS_SCRATCH` / `GLASS_DIRT_OVERLAY` lanes on `GpuMaterial`, and are sampled by `triangle.frag`. Row kept struck-through rather than deleted because the claim survived #2109 by 33 lines below this section's own "22 named roles" count and was filed again as #3907 | (consumed — no longer blocked) |
