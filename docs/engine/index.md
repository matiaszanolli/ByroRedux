...
| [NPC Spawn → AI Packages](npc-spawn-ai-packages.md) | byroredux, plugin, core | Cross-cutting: NPC_ spawn dispatch, PACK record parsing, package selection, and the currently runtime-backed procedures — honest about subsystem-blocked gaps. FO3/FNV-only; see [PACKAL](packal.md) for the Skyrim+ ambient gap. |
...
| [PACKAL — Package Abstraction Layer](packal.md) (PROPOSED) | plugin, scripting, byroredux | Unlike its `*AL` siblings, the parse-time fork is already unified (`PackRecord` carries both PACK shapes); the real gap is execution-time — Scene-driven package execution exists, ambient (non-Scene) execution doesn't understand Skyrim+'s tree/template shape at all. Real-data check: 100% of Skyrim.esm's ambient-only packages are Skyrim+-shaped. |
