...
| [EXAL — SpeedTree Full Rendering](exal-trees.md) | byroredux, spt, renderer | EXAL sub-doc, PROPOSED. Fills the near-field ring `exal.md` §5's distant LOD system already brackets: `.spt` holds no geometry to decode (#3808, 2026-09-07; the Phase 2/3 re-scoping is recorded in §3), branch/frond + leaf-card import shape, RT/BLAS boundary, wind reusing ground cover's `WindField` |
...
| [SKYAL — Sky Abstraction Layer](skyal.md) | renderer, byroredux | PARTIAL. Downstream of EXAL, so game-agnostic by construction. Shared `include/sky.glsl` + the measured bloom-overdrive fix are landed; sky cubemap bake (RT miss / reflections / ambient) and the volumetric cloud layer are specified, not built. Carries the deterministic sky-capture harness and its two traps (sRGB screenshots, stale `.spv` builds) |
...
| [Built-in Fixes](builtin-fixes.md) | plugin, scripting, nif, byroredux | PROPOSED. Every script-extender and unofficial-patch fix for every supported game built into core: per-game fix catalog, cited and re-derived (never redistributed), guarded by vanilla preconditions, applied after official content so user plugins still win |
