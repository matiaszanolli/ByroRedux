...
| [PACKAL — Package Abstraction Layer](packal.md) | plugin, scripting, byroredux | Unlike its `*AL` siblings, the parse-time fork is already unified (`PackRecord` carries both PACK shapes); the real gap is execution-time — Scene-driven package execution exists, ambient (non-Scene) execution didn't understand Skyrim+'s tree/template shape at all. First slice (Skyrim+ ambient Sandbox) shipped 2026-08-19: 722 of 2,052 real `Skyrim.esm` NPCs now resolve a Sandbox radius, up from 0. |
