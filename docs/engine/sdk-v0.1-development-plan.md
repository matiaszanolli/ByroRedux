...
package: ten SKSE `Game` content calls plus the vanilla Papyrus
`Game.GetFormFromFile` lookup cover regular/light counts, name-to-index and
index-to-name lookup, active plugin, dependency-count, light-plugin master
queries, and portable plugin-local form lookup. They preserve
...
- Curated SKSE, F4SE, xNVSE, and OBSE script API aliases backed by the common
  service catalog. **The first exact SKSE pack is live: ten `Game` regular and
