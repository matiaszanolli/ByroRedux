...
| Reactive mask | New render-resolution `R8_UNORM`, cleared to zero and MAX-blended during transparency. Glass, particles, and alpha-blended decals use `min(alpha, 0.9)`; water and the screen-composition proxy write `1.0` because their shading can change independently of surface motion. Stable alpha-blended meshes still write motion where available. |
