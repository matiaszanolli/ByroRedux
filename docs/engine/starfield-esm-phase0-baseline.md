...
3. **Does STAT decode SF base records?** 20 607 immediate STAT records vs the 254 MB CELL payload. If a Cydonia REFR references a STAT base form id and the STAT didn't get indexed, the REFR is silently skipped (no geometry spawned) — see the `statics.get` miss branch in `byroredux/src/cell_loader/references/mod.rs`.
4. **GBFM frequency answer**: 3 141 records in the vanilla ESM. That's significant but not dominant — and crucially, the GBFM total bytes (36 MB) are dwarfed by WRLD/CELL/STAT. **Recommendation: Phase 3 should stub GBFM (warn-once-and-skip pattern) rather than parse it.** A Cydonia REFR pointing at a GBFM-templated base would be silently skipped (no geometry spawned), but the Phase 0 measurement gives us a way to count those after Phase 5 — if the missing-form-id count is dominated by GBFM-targeted refs, Phase 3.5 promotes GBFM. If GBFM-targeted refs are <10% of skipped refs, defer.
   **Measured 2026-08-30 (SF-D4-01) — rule fires DEFER, decision closed**: decomposing every unresolved REFR across three Starfield cells, GBFM-targeted refs are 2 of 2,465 (0.081%) in Cydonia, 1 of 895 in CydoniaMainLevel02, 0 of 4,218 in Nishina01 — far under the 10% promote threshold. The same decomposition surfaces the actual dominant class, unranked until now: **PDCL decals at 74.9%** of Cydonia's unresolved REFRs — applying this same rule to PDCL would fire "promote". 92.9% of Cydonia's 2,465 unresolved REFRs are non-geometry by design (PDCL decals, audio, actors, consciously skipped types); only 175 of 27,898 REFRs (0.63%) are genuinely missing placeable geometry, so Cydonia's effective geometry resolve rate is ~99.4%, not the headline 91.16% — the 8.8% unresolved is not primarily a geometry gap. Of those 175, 144 are the model-less STAT/BNDS/ACTI/ARMO records #1576 already tracks (BNDS 60 + STAT 44 + ACTI 33 + ARMO 7).
...
- **`sf_parse_check`** is currently a one-shot binary; promote to a regression test (_tests/sf_parse_baseline.rs_ — proposed, does not exist yet) gated on `BYROREDUX_STARFIELD_DATA` so any future commit that breaks SF parsing fails the build.
