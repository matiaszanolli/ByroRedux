...
|--------|-----------|----------|
| `cargo run --release -p byroredux-scripting --example mq101_conformance` | MQ101 intro vertical-slice preflight | Production ESM/BSA/PEX paths recover the `MQ101` quest, aliases, stage-fragment bindings, attached properties, critical intro scripts, cart HKX files, and localized FUZ dialogue. Also reports the exact share of bound quest fragments the current effect lowerer understands. This is a data-only preflight; it does not need Vulkan or claim the cinematic runtime is implemented. Pass a data-directory argument or set `BYROREDUX_SKYRIM_DATA` to override the default install path. |
