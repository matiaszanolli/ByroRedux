...
| [`p2-melee-core.sh`](p2-melee-core.sh) | Playable slice P2 combat-core checkpoint | Loads frozen Bleak Falls reference `000380B4`, uses setup-only `combat.approach` to place the real character capsule, and drives seven unarmed swings through the live Attack binding. Hard gates require 50 Health → seven `HitEvent`s at 8 damage → one `Dead` transition → the existing 18-body ragdoll. Loot/UI/save continuity remain outside this core checkpoint. |
