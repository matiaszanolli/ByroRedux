...
| `cargo test -p byroredux -- --ignored installed_oblivion_creature_assets_resolve_from_their_records` | #2567 Oblivion creature-asset corpus guard | Every creature path this module derives (skeleton + NIFZ parts) must resolve against a real installed `Oblivion.esm` + every `*meshes.bsa` (base + Shivering Isles), asserting ≥90% resolution — not just "routes through the actor pipeline". Needs `BYROREDUX_OBLIVION_DATA` (defaults to the local Steam install path). |
