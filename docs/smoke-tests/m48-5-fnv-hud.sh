#!/usr/bin/env bash
# M48.5 Fallout: New Vegas HUD smoke on a DLC launch (#4718) — the FNV twin
# of m48-5-fo3-hud.sh. `--hud` used to pick FO3 vs FNV from the `--esm` file
# name, so the documented DLC/mod shape (`--master FalloutNV.esm --esm
# HonestHearts.esm`) got the FO3 profile: `Fallout - Textures.bsa`, which
# holds none of FNV's HUD art or fonts, and a fully transparent HUD while
# `hud: loaded` still logged success. This script launches exactly that
# shape and gates on the resolved profile as well as on pixels.
#
# It verifies the game-agnostic MenuXml HUD resolves the FNV profile from the
# load order, opens `Fallout - Textures.bsa` plus its `Textures2` sibling,
# assembles the meters + compass, and responds to console control.
#
# Workflow (same `--bench-hold` + byro-dbg pattern as m48-4-oblivion-hud.sh):
#   1. Spawn the engine on an FNV interior (Doc Mitchell's house) with
#      `--master FalloutNV.esm --esm HonestHearts.esm --hud`.
#   2. Gate on the `hud: loaded` boot line naming the New Vegas profile, and
#      on the absence of the compass-strip-unavailable warning.
#   3. byro-dbg: `hud.status`, pin the two bars (`hud.values <hp> <ap>` — the
#      Fallout profiles drive 2 bars, not Oblivion's 3), capture screenshots.
...
#      band is inked.
#
# Pre-conditions: Fallout: New Vegas with the Honest Hearts DLC installed
# (BYROREDUX_FNV_DATA or the default Steam path), Vulkan device, Xvfb.
#
...
# Gate 1 — HUD booted from the FNV archives *as* New Vegas. The profile is
# the #4718 regression: the FO3 profile also logs `hud: loaded`, so the line
# alone proves nothing. Its texture pool must also have taken the Textures2
# sibling, and the compass strip (FNV art, Textures2 only) must have resolved.
grep -q "hud: loaded menus" "$OUT_DIR/engine.log" || { echo "FAIL: no 'hud: loaded' line"; exit 1; }
grep -q "hud: loaded .* profile='Fallout: New Vegas'" "$OUT_DIR/engine.log" \
    || { echo "FAIL: the DLC launch did not resolve the New Vegas profile (#4718)"; grep "hud: loaded" "$OUT_DIR/engine.log"; exit 1; }
grep -q "Opened HUD textures archive: '.*Fallout - Textures2.bsa'" "$OUT_DIR/engine.log" \
    || { echo "FAIL: the HUD texture pool did not include Textures2"; exit 1; }
if grep -q "hud: compass strip .* unavailable" "$OUT_DIR/engine.log"; then
    echo "FAIL: the compass strip did not resolve"; exit 1
fi
echo "PASS: hud loaded as New Vegas"
...
# once: the FNV fixture cell keeps streaming content well after
...
# Gate 2 — the console reports a launched HUD with the Fallout bar labels.
grep -q 'hud: launched visible=true hp=' "$OUT_DIR/dbg.out" || { echo "FAIL: hud.status did not report the Fallout bars"; exit 1; }
echo "PASS: hud.status"

# Gate 3 — pixel gates over the composited captures.
#
# The Fallout meters are tick rows (repeated 8x20 `hud_tick_mark.dds`, drawn
...
echo "M48.5 Fallout: New Vegas HUD smoke (DLC launch): PASS"
